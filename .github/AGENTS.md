...
- GitHub 把正文 / 标题编辑和 PR base 变更都归入 `edited`。Quality / Desktop 的 PR concurrency 以 PR number 和当前不可变 base SHA 为 lane：同一 base 的 source 与普通 metadata edit 共用 lane，后者不取消正在运行的 source validation，而是等待后消费 exact base/head evidence；base retarget 进入新 base lane、禁用复用并与 source event 一样取消该 lane 的过期 validation，再按精确新 base/head 重新分类。证据 miss、cache miss、Autofix 判定失败或输出不完整都必须 fail closed 到当前 validation scope。
