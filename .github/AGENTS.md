...
- `quality.yml`：所有 `main` push / 源码 PR / 手动触发都保持 `lint`、`format-check`、`typecheck`、`commit-message` 的稳定 check 身份。PR 统一调用 validation-scope v2，按 client / node / shared、env contract 与 docs-media 目标选择检查，未知路径 fail closed 到全量；client production closure 在现有 typecheck runner 内复用同一次依赖安装执行 Vite build，format 与 env contract 使用无 workspace install 的轻量入口。公开 README / `.oo/docs` 追加 docs build / media，changelog / release rule docs 追加 release docs preflight。新源码提交只取消同 PR 的旧源码 run。
- `pr-change-policy.yml`：保持 `pr-change-policy` required check 身份，使用无 workspace install 的 CJS 入口；独立监听 PR 创建、同步和正文编辑。正文 `edited` 只取消同 PR 的旧 policy run，不会取消或生成 skipped 的 Quality 源码门禁。
- GitHub 把 PR base 变更也归入 `edited`；当前从其他 base retarget 到 `main` 后不会复用旧 base 的源码门禁，required contexts 保持缺失并阻止合入，直到新 head commit 或 close / reopen 触发 Quality。不要为了自动处理 base edit 把普通正文编辑重新接回 Quality；后续应使用不生成 skipped required contexts 的独立 base-change dispatcher。
...
- `desktop-package.yml`：PR 先在 Ubuntu 复用 validation-scope v2；普通 client、adapter、品牌资产和文档改动只完成 `macOS installer` required gate，不申请 macOS runner。桌面源码、native authority、打包工具、根 manifest / lockfile、正式包内 runtime closure 或未知路径才构建 arm64+x64 unsigned app bundle 并跑 authority smoke。nightly 用 unsigned arm64 DMG 跑完整 package / smoke / install verify，保留 3 天用于提前暴露发布回归。`pkg/oneworks-desktop/v*` tag 或手动 dispatch 统一构建 arm64+x64 的 DMG / PKG / ZIP；签名构建先无等待提交公证并保留精确字节、摘要、attempt / submission ID 和 build metadata，再做有界轮询，Apple 延迟时用 `notarization_run_id` / `notarization_stage` 恢复并固化更新状态，不得重签或重复提交。`notarization_history_only` 只读查询团队历史。带 `release_tag`、不勾 `create_release` 可生成正式身份的候选产物，之后用 `candidate_run_id` 提升同一 artifact，无需重打包。GitHub Release 成功后复用 `deploy-homepage.yml` 自动等待官网刷新。
