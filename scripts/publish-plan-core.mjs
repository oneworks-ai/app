...
export function getPublishAliases(packageJson) {
  const rawAliases = packageJson?.oneworks?.publishAliases
  if (!Array.isArray(rawAliases)) {
    return []
  }

  return Array.from(
    new Set(rawAliases.map(alias => String(alias).trim()).filter(Boolean))
  ).filter(alias => alias !== packageJson.name)
}

export function createPublishAliasManifest(sourceJson, aliasName) {
  const aliasJson = JSON.parse(JSON.stringify(sourceJson))
  ...
  if (
    aliasJson.oneworks != null &&
    typeof aliasJson.oneworks === 'object' &&
    !Array.isArray(aliasJson.oneworks)
  ) {
    delete aliasJson.oneworks.publishAliases
  }
...
选项:
  -p, --packages <names>    逗号分隔的包名列表
      ...
      --skip-existing       发布前跳过 registry 已存在的同名同版本（默认开启）
      --no-skip-existing    不检查已发布版本，直接执行 publish
...
export async function loadWorkspacePackages(repoRoot, fsOps = defaultFs) {
  ...
  for (const pkg of Array.from(packages.values())) {
    for (const aliasName of getPublishAliases(pkg.json)) {
      if (packages.has(aliasName)) {
        throw new Error(`发布别名 ${aliasName} 与已有包重名`)
      }

      packages.set(aliasName, {
        ...pkg,
        name: aliasName,
        json: createPublishAliasManifest(pkg.json, aliasName),
        publishAliasFor: pkg.name
      })
    }
  }
...
function collectRequestedPackages(packages, requestedNames) {
  ...
  for (const name of requestedNames) {
    ...
    for (const [candidateName, candidatePkg] of packages) {
      if (candidateName === sourceName || candidatePkg.publishAliasFor === sourceName) {
...
export function createPublishPlan(packages, options) {
  ...
  for (const name of nodes) {
    const pkg = packages.get(name)
    if (!pkg?.publishAliasFor || !adjacency.has(pkg.publishAliasFor)) {
      continue
    }

    if (!adjacency.get(pkg.publishAliasFor).has(name)) {
      adjacency.get(pkg.publishAliasFor).add(name)
      indegree.set(name, (indegree.get(name) ?? 0) + 1)
    }
  }
  ...
  const items = order.map((name) => {
    ...
    return {
      ...
      publishAliasFor: pkg?.publishAliasFor ?? null,
  ...
  plan.items.forEach((item, index) => {
    const versionPart = item.version
      ? item.nextVersion
        ...
        : item.version
      : '未声明版本'
    const aliasPart = item.publishAliasFor
      ? ` | alias of: ${item.publishAliasFor}`
      : ''
    ...
    const impactsPart = item.impactedDependents.length > 0
      ...
      : ''
    lines.push(
      `${index + 1}. ${item.name}@${versionPart} (${
        path.relative(repoRoot, item.dir)
      })${aliasPart}${depsPart}${impactsPart}`
    )
  })
...
export function serializePlan(plan, repoRoot, options) {
  return {
    ...
    order: plan.items.map((item, index) => ({
      ...
      publishAliasFor: item.publishAliasFor,
...
export function getPublishTargetVersion(item) {
  return item.nextVersion || item.version || ''
}

export function isPackageVersionPublished(item, runCommand = spawnSync) {
  const version = getPublishTargetVersion(item)
  ...
  const result = runCommand(
    'npm',
    ['view', `${item.name}@${version}`, 'version', '--json'],
    {
      encoding: 'utf8',
      stdio: 'pipe'
    }
...
export function stagePublishAliasManifest(item) {
  if (!item.publishAliasFor) {
    return null
  }

  const packageJsonPath = path.join(item.dir, 'package.json')
  const originalContent = readFileSync(packageJsonPath, 'utf8')
  const sourceJson = JSON.parse(originalContent)
  const aliasJson = createPublishAliasManifest(sourceJson, item.name)

  writeFileSync(packageJsonPath, `${JSON.stringify(aliasJson, null, 2)}\n`)
...
export async function executePublishPlan(
  ...
  for (const item of plan.items) {
    ...
    if (options.skipExisting && !options.dryRun && isPackageVersionPublished(item, runCommand)) {
      const version = getPublishTargetVersion(item)
      stdout.write(`跳过已发布版本: ${item.name}@${version}\n`)
      attempts.push({
        name: item.name,
        status: 0,
        attempts: 0,
    ...
    const restoreAliasManifest = stagePublishAliasManifest(item)
...
export async function runPublishPlanCli(
  ...
  if (options.publish) {
    ...
    if (publishResult.failures.length > 0) {
      ...
      throw new Error(`${publishResult.failures.length} 个包发布失败`)
    }
  }
