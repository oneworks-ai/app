...
import { projectHome, repoRoot } from './runtime'
...
const sanitizeValue = (value: string) => (
  value
    ...
    .replaceAll(projectHome, '<project-home>')
...
const createSnapshotPath = (value: string) => {
  const resolvedValue = path.resolve(value)
  if (isPathInside(repoRoot, resolvedValue)) {
    return sanitizeValue(toPosixPath(path.relative(repoRoot, resolvedValue)))
  }
  if (isPathInside(projectHome, resolvedValue)) {
    return sanitizeValue(`<project-home>/${toPosixPath(path.relative(projectHome, resolvedValue))}`)
  }
  return sanitizeValue(toPosixPath(resolvedValue))
}
