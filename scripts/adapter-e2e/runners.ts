...
export const runOpenCodeUpstream = async (input: {
  ...
  const configDir = path.resolve(mockHome, '.opencode-adapter', input.sessionId, 'config-dir')
  ...
  const result = await runProcess({
    ...
    env: {
      ...
      __ONEWORKS_PROJECT_NODE_PATH__: process.execPath,
      __ONEWORKS_PROJECT_REAL_HOME__: realHome,
      __ONEWORKS_PROJECT_CLI_PACKAGE_DIR__: cliPackageDir,
      __ONEWORKS_PROJECT_PACKAGE_DIR__: cliPackageDir,
      __ONEWORKS_OPENCODE_TASK_SESSION_ID__: input.sessionId,
