...
const DEFAULT_FORMULA_PATH = 'Formula/oneworks.rb'

export const updateOneWorksFormula = (
  content: string,
  input: {
    sha256: string
    tarballUrl: string
  ...
  const urlPattern = /url "https:\/\/registry\.npmjs\.org\/(?:@oneworks\/[^"]+|oneworks)\/-\/[^"]+\.tgz"/
...
const runHomebrewTapSyncPackage = async (input: {
  ...
  const formulaPath = input.formulaPath ?? input.defaultFormulaPath
...
export const runHomebrewTapSyncOneWorks = async (input: {
  version: string
  tapDir?: string
  formulaPath?: string
  dryRun?: boolean
  ...
  await runHomebrewTapSyncPackage({
    ...input,
    defaultFormulaPath: DEFAULT_FORMULA_PATH,
    packageName: ONEWORKS_PACKAGE_NAME
  })
