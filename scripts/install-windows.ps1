...
if (!$SkipVerify) {
  if (!(Test-Command oneworks)) {
    throw "oneworks was installed but is not on PATH yet. Open a new PowerShell window and run 'oneworks --version'."
  }

  Write-Step "Verifying oneworks"
  oneworks --version
}

Write-Step "Done. Try: oneworks --help"
