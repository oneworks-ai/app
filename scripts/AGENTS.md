...
仓库长期开发服务也走同一个 CLI：当用户要求拉取最新代码并启动 web / daemon / Electron / PWA / homepage / docs / Relay / desktop-control / Android emulator 时，直接在仓库根目录运行 `pnpm --silent tools dev-service ensure <target> --json`。`scripts/run-tools.mjs` 会先精确初始化 workspace 必需的 `assets/avatar` 与 `assets/demo-video` submodule，再注册 TS；缺少 register 依赖时执行 `pnpm install`。dev-start 在 fetch / pull 后还会把 `assets/avatar` 对齐到当前 gitlink，再校验 workspace 安装，并在操作租约内复用或启动对应服务。
...
- `pnpm --silent tools dev-service stop web --forget-stale --json`：仅用于普通 `stop` 因 PID 身份变化而拒绝、且该 target 已不健康的恢复场景；所有健康端点必须不可达，所有仍运行 PID 的当前身份必须与记录身份不同。该命令只遗忘陈旧状态，不向这些 PID 发送信号；机器级 target 禁用。
...
- `pnpm tools desktop-control record-batch launcher-open-workspace-ui-tour --workspace <path> --app <app>`（可复用录制器、scenario、fixture、后期逻辑和完整硬门禁由 `assets/demo-video` submodule 拥有；这里仅维护 One Works Electron / Desktop Control 薄集成，视频设计与投放记录见 `assets/demo-video/docs/creative/` 和 `assets/demo-video/catalog/videos.json`）
  ...
  - 合成光标必须区分移动、按下、释放：移动到目标后先完整播放按下与释放回弹，留一个很短的 visual-to-input buffer，再触发真实 `mousePressed` / `mouseReleased` 输入事件。很多控件会在 pointer down 阶段切页面，不能只延后 release；不要让页面行为先变化、点击动画后补，也不要把完整反馈全部放在业务行为之后。场景要求“放大到鼠标区域”时使用 scenario 的 selector 驱动镜头事件，对已包含真实系统窗口和合成光标的整幅视频统一缩放 / 裁切；只放大光标不算完成。镜头在输入前开始、以平滑缓动抵达目标，保持随后浮层完整可见；不要重绘或模拟产品 UI
  ...
  - 交付 Electron / workspace 录屏时必须同步加载耗时分析：至少说明录制器启动、app spawn 到首窗、workspace server ready、renderer/chat ready、runtime package cache 命中状态，以及 stills / 抽帧是否证明已进入对话界面。公开 Launcher 素材传 `--demo-fixture adapter-promo` 使用命名式安全数据；fixture 在 Electron selector-state 数据源固定展示虚拟 Home / 项目名称和路径，只在打开动作时映射到真实 `--workspace`，关闭全局与本地私有配置层，并在每个 CDP page target 安装幂等演示数据投影，把模板标签、账号、邮箱及明确的 Workspace 顶栏项目标签替换成固定中性值。不要用全页 workspace basename 子串替换伪装数据。英文 / 中文语言仍须在 BrowserWindow preload 前固定。Launcher 保持可见直到隐藏的真实 Workspace 已进入可交互界面，再由同一组 bounds 平滑扩展；任意帧只允许一套窗口壳。不要用后期蒙版 / 模拟窗口替代 fixture，也不要在 scenario 或 React 组件里硬编码个人路径。
...
- `pnpm tools relay-auth-fixture <single-user|single-server-multi-user|multi-server-multi-user|restore|path> [--json]`
