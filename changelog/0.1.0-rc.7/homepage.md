...
- Replace the homepage's One Works marks and embedded PWA preview default icon with the shared Linear-style renderer, while preserving the product's global icon default.
- Skip the embedded PWA preview, scroll narrative, and duplicate decorative brand canvases on mobile viewports; keep one centered hero mark with a lightweight connected ambient network derived from the complete adapter list instead.
