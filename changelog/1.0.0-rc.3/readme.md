...
- Add member-scoped external channel connections and definition-driven Leaders for Team Chats, including single-select Leaders with automatically selected related members, multi-room mappings, per-connection processing policies, deduplicated inbound routing, delivery-state degradation, and related-channel/member attribution in the UI.
