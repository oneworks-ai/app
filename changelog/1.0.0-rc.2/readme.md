...
- Show a meaningful Desktop Workspace loading surface and the real Launcher before backend readiness, while using a packaged server startup bundle, lazy-loading API, WebSocket protocol, and optional debug/runtime modules, and deferring cache and hidden Launcher work beyond the real workspace sender; readiness is reported only after that sender is usable and the loading overlay is gone.
