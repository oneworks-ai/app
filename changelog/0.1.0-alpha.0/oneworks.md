...
- Fix worktree-scoped Web and Electron session startup so linked worktrees keep their own workspace path, Electron leaves the startup overlay without a manual refresh, Codex runtime config is normalized before session startup, and sender automation updates the underlying editor state.
