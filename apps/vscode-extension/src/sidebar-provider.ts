...
export class OneWorksSidebarProvider implements vscode.WebviewViewProvider {
  ...
  resolveWebviewView(webviewView: vscode.WebviewView) {
    ...
    if (workspaceFolder == null) {
      this.renderStatus('Open a workspace', 'One Works needs a local workspace folder to start a project server.')
      return
    }
  ...
  async restart() {
    ...
    void vscode.window.showInformationMessage(`One Works restarted for ${workspaceFolder.name}.`)
  }
  ...
  async stop() {
    ...
    if (this.activeWorkspaceFolder?.uri.fsPath === workspaceFolder.uri.fsPath) {
      ...
      this.renderStatus('One Works stopped', workspaceFolder.uri.fsPath)
    }
    void vscode.window.showInformationMessage(`One Works stopped for ${workspaceFolder.name}.`)
  }

  async stopAll() {
    await this.manager.stopAll()
    this.activeServer = undefined
    this.renderStatus('One Works stopped', 'All project servers have been stopped.')
    void vscode.window.showInformationMessage('One Works servers stopped.')
  }
