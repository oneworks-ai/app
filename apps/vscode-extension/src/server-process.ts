  ...
  new Promise<void>((resolve, reject) => {
    const retry = () => {
      if (Date.now() - startedAt > SERVER_READY_TIMEOUT_MS) {
        reject(new Error('Timed out while waiting for the One Works server.'))
        return
      }
      setTimeout(() => {
...
export const assertServerUiReady = async (port: number) => {
  ...
  if (statusCode < 200 || statusCode >= 400) {
    throw new Error(
      [
        `One Works UI was not served from ${SERVER_UI_READY_PATH} (status ${statusCode}).`,
        '',
        'Install or update the integrated web runtime:',
        'pnpm add -D oneworks',
        '',
        'Or configure `oneworks.bootstrapCommand` with a working bootstrap launcher.'
      ].join('\n')
    )
  }
}
...
export const waitForServerStartup = (child: ChildProcess, port: number) =>
  new Promise<void>((resolve, reject) => {
    ...
    const onExit = (code: number | null, signal: NodeJS.Signals | null) => {
      settleReject(
        new Error(`One Works server exited before it was ready (code=${code ?? 'null'} signal=${signal ?? 'null'}).`)
      )
    }
