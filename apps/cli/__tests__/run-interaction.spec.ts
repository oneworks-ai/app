...
} from '#~/commands/run/input-control.js'
...
describe('run command interaction handling', () => {
  ...
  it('treats runtime protocol consumers as an interaction input channel', () => {
    expect(supportsRuntimeInteractionResponses(undefined, true)).toBe(true)
    expect(supportsRuntimeInteractionResponses('text', true)).toBe(true)
    expect(supportsRuntimeInteractionResponses(undefined, false)).toBe(false)
    expect(supportsRuntimeInteractionResponses('stream-json', false)).toBe(true)
  })
