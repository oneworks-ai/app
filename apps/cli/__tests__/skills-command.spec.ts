...
describe('skills command', () => {
  ...
  beforeEach(async () => {
    resetConfigCache()
    testHome = await realpath(await mkdtemp(path.join(os.tmpdir(), 'ow-skills-command-home-')))
    tempDirs.push(testHome)
    process.env.__ONEWORKS_PROJECT_REAL_HOME__ = testHome
    vi.clearAllMocks()
  ...
  it('adds a configured skill and installs it immediately', async () => {
    ...
    mocks.installProjectSkill.mockResolvedValue({
      ...
      installDir: path.join(cwd, '.oo/skills/internal-review'),
      name: 'internal-review',
      ref: 'example-source/default/public@design-review',
      skillPath: path.join(cwd, '.oo/skills/internal-review/SKILL.md')
    })
    ...
    const config = JSON.parse(await readFile(path.join(cwd, '.oo.config.json'), 'utf8'))
  ...
  it('adds a configured skill without copying existing global skills into project config', async () => {
    const cwd = await realpath(await mkdtemp(path.join(os.tmpdir(), 'ow-skills-command-')))
    tempDirs.push(cwd)
    process.chdir(cwd)
    ...
    const projectConfig = JSON.parse(await readFile(path.join(cwd, '.oo.config.json'), 'utf8'))
  ...
  it('adds a configured skill to global config', async () => {
    const cwd = await realpath(await mkdtemp(path.join(os.tmpdir(), 'ow-skills-command-')))
    tempDirs.push(cwd)
    process.chdir(cwd)
    await mkdir(path.join(testHome, '.oo'), { recursive: true })
    ...
    await expect(readFile(path.join(cwd, '.oo.config.json'), 'utf8')).rejects.toThrow()
  })
