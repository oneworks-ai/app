...
The Admin OpenAPI document also includes platform user group, platform team, team member group, team policy, message, configuration profile, and configuration secret management endpoints under `/api/admin/access-groups*`, `/api/admin/teams*`, `/api/admin/teams/<teamId>/access-groups*`, `/api/admin/team-policy`, `/api/admin/messages`, `/api/admin/config-profiles*`, `/api/admin/config-assignments*`, and `/api/admin/config-secrets*`. Access group schemas expose default `name` / `description` fields plus `localizedNames` / `localizedDescriptions` maps keyed by supported locale tags for platform and team-scoped groups. Built-in system groups automatically include default localized copy for every supported language.
...
The personal user OpenAPI document also includes current-user team, team member group, and managed configuration endpoints under `/api/relay/teams*`, `/api/relay/teams/<teamId>/access-groups*`, `/api/relay/team-policy`, `/api/relay/config/global`, `/api/relay/config-snapshot`, `/api/relay/config-profiles*`, `/api/relay/config-assignments*`, and `/api/relay/config-secrets*`. Project-rule assignments own their document snapshots at `/api/relay/config-assignments/<assignmentId>/documents`; the matching Admin route is `/api/admin/config-assignments/<assignmentId>/documents`. These endpoints store and relay client-produced encrypted-format document payloads together with synchronization, scope, and audit metadata, but never parse or index document content. A Relay deployment operator can reproduce the current namespace-derived keys and decrypt the content, so this is not a zero-knowledge protocol against the operator. Account, team, and project-rule document namespaces remain independent.
...
- Build command: `pnpm build:vercel`; it builds the shared type and icon exports, materializes both runtime workspace packages during hosted CI, builds `apps/relay-admin`, copies the Admin static assets into `apps/relay-server/public/admin`, then builds the Relay Server function.
- Local prebuilt CLI deploys should run `vercel build --prod`, then `pnpm prepare:vercel-output`, then `vercel deploy --prebuilt --prod`. The prepare step copies the Postgres, WebAuthn, icon, shared types, and transitive runtime dependency trees into the Vercel function output for pnpm workspace deployments.
