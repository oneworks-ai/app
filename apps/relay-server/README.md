...
The Admin OpenAPI document also includes platform team, team policy, message, configuration profile, and configuration secret management endpoints under `/api/admin/teams*`, `/api/admin/team-policy`, `/api/admin/messages`, `/api/admin/config-profiles*`, `/api/admin/config-assignments*`, and `/api/admin/config-secrets*`.

The Admin API docs page at `/admin/openapi` reads both OpenAPI documents, keeps personal user APIs and platform admin APIs in separate tabs, and offers raw JSON links for API clients. The Admin profile page at `/admin/profile` lets a logged-in user create system access tokens for API clients. System access tokens are shown only once, stored server-side as hashes, and authorize requests according to the owning user's role and permissions. Use them as Bearer tokens when calling the OpenAPI endpoints. A system access token cannot create or revoke other system access tokens; use a normal Relay login session for those profile security actions.
...
The personal user OpenAPI document also includes current-user team and managed configuration endpoints under `/api/relay/teams*`, `/api/relay/team-policy`, `/api/relay/config-snapshot`, `/api/relay/config-profiles*`, `/api/relay/config-assignments*`, and `/api/relay/config-secrets*`.
