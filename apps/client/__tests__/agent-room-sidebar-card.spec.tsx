...
describe('agent room sidebar card', () => {
  it('renders the room title without the label or statistics metadata', async () => {
    ...
    expect(html).toContain('room-avatar__model')
    expect(html).toContain('interactive-avatar__canvas')
  ...
  it('keeps the room leading icon centered when the card has a latest message', () => {
    ...
    expect(styles).toContain('.room-avatar')
    ...
    expect(styles).toContain('&.room-avatar')
    expect(styles).toContain('--room-avatar-radius: var(--room-item-avatar-radius);')
