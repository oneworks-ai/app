...
describe('shared sender surface toolbar contract', () => {
  it('lets themes own toolbar spacing, action padding, and structural dividers', () => {
    ...
    expect(styles).toMatch(
      /\.chat-status-bar__actions\s*>\s*:is\(\.chat-status-bar__account-group,\s*\.adapter-select-tooltip-target\):not\(\s*:first-child\s*\)/
    )
    expect(styles).toMatch(
      /\.chat-status-bar__account-group\s*>\s*:is\(\.sender-select-shell--account,\s*\.account-quota-indicators\):not\(\s*:first-child\s*\)/
    )
    expect(styles).not.toMatch(
      /\.chat-status-bar__(?:actions|account-group)\s*>\s*:not\(:first-child\)/
    )
    expect(styles).toMatch(
      /\.chat-status-bar-frame:has\(\.sender-select-shell--account\.is-open\)\s*\{[^}]*z-index:\s*4/
    )
