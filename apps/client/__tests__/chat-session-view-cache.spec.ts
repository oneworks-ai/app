...
describe('chat session view cache', () => {
  ...
  it('preserves compact state across hydrated restores', () => {
    const restored = restoreChatSessionViewSnapshot(createChatSessionViewSnapshot({
    ...
    expect(restored.sessionCompactionInfo?.id).toBe('compact-1')
    expect(restored.sessionCompactionInfo?.trigger).toBe('auto')
    expect(restored.sessionCompactionEvents).toHaveLength(1)
  })
