...
export const setupMobileViewport = () => {
  ...
  const updateViewportVars = () => {
    ...
    document.documentElement.style.setProperty('--oneworks-visual-viewport-height', `${Math.round(viewportHeight)}px`)
    document.documentElement.style.setProperty('--oneworks-keyboard-inset-bottom', `${Math.round(keyboardInset)}px`)
  ...
  const scheduleFocusedInputScroll = () => {
    ...
    scrollFrame = window.requestAnimationFrame(() => {
      const keyboardInset = updateViewportVars()
      if (keyboardInset > KEYBOARD_VISIBLE_THRESHOLD) {
