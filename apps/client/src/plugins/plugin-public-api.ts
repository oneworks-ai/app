...
export const parsePublicPluginRuntimeInstance = (
  ...
  const requestId = parseOptionalPublicText(value, 'requestId', state)
  const scope = parseOptionalPublicText(value, 'scope', state)
  if (requestId == null || scope == null) return undefined

  const result: PublicPluginRuntimeInstance = { requestId, scope }
  for (const key of ['description', 'displayName', 'name', 'packageId', 'requestedVersion', 'version'] as const) {
  ...
  const sourceGroup = parseOptionalPublicText(value, 'sourceGroup', state)
  ...
  const entryKind = parseOptionalPublicText(value, 'devClientEntryKind', state)
