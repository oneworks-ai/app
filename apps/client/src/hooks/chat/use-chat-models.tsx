...
export function useChatModels({
  ...
  const [selectedModel, setSelectedModel] = useState<string | undefined>(() => {
    try {
      const raw = localStorage.getItem('oneworks_chat_selected_model')
  ...
  useEffect(() => {
    try {
      if (selectedModel == null || selectedModel.trim() === '') {
        localStorage.removeItem('oneworks_chat_selected_model')
      } else {
        localStorage.setItem('oneworks_chat_selected_model', selectedModel)
      }
    } catch {
    }
  }, [selectedModel])
