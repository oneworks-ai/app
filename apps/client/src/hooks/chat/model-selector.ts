...
export const resolveChatAdapterSelection = (params: {
  ...
  if (adapterSet.has(DEFAULT_NATIVE_ADAPTER)) return DEFAULT_NATIVE_ADAPTER
  return adapterKeys[0]
}

export const hasRunnableChatModelSelection = (params: {
  availableAdapters: Iterable<string>
  builtinModels?: Iterable<string>
  serviceModels: ServiceModelEntry[]
}) => (
  params.serviceModels.length > 0 ||
  Array.from(params.builtinModels ?? []).length > 0 ||
  Array.from(params.availableAdapters).length > 0
)
