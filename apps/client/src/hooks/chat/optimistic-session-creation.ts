/* eslint-disable max-lines -- optimistic session helpers share creation, resolution, and list merge contracts. */
...
export const OPTIMISTIC_USER_MESSAGE_ID_SUFFIX = ':optimistic-user-message'
...
export const createOptimisticSessionCreation = (
  ...
  return {
    message: {
      id: `${request.id}${OPTIMISTIC_USER_MESSAGE_ID_SUFFIX}`,
      role: 'user',
      content,
      createdAt: now
    },
    ...
    session: {
      ...
      isArchived: false,
      isStarred: false,
      parentSessionId: request.options.parentSessionId,
...
export const getActiveOptimisticSessionCreation = (
  session: Session | undefined,
  creations: OptimisticSessionCreationMap
) => {
  if (session?.id == null || session.id === '') return undefined
  const creation = creations[session.id]
...
export const mergeOptimisticSessions = (
  ...
  const mergedSessions = sessions.map(session => {
    const creation = creations[session.id]
    return isOptimisticSessionResolvedBySession(creation, session) ? session : creation?.session ?? session
  })
...
export const removeSessionFromList = (sessions: Session[], id: string) => sessions.filter(session => session.id !== id)
