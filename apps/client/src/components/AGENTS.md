...
- `action-search-toolbar/`：配置页、管理页和列表页共用的搜索 + 紧凑 action toolbar。新增历史、下载、导入记录这类带搜索和筛选 action 的页面时优先复用它，不要在业务组件里各自拼 AntD `Input` / 图标按钮样式。Route View 已由外层 `contentInset` 提供上、左、右留白时传 `inset={false}`，不要再叠一层 toolbar padding；独立内容区保持默认 inset。
...
- `usage/`：Launcher 全局入口与 workspace 配置子页复用的 Token 用量视图；默认按 surface 决定全局或当前 workspace scope，只有存在多个候选项或单一归因值混有未归因数据时才展示收纳后的筛选入口。跨 runtime 数据和自适应筛选的完整约定见 `../../../../.oo/rules/maintenance/token-usage-analytics.md`。
...
- 折叠窗口栏当前就在 `/` 新会话时，普通 Web、Electron 和 `__oneworks_desktop` 模拟态都不为 `nav-rail-create-session-indicator` 预留 action 宽度；该指示器作为不可交互的装饰层，锚在折叠窗口栏结束 / Route Header 开始的共享边界，与标题自带的会话图标完全重合。两层必须共同消费 `--nav-rail-window-icon-size` / `--app-chrome-icon-size` 和整条 chrome 的垂直中心，不用局部像素偏移修正；Electron / 模拟态仍保留历史导航和对应宽度计算。
