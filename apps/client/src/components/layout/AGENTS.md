...
- Native or floating Dockview removal happens before `onDidRemovePanel`. If the route vetoes that close, the shared workspace must restore the last settled serialized layout and resynchronize controlled panels; returning `false` alone cannot recover a panel Dockview already removed.
