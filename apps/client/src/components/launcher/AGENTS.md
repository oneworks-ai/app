...
- Launcher 内的 destructive confirmation 属于 Launcher modal owner；AntD imperative modal 必须挂到 `LauncherRoute` 提供的 owner container，让 overlay focus trap 能把最内层可见 dialog 作为临时边界，不能默认 portal 到 `document.body`。
...
- 默认主题里的 command 行和设置 section tab 在 hover、focus、active 状态都不铺背景：command 使用左侧 indicator + 强调文字表达选中，hover / focus 使用前景色与焦点轮廓；section tab 使用强调文字 + 下划线表达选中。显式主题包可以通过 `--oneworks-launcher-item-hover-bg` 和 `--oneworks-launcher-item-active-bg` 覆盖自己的交互语言。
