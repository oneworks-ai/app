/* eslint-disable max-lines -- session context menu keeps grouped actions and rename flow together. */
...
export function SessionContextMenu({
  ...
  const canOpenDesktopSessionWindow = window.oneworksDesktop != null &&
    (window.oneworksDesktop.platform === 'darwin' || navigator.platform.toLowerCase().includes('mac'))
    ...
    return [
        ...
        ? [
          {
            ...
            onClick: () => {
              closeMenu()
              const sessionUrl = buildSessionUrl(session.id, { sidebarCollapsed: true })
          ...
          { key: 'divider-open-window', type: 'divider' as const, label: '', icon: '', onClick: () => undefined }
        ]
        : []),
      {
        key: 'star',
      ...
      },
      {
        ...
        onClick: () => {
          closeMenu()
          void copyTextWithFeedback({
            text: `oneworks --resume ${session.id}`,
            messageApi: message,
  ...
  }, [canOpenDesktopSessionWindow, message, modal, onArchive, onDelete, onRename, onStar, pendingAction, session, t])
