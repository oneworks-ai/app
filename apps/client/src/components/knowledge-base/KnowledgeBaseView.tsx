...
export function KnowledgeBaseView({
  ...
  const handleSaveAsset = React.useCallback(async () => {
    ...
    const execution = await executeAssetCreate({
      ...
      onCreateFailure: error => {
        void message.error(
          error instanceof ApiError && error.code === 'asset_name_exists'
            ? t('knowledge.assets.nameExists')
            : getApiErrorMessage(error, t('knowledge.assets.createFailed'))
        )
      },
      onIndeterminate: () => {
  ...
  const getContentControls = (onCreate: () => void) => (
    <KnowledgeContentControls onCreate={onCreate} />
  )
