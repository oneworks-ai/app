...
export const SectionForm = ({
  ...
  const renderFieldGroups = ({
    ...
    const getRelatedFieldGroupKey = (field: FieldSpec) => (
      field.path.length > 1 ? field.path[0] : undefined
    )
    const renderFields = (fieldsToRender: FieldSpec[]) => (
    ...
    const renderRelatedFieldRuns = (fieldsToRender: FieldSpec[]) => {
      ...
      while (index < fieldsToRender.length) {
        ...
        const relatedGroupKey = getRelatedFieldGroupKey(field)
        if (relatedGroupKey == null) {
          nodes.push(...renderFields([field]))
          index += 1
          continue
        }
        ...
        while (index < fieldsToRender.length) {
          const nextField = fieldsToRender[index]
          if (nextField == null || getRelatedFieldGroupKey(nextField) !== relatedGroupKey) break
          runFields.push(nextField)
          index += 1
        }
        ...
        if (runFields.length > 1) {
          nodes.push(
            <div
              key={`${keyPrefix}:related:${relatedGroupKey}:${runFields[0]?.path.join('.')}`}
              className='config-view__field-list config-view__field-list--grouped'
            >
              {renderFields(runFields)}
            </div>
          )
        } else {
          nodes.push(...renderFields(runFields))
        }
      }
    ...
    const shouldRenderGroupHeaders = orderedGroups.length > 1
        ...
        {orderedGroups.map((groupKey) => {
          ...
          if (groupKey === 'default') {
            ...
            const listClassName = [
              ...
              groupedDefaultSections.has(sectionKey) ? 'config-view__field-list--grouped' : ''
            ].filter(Boolean).join(' ')
            return (
              <div key={`${keyPrefix}:${groupKey}`} className={listClassName}>
                {groupedDefaultSections.has(sectionKey)
                  ? renderFields(groupFields)
                  : renderRelatedFieldRuns(groupFields)}
              </div>
            )
          }
          ...
          const collapseItems = Array.from(collapseGroups.values()).map(group => ({
            ...
            children: (
              <div className='config-view__field-list config-view__field-list--grouped'>
          ...
          const groupBody = (
            <div className='config-view__subsection-body'>
              {nonCollapseFields.length > 0 && (
                <div className='config-view__field-list'>
                  {renderRelatedFieldRuns(nonCollapseFields)}
                </div>
              )}
          ...
          if (groupMeta?.collapsible === true && shouldRenderGroupHeaders) {
            return (
              <Collapse
