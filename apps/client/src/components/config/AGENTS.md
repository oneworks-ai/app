...
- 所有已注册 adapter detail 共用“基础配置 / 模型配置 / 账号 / 进阶配置”的 route-backed tabs；账号能力关闭时不渲染空账号 tab。账号列表留在账号 tab，具体账号详情 (`accounts/:account`) 是独立末级页面，不重复渲染适配器 tabs。默认账号只通过账号列表行尾的星标选择，当前默认账号使用实心星，不再额外渲染重复的 `defaultAccount` 字段。adapter 卡片不是可删除的用户记录，卡片级清理动作必须表达为“重置当前范围内的全部配置”，并在确认后删除本地覆盖以恢复内置默认值。
- 宿主 `NativeTabs` / tab chrome 已经提供当前页签的标题和切换语义；tab panel 应直接从可操作内容开始，不要在容器里重复同名 heading、介绍或只复述既有契约的 notice。只有会改变用户决策、且无法由字段 description 表达的告警才单独占一行。
