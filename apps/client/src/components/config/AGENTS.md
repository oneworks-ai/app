...
- 所有已注册 adapter detail 共用“基础配置 / 模型配置 / 账号 / 进阶配置”的 route-backed tabs；账号能力关闭时不渲染空账号 tab，账号列表与账号详情都留在账号 tab 的嵌套路由内。默认账号只通过账号列表行尾的星标选择，当前默认账号使用实心星，不再额外渲染重复的 `defaultAccount` 字段。adapter 卡片不是可删除的用户记录，卡片级清理动作必须表达为“重置当前范围内的全部配置”，并在确认后删除本地覆盖以恢复内置默认值。
