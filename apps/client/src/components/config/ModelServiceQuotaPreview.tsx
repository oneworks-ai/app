...
export const ModelServiceQuotaPreview = ({
  item,
  serviceKey,
  source,
  t,
  ...
  source: ConfigSource
  t: TranslationFn
  variant?: 'cardFooter' | 'compact' | 'list'
}) => {
  ...
  const canAutoQueryAccountStatus = actionCapabilities.canQueryBalance
  ...
  useEffect(() => {
    ...
    void queryBalance()
  }, [canAutoQueryAccountStatus, queryBalance, serviceFingerprint])

  if (!canAutoQueryAccountStatus) return null
  ...
  const quotaPreview = (
    ...
    >
      {quotaRows.map(row => {
        ...
        if (variant === 'cardFooter' || variant === 'list') {
          return (
