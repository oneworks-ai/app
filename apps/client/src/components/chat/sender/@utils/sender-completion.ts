...
const stripCompletionTrigger = (value: string, trigger: SenderCompletionTrigger) =>
...
export const resolveSenderCompletionMatch = (
  ...
  const query = match[3] ?? ''
  const replaceStart = match.index + (match[1]?.length ?? 0)
  const normalizedQuery = query.toLowerCase()
  ...
  const items = source.values
    .map(item => stripCompletionTrigger(item, trigger))
    .filter(item => normalizedQuery === '' || item.toLowerCase().includes(normalizedQuery))
    .map((item) => {
      const completionText = `${trigger}${item}`
