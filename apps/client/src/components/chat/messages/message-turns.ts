...
const isStandaloneTurnItem = (item: ChatRenderItem) => item.type === 'agent-room-child-request'
const hasNonCollapsibleTurnItem = (items: ChatRenderItem[]) =>
  items.some(item => item.type === 'status-notice' || item.type === 'workspace-changes')
...
export const buildMessageTurns = (params: {
  ...
  return drafts.map((draft, index) => {
    ...
    const isExpandable = draft.startedByUser && hiddenCount > 0 && !hasNonCollapsibleTurnItem(draft.items)
    ...
    const isCollapsed = isExpandable &&
      !hasHashTarget &&
      !params.expandedTurnIds.has(draft.id) &&
      (shouldCollapseByDefault || (params.collapsedTurnIds?.has(draft.id) ?? false))
