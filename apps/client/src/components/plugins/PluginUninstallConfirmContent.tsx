...
export function PluginUninstallIntentConfirmContent(props: {
  identity: PluginMarketplaceUninstallIdentity
}) {
  const { t } = useTranslation()
  return (
    <div data-testid='plugin-uninstall-intent-confirm-content'>
      <p>{t('pluginStore.uninstall.disableDifference')}</p>
      <p>
        {t('pluginStore.uninstall.projectScope', {
          marketplace: projectPluginPresentationValue(props.identity.marketplace),
          plugin: projectPluginPresentationValue(props.identity.plugin)
        })}
      </p>
    </div>
  )
...
export function PluginUninstallConfirmContent(props: {
  ...
  return (
    <div data-testid='plugin-uninstall-confirm-content'>
      ...
      <p>
        {t('pluginStore.uninstall.projectScope', {
          marketplace: projectPluginPresentationValue(props.plan.identity.marketplace),
          plugin: projectPluginPresentationValue(props.plan.identity.plugin)
        })}
      </p>
