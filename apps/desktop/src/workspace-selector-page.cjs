  ...
  <body>
    <main>
      ...
      <div class="panel">
        ...
        <div class="actions">
          <button type="button" id="new-folder-button" class="primary-button">New Project...</button>
          <button type="button" id="browse-button" class="secondary-button">Open Folder...</button>
        </div>
      </div>
    </main>
    ...
    <script>
      (() => {
        ...
        const openWorkspace = async (workspaceFolder) => {
          try {
            setError('')
            await window.oneworksDesktop.openWorkspace(workspaceFolder)
          } catch (error) {
        ...
        browseButton.addEventListener('click', () => {
          void window.oneworksDesktop.chooseWorkspace()
        ...
        newFolderButton.addEventListener('click', () => {
          void window.oneworksDesktop.createWorkspace()
            .then(selectedWorkspace => {
              if (typeof selectedWorkspace === 'string' && selectedWorkspace.trim() !== '') {
                return openWorkspace(selectedWorkspace)
              }
        ...
        window.oneworksDesktop.onWorkspaceSelectorStateChange(renderState)
        void window.oneworksDesktop.getWorkspaceSelectorState()
          .then(renderState)
          .catch(error => {
            setError(error instanceof Error ? error.message : String(error))
          })
