...
- Long-lived macOS emulator lifecycle must use the shared dev-service protocol: start or reuse it with `pnpm --silent tools dev-service ensure android-emulator --json`, inspect it with `pnpm --silent tools dev-service status android-emulator --json`, and terminate it with `pnpm --silent tools dev-service stop android-emulator --json`. This keeps ownership, operation history, and cross-session handoff visible; do not kill a recorded emulator PID directly.
- `pnpm -C apps/android emulator:visible -- --avd OneWorksApi35Visible ...` is the internal launcher used by that service. Invoke it directly only for focused, interactive diagnosis of launcher behavior: the launcher exits after readiness while the emulator child remains detached, and the diagnostic is serialized through the same machine-level kernel guard. A directly launched emulator is recorded as `coordination: "uncoordinated"` with `ownerRoot: null`; direct reuse of a dev-service-owned emulator preserves its existing coordinated owner instead of overwriting that fact. Direct `--restart` is rejected because mutation requires the full dev-service operation lease and explicit authorization. Never treat a direct invocation as the owner of a shared long-lived development emulator. When dev-service invokes the launcher, it records `coordination: "dev-service"` and the owning worktree root. When reusing an emulator, the launcher identifies its serial by querying `adb -s <serial> emu avd name`; it must not guess from `adb devices` ordering.
