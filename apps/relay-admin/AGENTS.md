...
- Admin 运行界面的普通视觉 padding 必须统一走 `src/app/AdminApp.css` 里的 `--relay-admin-padding`，当前固定为 `10px`。不要在 feature CSS、共享 UI CSS 或 AntD 覆盖里手写 `8px`、`12px`、`14px` 等 padding；需要更大的视觉距离时优先使用 `gap` / `margin` / 分隔线布局，纯图标按钮或布局重置可以继续使用 `padding: 0`。AntD tabs 的垂直 padding 放在可点击的 `.ant-tabs-tab-btn` 上，外层 `.ant-tabs-tab` 保持 `padding: 0`，避免 DevTools 中内容高度和命中容器高度不一致。`route-container-layout__body` 顶部用于避让 overlay header 的 `padding-top: var(--route-container-header-overlay-height)` 是布局 inset，不要改成伪元素、margin 或 10px token。登录页 `src/login` 有独立登录面板变量，不混用这套后台工作台 padding token。
