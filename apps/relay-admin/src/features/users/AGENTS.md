...
- `UserDetailPage.tsx`：`/users/:userId/:tab` 的账号详情页，包含 `/users/:userId/diagnostics` 的隐私安全诊断时间线。
...
- 诊断页只显示服务端归一化后的诊断事实与不透明 `deviceId` 关联，不得借诊断功能暴露其他用户的设备名称、工作区、插件范围或原始日志内容。
