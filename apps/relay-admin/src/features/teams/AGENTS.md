...
`features/teams` 负责 Relay Admin 中的团队与团队配置方案管理界面。这里消费 `apps/relay-server` 的 `/api/admin/teams`、`/api/admin/team-policy`、`/api/admin/teams/:teamId/config-profiles`、`/api/admin/teams/:teamId/config-secrets`、`/api/admin/teams/:teamId/documents`、`/api/admin/config-profiles/*`、`/api/admin/config-assignments/*` 和 `/api/admin/config-secrets/*`。
...
- `TeamDetailPage.tsx`：`/teams/:teamId/members|profiles|secrets|documents|audit` 二级详情子页，负责成员、配置方案、密钥、同步文档和操作审计 tabs；旧 `/teams/:teamId` 只做跳转。
