...
- Relay 插件的账号 popover 和 profile tabs 镜像本页面与 `src/app/AdminNavRail.tsx`。修改账号菜单、tab key、tab label、当前用户 profile API 或安全操作语义时，必须检查 `packages/plugins/relay/src/client/index.ts`、`packages/plugins/relay/src/client/render.ts` 和 `packages/plugins/relay/src/server/controller.ts` 是否需要同步。
- Admin 与插件的差异点：Admin 前端直接用登录 session token 调 `/api/profile/*`；Relay 插件前端不拿 token，由插件 server 从 `~/.oneworks/auth.json` 读取登录态后代调远端，并隐藏服务端类型 / 地址。同步 UI 时不要把 Admin 的 token 持有方式复制到插件客户端。
