...
export const resolveChannelServerBaseUrl = (config?: ChannelBaseConfig) => {
  ...
  const publicBase = env.__ONEWORKS_PROJECT_PUBLIC_BASE_URL__?.trim()
  const origin = publicBase || `http://${env.__ONEWORKS_PROJECT_SERVER_HOST__}:${env.__ONEWORKS_PROJECT_SERVER_PORT__}`
