...
interface ChannelAutoDeliveryState {
...
const autoDeliveryStates = new Map<string, ChannelAutoDeliveryState>()
const seenMessageIds = new Map<string, number>()
const maxSeenMessageIds = 5000
const seenMessageTtlMs = 7 * 24 * 60 * 60 * 1000
const seenMessageCleanupIntervalMs = 60 * 60 * 1000
let lastSeenMessageCleanupAt = 0
...
export const deleteBinding = (sessionId: string) => {
  ...
  autoDeliveryStates.delete(sessionId)
}
...
export const resolveAutoDeliveryState = (sessionId: string) => {
  let state = autoDeliveryStates.get(sessionId)
  if (state == null) {
    state = {}
    autoDeliveryStates.set(sessionId, state)
  }
  return state
}

export const clearAutoDeliveryState = (sessionId: string) => {
  autoDeliveryStates.delete(sessionId)
}
...
export const isDuplicateMessage = (key: string) => {
  ...
  const now = Date.now()
  const isNewMessage = getDb().rememberChannelMessage(key, now)
  seenMessageIds.set(key, now)
  if (seenMessageIds.size > maxSeenMessageIds) {
    seenMessageIds.clear()
  }
  if (now - lastSeenMessageCleanupAt > seenMessageCleanupIntervalMs) {
    lastSeenMessageCleanupAt = now
