import type { EffortLevel, Session, SessionPermissionMode, SessionWorkspace } from '@oneworks/core'
...
defineMessages('zh', {
  ...
  'cmd.session.stop.description': '停止当前运行中的会话',
...
defineMessages('en', {
  ...
  'cmd.session.stop.description': 'Stop the current running session',
  ...
  command<ChannelContext>('session')
    ...
    .subcommand(
      command<ChannelContext>('stop')
        .description('cmd.session.stop.description')
        .adminOnly()
        .action(async ({ ctx }) => {
          const session = ctx.getBoundSession()
          if (!ctx.sessionId || !session) {
