...
export function createSessionWorkspacesRepo(db: SqliteDatabase) {
  const list = (filter: {
    ...
    const where = filter.state == null ? '' : 'WHERE state = ?'
    ...
    `)
    const rows = filter.state == null
      ? stmt.all<SessionWorkspaceDbRow>()
      : stmt.all<SessionWorkspaceDbRow>(filter.state)
    return rows.map(mapSessionWorkspaceRow)
  }
