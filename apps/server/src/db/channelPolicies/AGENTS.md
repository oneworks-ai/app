...
- `schema.ts`：维护频道策略运行态表，包括固定回复节流、下班 backlog、webhook nonce，以及 policy state/event。
...
- `policy-record.ts` / `policy-repo.ts`：issuer-scoped subject 的 policy state、CAS revision 和幂等审计事件；同一 inbound hit 的状态变更与 event 必须保持同一事务。
