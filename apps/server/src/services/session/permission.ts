...
import { buildConfigSections, updateConfigFile } from '@oneworks/config'
import type {
...
const CHANNEL_SESSION_BUILTIN_PERMISSION_KEY_SET = new Set<string>(CHANNEL_SESSION_BUILTIN_PERMISSION_KEYS)
...
export const syncPermissionStateMirror = async (
  ...
  const env = createWorkspaceRuntimeEnv(workspaceFolder, process.env)
...
export const resolvePermissionLookupKeysFromInput = (params: {
  toolName?: string
  toolInput?: unknown
}) => (
  resolveTrustedOneworksCliPermissionSubjectFromToolCall(params) == null
    ? []
    : TRUSTED_ONEWORKS_CLI_PERMISSION_BASH_LOOKUP_KEYS
)
