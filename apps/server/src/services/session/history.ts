...
const resolveMessageBranchGroupId = (
  originalSession: Session,
  messageId: string,
  branchBaseMessageIndex: number
) => {
  if (
    originalSession.messageBranchGroupId != null &&
    originalSession.messageBranchGroupId !== '' &&
    originalSession.messageBranchBaseMessageIndex === branchBaseMessageIndex
  ) {
    return originalSession.messageBranchGroupId
  }

  const existingBranch = getDb().getSessions('all').find(session =>
    session.messageBranchGroupId != null &&
    session.messageBranchGroupId !== '' &&
    session.messageBranchSourceSessionId === originalSession.id &&
    session.messageBranchSourceMessageId === messageId
  )

  return existingBranch?.messageBranchGroupId ?? uuidv4()
}

export const buildHistorySeedFromEvents = (events: WSEvent[]) => buildHistorySeed(getVisibleMessages(events))
...
export async function branchSessionFromMessage(options: {
  ...
  title?: string
}): Promise<{ historySeed?: string; session: Session; replayContent?: string | ChatMessageContent[] }> {
  const db = getDb()
  ...
  const branchBaseMessageIndex = visibleMessages.length
  const messageBranchGroupId = resolveMessageBranchGroupId(
    originalSession,
    options.messageId,
    branchBaseMessageIndex
  )
  ...
  const branchSession = db.createSession(
    ...
    originalSession.id,
    {
      ...
      messageBranchAction: options.action,
      messageBranchBaseMessageIndex: branchBaseMessageIndex,
      messageBranchGroupId,
      messageBranchSourceMessageId: options.messageId,
      messageBranchSourceSessionId: originalSession.id
    }
  )
  ...
  db.updateSession(branchSession.id, {
    ...
    fastMode: originalSession.fastMode,
