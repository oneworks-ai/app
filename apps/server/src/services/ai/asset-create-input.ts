import { toCanonicalAssetSlug } from '@oneworks/utils'
...
const SECRET_PATTERNS = [
  ...
  /\b(?:api[_-]?key|token|secret|password)\s*[:=]\s*\S{4,}/iu,
  /\b(?:api[_-]?key|token|secret|password)\s+\S{4,}/iu,
  /\bBearer\s+[\w.~+/-]{12,}=*/u,
  /\b(?:AKIA[0-9A-Z]{16}|gh[pousr]_\w{20,}|sk-[\w-]{12,})\b/u,
  /\baws_secret_access_key\s*[:=]\s*\S+/iu,
  /\baws_secret_access_key\s+\S+/iu
]
...
export const validateCreateAssetInput = (input: unknown): ValidatedCreateAssetInput => {
  ...
  if (!hasOwn(input, 'kind') || !hasOwn(input, 'name')) {
    throw badRequest('Missing asset kind or name', undefined, 'invalid_asset_input')
  }
  ...
  if (typeof input.name !== 'string') {
    throw badRequest('Invalid asset name', undefined, 'invalid_asset_input')
  }
  const slug = toCanonicalAssetSlug(input.name)
  if (slug == null) throw badRequest('Invalid asset name', undefined, 'invalid_asset_name')
  ...
  const params = (input.params ?? []).map((param) => {
    if (
      !isPlainRecord(param) ||
      Object.keys(param).some(key => key !== 'name' && key !== 'description') ||
    ...
    return {
      ...
      ...(param.description == null ? {} : { description: readString(param.description, 'parameter description', 500) })
    }
  })
  ...
  return {
    ...
    name,
    slug,
    description: readString(input.description, 'asset description', 2_000),
    params
  }
}
