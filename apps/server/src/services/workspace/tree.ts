...
const getWorkspaceLinkType = (linkStat?: Stats) =>
  linkStat == null ? 'missing' : linkStat.isDirectory() ? 'directory' : linkStat.isFile() ? 'file' : 'other'
...
export const listWorkspaceTree = async (
  ...
  for (const entry of entries) {
    ...
    if (entry.isSymbolicLink()) {
      ...
      const linkType = getWorkspaceLinkType(linkStat)
