...
export const resolveWorkspaceFileEntryPath = async (
  rawPath: string | undefined,
  options: { workspaceFolder?: string } = {}
) => {
  const resolved = await resolveWorkspaceEntryPath(rawPath, options)
  ...
  if (
    (resolved.normalizedPath === '.git' || resolved.normalizedPath.endsWith('/.git')) &&
    await readGitdirFileTarget(resolved.filePath) != null
  ) {
