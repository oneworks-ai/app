...
import { resolveProjectOoPath } from '@oneworks/utils/ai-path'
...
const getRuntimeWorkspaceId = () => {
  const workspaceId = readNonEmptyEnv('__ONEWORKS_PROJECT_WORKSPACE_ID__')
  if (workspaceId == null) return undefined
  if (
    path.isAbsolute(workspaceId) ||
    /^(?:file:\/\/\/|[a-z]:[\\/]|\\\\)/iu.test(workspaceId)
  ) {
...
const createRuntimeEndpointId = (
  ...
  if (role === 'workspace') {
    const workspaceId = getRuntimeWorkspaceId() ?? (
      workspaceFolder === ''
        ? String(process.pid)
        : createHash('sha256').update(workspaceFolder).digest('hex').slice(0, 24)
    )
...
const serializeRuntimeEndpoint = (
  ...
  const serverBaseUrl = endpoint.serverBaseUrl == null ||
  ...
  return {
    ...
    ...(endpoint.startedAt == null ? {} : { startedAt: endpoint.startedAt }),
  ...
  try {
    ...
    for (const [key, entry] of url.searchParams.entries()) {
      if (
        ...
        isCredentialLikeNativeAppValue(entry)
      ) return undefined
    }
    return value
  } catch {
...
export class PluginManager {
  ...
  private async clearRecordRuntime(record: RuntimeRecord) {
    ...
    for (const disposable of disposables) {
      await Promise.resolve(disposable()).catch(error => {
        logger.warn({
          error: sanitizePluginErrorMessage(error, this.getPrivateRoots([record.instance.pluginRoot])),
          ...(sanitizePublicScope(record.instance.scope, this.getPrivateRoots()) == null
            ? {}
            : { scope: sanitizePublicScope(record.instance.scope, this.getPrivateRoots()) })
        }, '[plugins] dispose failed')
      })
    }
  }
  ...
  getPublicRuntimeEndpoint(): PluginRuntimeEndpoint {
  ...
  async listRuntimeEndpoints(): Promise<PluginRuntimeEndpoint[]> {
    ...
    try {
      ...
      logger.warn(
        {
          error: sanitizePluginErrorMessage(error, this.getPrivateRoots())
        },
        '[plugins] failed to list workspace runtime endpoints'
      )
    }
  ...
  async listPublicRuntimeEndpoints(): Promise<PluginRuntimeEndpoint[]> {
  ...
  private async addInstance(
    ...
    try {
      ...
      this.validateServerManifest(scope, manifest)
  ...
  private validateServerManifest(scope: string, manifest: PluginRuntimeManifest) {
    ...
    server.roles = roles
    logger.debug?.({ scope, roles }, '[plugins] validated server runtime roles')
  }
  ...
  private failRecordWatch(record: RuntimeRecord, error: unknown) {
    ...
    logger.warn({
      ...
      ...(sanitizePublicScope(record.instance.scope, this.getPrivateRoots()) == null
        ? {}
        : { scope: sanitizePublicScope(record.instance.scope, this.getPrivateRoots()) })
    }, '[plugins] failed to start watch mode')
  }
  ...
  private startDiscoveryWatch(root: string) {
    try {
      ...
      watcher.once('error', (error) => {
        ...
        logger.warn(
          {
            error: sanitizePluginErrorMessage(error, this.getPrivateRoots([root]))
          },
          '[plugins] discovery watch failed; continuing without live reload'
        )
      })
    } catch {
    ...
    record.watchTimer = setTimeout(() => {
      ...
      void this.reload()
        ...
        .catch((error) => {
          logger.warn({
            error: sanitizePluginErrorMessage(error, this.getPrivateRoots([record.instance.pluginRoot])),
            ...(sanitizePublicScope(scope, this.getPrivateRoots()) == null
              ? {}
              : { scope: sanitizePublicScope(scope, this.getPrivateRoots()) })
          }, '[plugins] failed to reload after watched file change')
        })
      ...
      void this.reload()
        ...
        .catch((error) => {
          logger.warn(
            ...
            '[plugins] failed to reload after discovery root change'
          )
        })
    }, PLUGIN_WATCH_DEBOUNCE_MS)
  }
    ...
    for (const [subscriber, scope] of this.watchSubscribers.entries()) {
      ...
      try {
        ...
        logger.warn(
          {
            error: sanitizePluginErrorMessage(error, this.getPrivateRoots()),
            ...(sanitizePublicScope(event.scope, this.getPrivateRoots()) == null
              ? {}
              : { scope: sanitizePublicScope(event.scope, this.getPrivateRoots()) })
          },
          '[plugins] failed to send watch event'
        )
      }
    }
  }
  ...
  private async activateRecord(record: RuntimeRecord) {
    ...
    try {
      ...
      const diagnostic = {
        ...
        message: sanitizePluginErrorMessage(error, this.getPrivateRoots([record.instance.pluginRoot])),
        scope: record.instance.scope,
        pluginRoot: record.instance.pluginRoot
      }
