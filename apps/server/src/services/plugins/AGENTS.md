...
- `manager` runtime 运行在 management server 进程，负责设备 / daemon 级管理面：发现和启动 workspace server、聚合运行中的 workspace metadata、承接 launcher 控制面，以及通过平台 channel 路由跨 runtime 消息。它不直接承载某个 workspace 的 plugin registry，也不把 Relay 这类可选插件能力写进默认管理链路。
