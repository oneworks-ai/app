...
export const createClientSourceBoundaryPlugin = ({
  ...
  const resolvedBareDependencyFiles = new Set<string>()
  ...
  return {
    ...
    async resolveId(source, importer, options) {
      ...
      resolvedBareDependencyFiles.add(realResolved)
      return resolved
    },
    async generateBundle(_options, bundle) {
      ...
      for (const watchFile of this.getWatchFiles()) {
        ...
        const isResolvedBareDependencyManifest = realWatchFile != null &&
          path.basename(realWatchFile) === 'package.json' &&
          [...resolvedBareDependencyFiles].some(moduleFile => isPathInside(path.dirname(realWatchFile), moduleFile))
        if (
          realWatchFile != null &&
          realWatchFile !== packageManifest &&
          !isBundledDependencyManifest &&
          !isResolvedBareDependencyManifest &&
          !bundledModuleFiles.has(realWatchFile) &&
          !resolvedBareDependencyFiles.has(realWatchFile) &&
