...
} from './marketplace-uninstall-quotes.js'
...
const buildResolvedPlan = async (
  scope: string,
  state: ConfigState,
  tokenForDigest: (digest: string) => string = digest => (
    issuePluginMarketplaceUninstallQuote({ digest, scope })
  )
  ...
  const installSource = install.config.source
  if (installSource.type !== 'marketplace') {
    return unavailable('not-managed-marketplace')
  }
  const duplicateSourceInstalls = installs.filter(candidate => (
    candidate.config.source.type === 'marketplace' &&
    candidate.config.source.marketplace === installSource.marketplace &&
    candidate.config.source.plugin === installSource.plugin
  ))
  ...
  return {
    ...
    plan: {
      ...
      token: tokenForDigest(snapshotDigest)
    },
    replacementSection,
    snapshotDigest
  }
...
export const uninstallPluginMarketplacePlugin = async (params: {
  ...
  const quoteClaim = claimPluginMarketplaceUninstallQuote(params)
  if (quoteClaim == null) throw new PluginMarketplaceUninstallStaleError()
  let reusable = true
  try {
    const result = await withCurrentWorkspaceMutation(async (state) => {
      const recovered = await recoverPendingRemovals(state)
      if (recovered.length > 0) {
        await getPluginManager().reload()
        reusable = false
      ...
      const resolved = await buildResolvedPlan(
        params.scope,
        await loadConfigState(),
        () => params.token
      )
      reusable = false
      if (!('plan' in resolved) || resolved.snapshotDigest !== quoteClaim.digest) {
        throw new PluginMarketplaceUninstallStaleError()
      }
      ...
      try {
        ...
        if (!configUpdated) {
          ...
          reusable = !(error instanceof PluginMarketplaceUninstallStaleError)
          throw error
        }
        if (readyForCleanup) throw error
    ...
    consumePluginMarketplaceUninstallQuote(quoteClaim)
    return result
  } catch (error) {
    if (reusable) {
      releasePluginMarketplaceUninstallQuote(quoteClaim)
    } else {
      consumePluginMarketplaceUninstallQuote(quoteClaim)
    }
    throw error
