/* eslint-disable max-lines -- Android debugging needs ADB discovery, forwarding, input, and tree parsing together. */
...
const VIDEO_FRAME_BINARY_HEADER_SIZE = 10
...
export interface MobileDeviceLogsResponse {
  capturedAt: number
  deviceId: string
  lineLimit: number
  lines: string[]
...
const applyPortForwardingRules = async ({
  ...
  for (const rule of rules) {
    ...
    const ruleId = rule.id ?? `${rule.localAddress}:${rule.devicePort}`
...
const startMobileDeviceVideoStreamSession = async (
  deviceId: unknown
): Promise<{ response: MobileDeviceVideoStreamStartResponse; session: MobileDeviceVideoStreamSession }> => {
...
const encodeVideoFramePacket = (event: MobileDeviceVideoFrameEvent) => {
  const data = Buffer.from(event.data)
  const header = Buffer.alloc(VIDEO_FRAME_BINARY_HEADER_SIZE)
  header.writeUInt8(1, 0)
  header.writeUInt8(event.type === 'configuration' ? 0 : event.keyframe === true ? 2 : 1, 1)
  header.writeUInt32BE(event.width ?? 0, 2)
  header.writeUInt32BE(event.height ?? 0, 6)
...
const sendMobileDeviceVideoStreamStatus = (
  ...
  ws.send(JSON.stringify({ data: event, type: 'mobile-debug-video-status' }))
}

const pumpMobileDeviceVideoStreamSocket = async (
  ws: WebSocket,
  session: MobileDeviceVideoStreamSession
) => {
  const videoStream = await session.client.videoStream
  if (videoStream == null) throw new Error('scrcpy video stream is disabled.')
  const reader = videoStream.stream.getReader()
  try {
    while (!session.isClosed && ws.readyState === WebSocketImpl.OPEN) {
      const { done, value } = await reader.read()
      if (done) break
      const event = toMobileDeviceVideoFrameEvent(session, value, {
        height: videoStream.height || videoStream.metadata.height,
        width: videoStream.width || videoStream.metadata.width
      })
      ws.send(encodeVideoFramePacket(event))
    }
    await closeMobileDeviceVideoStreamSession(session, 'closed')
    sendMobileDeviceVideoStreamStatus(ws, {
      deviceId: session.deviceId,
      status: 'closed',
      streamId: session.streamId
    })
  } catch (error) {
    await closeMobileDeviceVideoStreamSession(session, 'error')
    sendMobileDeviceVideoStreamStatus(ws, {
      deviceId: session.deviceId,
      message: toErrorMessage(error),
      status: 'error',
...
const findMobileDeviceVideoStreamSession = (deviceId: string) => {
...
const trySendScrcpyMobileDeviceInput = async (
  ...
  const session = findMobileDeviceVideoStreamSession(deviceId)
  const controller = session?.controller
...
const readCurrentUserRotation = async (adbPath: string, deviceId: string) => {
  try {
    const result = await runCommand(adbPath, ['-s', deviceId, 'shell', 'settings', 'get', 'system', 'user_rotation'])
...
const sendAdbAction = async (adbPath: string, deviceId: string, action: MobileDeviceInputEvent['action']) => {
  if (action === 'collapse-panels') {
    await runCommand(adbPath, ['-s', deviceId, 'shell', 'cmd', 'statusbar', 'collapse'], ADB_INPUT_TIMEOUT_MS)
    return
  }
  if (action === 'notifications') {
    await runCommand(
      adbPath,
      ['-s', deviceId, 'shell', 'cmd', 'statusbar', 'expand-notifications'],
      ADB_INPUT_TIMEOUT_MS
    )
    return
  }
  if (action === 'quick-settings') {
    await runCommand(adbPath, ['-s', deviceId, 'shell', 'cmd', 'statusbar', 'expand-settings'], ADB_INPUT_TIMEOUT_MS)
    return
  }
  if (action === 'rotate') {
    const currentRotation = await readCurrentUserRotation(adbPath, deviceId)
    const nextRotation = currentRotation % 2 === 0 ? 1 : 0
    await runCommand(adbPath, ['-s', deviceId, 'shell', 'settings', 'put', 'system', 'accelerometer_rotation', '0'])
    await runCommand(
      adbPath,
      ['-s', deviceId, 'shell', 'settings', 'put', 'system', 'user_rotation', String(nextRotation)],
      ADB_INPUT_TIMEOUT_MS
    )
    return
  }
...
export const readMobileDeviceLogs = async (
  deviceId: unknown,
  input: unknown
  ...
  const result = await runDeviceCommand(
    adbPath,
    device.id,
    ['shell', 'logcat', '-d', '-t', String(lineLimit), '-v', 'threadtime'],
    ADB_LOGCAT_TIMEOUT_MS
  )
  return {
    capturedAt: Date.now(),
    deviceId: device.id,
...
export const sendMobileDeviceInput = async (
  ...
  if (await trySendScrcpyMobileDeviceInput(device.id, inputEvent)) {
  ...
  if (inputEvent.kind === 'touch') {
    ...
    await sendAdbAction(adbPath, device.id, inputEvent.action)
  } else {
    await runCommand(
      adbPath,
...
export const handleMobileDeviceVideoStreamSocket = async (
  ...
  let session: MobileDeviceVideoStreamSession | undefined
  try {
    const started = await startMobileDeviceVideoStreamSession(deviceId)
    ...
    ws.send(JSON.stringify({
      data: started.response,
      type: 'mobile-debug-video-started'
    }))

    ws.once('close', () => {
      if (session != null) {
        void closeMobileDeviceVideoStreamSession(session, 'closed')
      }
    })
    await pumpMobileDeviceVideoStreamSocket(ws, session)
  } catch (error) {
    if (session != null) {
      await closeMobileDeviceVideoStreamSession(session, 'error')
    }
    ...
    ws.close(1011, 'Mobile debug video stream failed')
  }
}
