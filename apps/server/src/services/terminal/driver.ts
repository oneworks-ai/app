...
const nodePtyRequire = createRequire(
  typeof __filename === 'string'
    ? __filename
    : path.join(process.env.__ONEWORKS_PROJECT_PACKAGE_DIR__ ?? process.cwd(), 'package.json')
)
...
const ensureNodePtyHelperExecutable = () => {
  ...
  const packageRoot = path.dirname(nodePtyRequire.resolve('node-pty/package.json'))
