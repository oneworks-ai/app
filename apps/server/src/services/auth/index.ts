...
const resolveDefaultEnabled = (env: ServerEnv) => !isLocalServerHost(env.__ONEWORKS_PROJECT_SERVER_HOST__)
...
export const resolveWebAuthConfig = async (env: ServerEnv): Promise<WebAuthConfig> => {
  ...
  const envEnabled = toBoolean(processEnv.__ONEWORKS_PROJECT_WEB_AUTH_ENABLED__)
  const enabled = envEnabled ?? toBoolean(webAuth?.enabled) ?? resolveDefaultEnabled(env)
  const sessionTtlHours = toPositiveNumber(processEnv.__ONEWORKS_PROJECT_WEB_AUTH_SESSION_TTL_HOURS__) ??
    toPositiveNumber(webAuth?.sessionTtlHours) ??
    DEFAULT_SESSION_TTL_HOURS
  const rememberDeviceTtlDays = toPositiveNumber(processEnv.__ONEWORKS_PROJECT_WEB_AUTH_REMEMBER_DEVICE_TTL_DAYS__) ??
    toPositiveNumber(webAuth?.rememberDeviceTtlDays) ??
    DEFAULT_REMEMBER_DEVICE_TTL_DAYS
  const envPassword = processEnv.__ONEWORKS_PROJECT_WEB_AUTH_PASSWORD?.trim()
  ...
  if (envPassword != null && envPassword !== '') {
    accounts = [{
      username: processEnv.__ONEWORKS_PROJECT_WEB_AUTH_USERNAME?.trim() ||
