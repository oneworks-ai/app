...
export const AUTH_COOKIE_NAME = 'oneworks_web_auth'

export const getAuthDataDir = (env: Pick<ServerEnv, '__ONEWORKS_PROJECT_SERVER_DATA_DIR__'>) =>
  resolve(cwd(), env.__ONEWORKS_PROJECT_SERVER_DATA_DIR__)
...
const getSessionSecret = async (env: ServerEnv) => {
  if (env.__ONEWORKS_PROJECT_SERVER_ACTION_SECRET__?.trim()) {
    return env.__ONEWORKS_PROJECT_SERVER_ACTION_SECRET__.trim()
  }
