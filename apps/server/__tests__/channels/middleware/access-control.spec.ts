...
describe('checkChannelAccess', () => {
  ...
  it('blocks admin normal messages in stopped groups', () => {
    const config: any = {
      access: {
        admins: ['admin1'],
        blockedGroups: ['grp1']
    ...
    expect(checkChannelAccess(
      makeInbound({ channelId: 'grp1', senderId: 'admin1', sessionType: 'group' }) as any,
      config
    )).toBe(false)
  })

  it('allows admin /start in stopped groups', () => {
    const config: any = {
      access: {
        admins: ['admin1'],
        blockedGroups: ['grp1']
...
describe('accessControlMiddleware', () => {
  ...
  it('lets admin /start through from a stopped group', async () => {
    const next = vi.fn().mockResolvedValue(undefined)
    const ctx = makeCtx({
      commandText: '/start',
      config: { access: { admins: ['admin1'], blockedGroups: ['grp1'] } } as any,
