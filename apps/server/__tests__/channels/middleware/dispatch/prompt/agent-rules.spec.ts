...
beforeEach(() => {
  ...
  process.env.__ONEWORKS_PROJECT_WORKSPACE_FOLDER__ = '/project'
})
...
describe('loadChannelAgentRules', () => {
  it('returns content from the .oo/rules file when it exists', async () => {
    readFileMock.mockImplementation(async (path) => {
      if (String(path).includes('.oo/rules/AGENTS.channel.lark.md')) return 'ai rules'
  ...
  it('falls back to the project root file when .oo/rules is missing', async () => {
    readFileMock.mockImplementation(async (path) => {
      if (String(path).endsWith('AGENTS.channel.lark.md') && !String(path).includes('.oo/rules/')) {
        return '  root rules  '
      }
      throw Object.assign(new Error('ENOENT'), { code: 'ENOENT' })
    })
  ...
  it('skips a file whose content is blank and tries the next', async () => {
    readFileMock.mockImplementation(async (path) => {
      if (String(path).includes('.oo/rules/')) return '   '
