...
import { ADMIN_BOOTSTRAP_REPLY_TEXT } from '#~/channels/middleware/admin-bootstrap.js'
...
describe('adminGateMiddleware', () => {
  ...
  it('blocks when no admins are configured', async () => {
    const next = vi.fn()
    const ctx = makeCtx({ config: { access: {} } as any })
    await adminGateMiddleware(ctx, next)
    expect(next).not.toHaveBeenCalled()
    expect(ctx.reply).toHaveBeenCalledWith(ADMIN_BOOTSTRAP_REPLY_TEXT)
  })

  it('blocks when admins list is empty', async () => {
    const next = vi.fn()
    const ctx = makeCtx({ config: { access: { admins: [] } } as any })
    await adminGateMiddleware(ctx, next)
    expect(next).not.toHaveBeenCalled()
    expect(ctx.reply).toHaveBeenCalledWith(ADMIN_BOOTSTRAP_REPLY_TEXT)
  })
