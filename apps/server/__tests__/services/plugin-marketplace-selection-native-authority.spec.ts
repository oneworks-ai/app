...
describe('plugin marketplace selection with native authority', () => {
  ...
  let controlRoot = ''
  let installer: AdapterPluginInstaller
  ...
  beforeEach(async () => {
    ...
    loadAdapterPluginInstallerMock.mockResolvedValue(installer)
  ...
  it('keeps a real authority claim across a slow install, removal, and reinstall', async () => {
    ...
    const resolveSource = installer.resolveSource!
    installer.resolveSource = async context => {
      installStarted()
      await installGate
      return resolveSource(context)
    }
    const mutationRuntime = {
      openAuthority: (workspaceRoot: string) =>
        openFilesystemAuthorityForTest(workspaceRoot, {
          autoStart: false,
          controlRoot,
          secret,
    ...
    try {
      const install = setPluginMarketplaceSelection({
        enabled: true,
        marketplace: 'team-tools',
        plugin: 'reviewer',
        target: 'project'
    ...
    installer.resolveSource = resolveSource
    await expect(setPluginMarketplaceSelection({
      enabled: false,
      marketplace: 'team-tools',
      plugin: 'reviewer',
      target: 'project'
    }, mutationRuntime)).resolves.toEqual([{
      action: 'removed',
      marketplace: 'team-tools',
      plugin: 'reviewer'
    }])
