...
import type { ChatMessageContent, WSEvent } from '@oneworks/core'
...
describe('session history branching', () => {
  ...
  it('creates an edited child session from a user message', async () => {
    ...
    expect(branched.session.messageBranchAction).toBe('edit')
    expect(branched.session.messageBranchBaseMessageIndex).toBe(2)
    expect(branched.session.messageBranchGroupId).toEqual(expect.any(String))
    expect(branched.session.messageBranchSourceMessageId).toBe('user-2')
    expect(branched.session.messageBranchSourceSessionId).toBe(original.id)
    ...
    expect(provisionSessionWorkspace).toHaveBeenCalledWith(branched.session.id, {
    ...
    expect(vi.mocked(notifySessionUpdated)).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        messageBranchSourceSessionId: original.id,
  ...
  it('reuses the same message branch group when editing an existing branch version', async () => {
    const original = db.createSession('Original', 'session-root', 'completed')
    const events: WSEvent[] = [
      {
        type: 'message',
    ...
    const firstBranch = await branchSessionFromMessage({
      sessionId: original.id,
      messageId: 'user-2',
      action: 'edit',
      content: 'edited prompt'
    })
    db.saveMessage(firstBranch.session.id, {
      type: 'message',
      message: {
        id: 'edit-user-1',
        role: 'user',
    ...
    const secondBranch = await branchSessionFromMessage({
      sessionId: firstBranch.session.id,
      messageId: 'edit-user-1',
      action: 'edit',
      content: 'edited again'
    })

    expect(secondBranch.session.messageBranchGroupId).toBe(firstBranch.session.messageBranchGroupId)
    expect(secondBranch.session.messageBranchBaseMessageIndex).toBe(2)
    expect(secondBranch.session.messageBranchSourceMessageId).toBe('edit-user-1')
    expect(secondBranch.session.messageBranchSourceSessionId).toBe(firstBranch.session.id)
  })
