...
export type SessionWorkspaceChangeOutcome = 'completed' | 'failed' | 'terminated'

export interface SessionWorkspaceFileDiff {
  patch: string
  truncated: boolean
}

export interface SessionWorkspaceChangedFile extends GitChangedFile {
  additions: number
  deletions: number
  diff?: SessionWorkspaceFileDiff
}

export interface SessionWorkspaceChanges {
  id: string
  sessionId: string
  cwd: string
  ...
  files: SessionWorkspaceChangedFile[]
}
