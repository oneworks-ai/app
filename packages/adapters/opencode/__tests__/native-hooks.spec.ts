...
describe('ensureOpenCodeNativeHooksInstalled', () => {
  it('creates an isolated mock config dir and installs the managed plugin there', async () => {
    ...
    const pluginPath = join(managedConfigDir, 'plugins', 'oneworks-hooks.js')

    expect(installed).toBe(true)
    expect(ctx.env.__ONEWORKS_PROJECT_OPENCODE_NATIVE_HOOKS_AVAILABLE__).toBe('1')
  ...
  it('installs the managed bridge when builtin permission hooks are enabled without user plugins', async () => {
    ...
    const realHome = await createTempDir('opencode-hooks-builtin-home-')
    const mockHome = resolveTestMockHome(workspace, realHome)
    process.env.__ONEWORKS_PROJECT_REAL_HOME__ = realHome
    const ctx = {
      cwd: workspace,
      env: {
        HOME: mockHome,
        __ONEWORKS_PROJECT_ENABLE_BUILTIN_PERMISSION_HOOKS__: '1'
      },
      logger: {
        info: vi.fn(),
        warn: vi.fn(),
    ...
    const installed = await ensureOpenCodeNativeHooksInstalled(ctx)
    const pluginPath = join(mockHome, '.config', 'opencode', 'plugins', 'oneworks-hooks.js')
  ...
  it('keeps concurrent mock-home config mirroring idempotent when multiple OneWorks processes install native hooks together', async () => {
