...
describe('createOpenCodeSession stream runtime', () => {
  ...
  it('emits init, message, and stop for a successful stream turn', async () => {
    mockExecFileJsonResponses(execFileMock, [
      { id: 'sess_1', title: 'OneWorks:session-1', updatedAt: '2026-03-26T00:00:00.000Z' }
    ])
  ...
  it('emits exit when stop() is called after a successful stream turn', async () => {
    mockExecFileJsonResponses(execFileMock, [
      { id: 'sess_stop', title: 'OneWorks:session-stop', updatedAt: '2026-03-26T00:00:00.000Z' }
    ])
    spawnMock.mockImplementation(() => makeProc({ stdout: 'pong\n' }))
  ...
  it('reuses the cached OpenCode session id for later turns', async () => {
    mockExecFileJsonResponses(execFileMock, [{
      id: 'sess_1',
      title: 'OneWorks:session-stream',
      updatedAt: '2026-03-26T00:00:00.000Z'
    }], [{ id: 'sess_1', title: 'OneWorks:session-stream', updatedAt: '2026-03-26T00:01:00.000Z' }])
    spawnMock
  ...
  it('does not start a turn until a message arrives when create has no description', async () => {
    ...
    mockExecFileJsonResponses(execFileMock, [
      { id: 'sess_later', title: 'OneWorks:session-empty-create', updatedAt: '2026-03-26T00:00:00.000Z' }
    ])
  ...
  it('retries without the cached session id when resume hits a missing session', async () => {
    mockExecFileJsonResponses(execFileMock, [{
      id: 'sess_new',
      title: 'OneWorks:session-resume',
      updatedAt: '2026-03-26T00:00:00.000Z'
    }], [{ id: 'sess_new', title: 'OneWorks:session-resume', updatedAt: '2026-03-26T00:00:01.000Z' }])
    spawnMock
