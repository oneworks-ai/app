...
describe('createOpenCodeSession direct runtime', () => {
  ...
  it('emits exit in direct mode after the child process finishes', async () => {
    mockExecFileJsonResponses(execFileMock, [
      { id: 'sess_direct', title: 'OneWorks:session-direct', updatedAt: '2026-03-26T00:00:00.000Z' }
    ])
  ...
  it('injects the shared native hook bridge adapter env when opencode native hooks are enabled', async () => {
    mockExecFileJsonResponses(execFileMock, [
      { id: 'sess_native_hooks', title: 'OneWorks:session-native-hooks', updatedAt: '2026-03-26T00:00:00.000Z' }
    ])
    spawnMock.mockImplementation(() => makeProc({ exitCode: 0 }))

    const { ctx } = makeCtx({
      env: {
        __ONEWORKS_PROJECT_OPENCODE_NATIVE_HOOKS_AVAILABLE__: '1'
      }
    })
    ...
    expect(spawnOptions.env?.__ONEWORKS_OPENCODE_HOOKS_ACTIVE__).toBe('1')
  ...
  it('resolves the latest session id before starting direct resume mode', async () => {
    mockExecFileJsonResponses(execFileMock, [{
      id: 'sess_latest',
      title: 'OneWorks:session-direct-resume',
      updatedAt: '2026-03-26T00:00:00.000Z'
    }], [{ id: 'sess_latest', title: 'OneWorks:session-direct-resume', updatedAt: '2026-03-26T00:00:01.000Z' }])
    spawnMock.mockImplementation(() => makeProc({ exitCode: 0 }))
