...
describe('initOpenCodeAdapter', () => {
  it('symlinks real-home auth files into the shared OpenCode mock home', async () => {
    ...
    await mkdir(join(mockHome, '.local'), { recursive: true })
    await symlink(join(realHome, '.local', 'share'), join(mockHome, '.local', 'share'), 'dir')
    ...
    expect(await readFile(join(realHome, '.local', 'share', 'opencode', 'auth.json'), 'utf8')).toBe('{}\n')
  })

  it('keeps concurrent auth sync idempotent when multiple ow processes initialize OpenCode together', async () => {
    const workspace = await createWorkspace()
    const realHome = await createWorkspace()
    const mockHome = resolveTestMockHome(workspace, realHome)
