...
- `src/runtime/native-hooks.ts`
  - 把 `<project-home>/.mock/.config/opencode/` 写成托管 config/plugin
...
- `src/runtime/native-hooks.ts`
  - 负责 `<project-home>/.mock/.config/opencode/plugins/oneworks-hooks.js`
...
- OpenCode 的 skills 继续保持 session 级 `OPENCODE_CONFIG_DIR` 投影；不要把 `.oo/skills` 在 init 阶段全量挂进 mock home，否则会绕过 include/exclude skill 选择和 overlay 规划
...
这条命令会先试 `ow --adapter opencode`，超时后自动 fallback 到 upstream `opencode run --format json`。
它默认也会启动本地 mock LLM server，并通过仓库根 `.oo.config.json` 里的 `hook-smoke-mock` model service 驱动 OpenCode。adapter E2E 的共享 harness 在 `scripts/adapter-e2e/`，scripts CLI 入口在 `scripts/cli.ts`。
case 定义、spec 和期望快照统一维护在 `scripts/__tests__/adapter-e2e/`。

```bash
__ONEWORKS_PROJECT_CTX_ID__='hooks-smoke-opencode' \
...
```bash
HOME="<project-home>/.mock" \
OPENCODE_CONFIG_DIR="<project-home>/.mock/.opencode-adapter/<sessionId>/config-dir" \
__ONEWORKS_OPENCODE_HOOKS_ACTIVE__='1' \
__ONEWORKS_PROJECT_WORKSPACE_FOLDER__="$PWD" \
__ONEWORKS_PROJECT_NODE_PATH__="$(which node)" \
__ONEWORKS_PROJECT_REAL_HOME__="$HOME" \
__ONEWORKS_PROJECT_CLI_PACKAGE_DIR__="$PWD/apps/cli" \
__ONEWORKS_PROJECT_PACKAGE_DIR__="$PWD/apps/cli" \
__ONEWORKS_OPENCODE_TASK_SESSION_ID__='<sessionId>' \
__ONEWORKS_OPENCODE_HOOK_RUNTIME__='cli' \
...
- 终端输出 `E2E_OPENCODE`
- `<project-home>/logs/<ctxId>/<sessionId>.log.md` 出现 `SessionStart` / `PreToolUse` / `PostToolUse` / `Stop`
- `<project-home>/.mock/.config/opencode/opencode.json` 与 `plugins/oneworks-hooks.js` 仍落在 mock config dir
