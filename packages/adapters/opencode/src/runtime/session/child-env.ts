...
export const buildChildEnv = async (params: {
  ...
  const nativeHooksAvailable = params.ctx.env.__ONEWORKS_PROJECT_OPENCODE_NATIVE_HOOKS_AVAILABLE__ === '1'
    ...
    env: toProcessEnv({
      ...
      ...(nativeHooksAvailable
        ? {
          ...
          __ONEWORKS_OPENCODE_HOOK_RUNTIME__: params.options.runtime,
          __ONEWORKS_OPENCODE_TASK_SESSION_ID__: params.options.sessionId
        }
        : {}),
      ...(configDir == null
...
export const ensureSystemPromptFile = async (
  ...
  const promptDir = resolveProjectOoPath(
    ctx.cwd,
    ctx.env,
    '.mock',
    '.opencode-adapter',
