...
const NODE_PATH = process.env.__ONEWORKS_PROJECT_NODE_PATH__ ?? ${JSON.stringify(process.execPath)}
...
const callOneWorksHook = (payload) => {
  ...
  try {
    const result = spawnSync(NODE_PATH, [CALL_HOOK_PATH], {
      ...
      env: {
        ...process.env,
        __ONEWORKS_PROJECT_WORKSPACE_FOLDER__: payload.cwd ?? process.env.__ONEWORKS_PROJECT_WORKSPACE_FOLDER__ ?? process.cwd(),
      },
    })

    if ((result.status ?? 0) !== 0) {
      console.error("[vibe opencode hook] process exited with", result.status, result.stderr)
    ...
    console.error("[vibe opencode hook] failed", error)
    return { continue: true }
  }
}
...
export const OneWorksHooks = async ({ directory }) => ({
  event: async ({ event }) => {
    ...
    if (event?.type === "session.idle") {
      callOneWorksHook(createBaseInput("Stop", directory, false))
    }
  },
  "tool.execute.before": async (input, output) => {
    const result = callOneWorksHook({
    ...
    if (result?.continue === false) {
      throw new Error(stopReason(result, "blocked by OneWorks PreToolUse hook"))
    }
  },
  "tool.execute.after": async (input, output) => {
    ...
    if (result?.continue === false) {
      throw new Error(stopReason(result, "blocked by OneWorks PostToolUse hook"))
    }
  },
  "experimental.session.compacting": async (_input, output) => {
    const result = callOneWorksHook(createBaseInput("PreCompact", directory, false))
...
export const ensureOpenCodeNativeHooksInstalled = async (
  ...
  env.__ONEWORKS_PROJECT_OPENCODE_NATIVE_HOOKS_AVAILABLE__ = '0'
  ...
  try {
    ...
    env.__ONEWORKS_PROJECT_OPENCODE_NATIVE_HOOKS_AVAILABLE__ = enabled ? '1' : '0'
    return enabled
  } catch (error) {
