...
- `.oo/rules/HOOKS.md`
  - 通用 hooks 方案、事件矩阵、project mock-home 托管配置布局
...
- `src/hooks/*.ts`
  ...
  - `native.ts` 写 `<project-home>/.mock/.claude/settings.json`，`bridge.ts` 把 Claude payload 翻译成统一 hook 协议
- `src/ccr/*.ts`
...
- `src/hooks/native.ts`
  - 负责把 `<project-home>/.mock/.claude/settings.json` 写成托管 hooks 配置
- `src/claude/prepare.ts`
  - 注入 session 运行参数、native hook env、settings 与 mcp config
- `src/claude/init.ts`
  - adapter 初始化阶段安装 Claude native hooks，把 `.oo/skills` 软链到 `<project-home>/.mock/.claude/skills`，并写 mock-home `.claude.json` 项目信任状态
...
macOS 的默认 Claude home / Desktop 登录是机器级资源，继续使用机器级 lock、lease 和 binding；One Works 删除这张引用卡片时不得调用会影响默认原生登录的 logout。通过官方 CLI 在稳定 `CLAUDE_CONFIG_DIR` 中登录的其他账号只有在 auth status 证明凭证没有继承到 default / 其他 profile 时才按独立设备凭证处理：每个账号使用自己的 operation lock、session lease 和定向官方 logout，可以与默认账号及其他隔离账号并行运行；旧 CLI 若把 Keychain 凭证带入新 profile，必须在 login 前拒绝，logout 前也要重新验证隔离边界。列表按 email + organization identity 去重；同一身份不能作为 Desktop 引用和隔离账号重复保存。登录失败后的恢复只能清理已证明隔离的目标 profile，不得清除或登出默认 Claude home。
...
- 本包里的例子：
  - `serviceKey,modelName` 模型通过 `src/ccr/daemon.ts` 复用 `<project-home>/.mock/.claude-code-router` 后台进程。
...
这条命令默认会启动本地 mock LLM server，并通过仓库根 `.oo.config.json` 里的 `hook-smoke-mock-ccr` model service 驱动 Claude Code。
...
```bash
__ONEWORKS_PROJECT_CTX_ID__='hooks-smoke-claude' \
...
- 终端输出 `E2E_CLAUDE`
- `<project-home>/logs/<ctxId>/<sessionId>.log.md` 出现 `SessionStart` / `UserPromptSubmit` / `PreToolUse` / `PostToolUse` / `Stop`
- `<project-home>/.mock/.claude/settings.json` 仍指向 One Works 托管 hook bridge
- `<project-home>/.mock/.claude.json` 为当前 workspace 写入 `projects["<cwd>"].hasTrustDialogAccepted = true`

Claude 维护时优先检查两点：

- 仓库默认不提交项目级 `.claude/settings.json`；Claude 托管入口是 `<project-home>/.mock/.claude/settings.json`
- workspace trust state 落在 `<project-home>/.mock/.claude.json`，不是 `.claude/settings.json`
- workspace skills 走 `<project-home>/.mock/.claude/skills`，这是在 mock home 里模拟 Claude 的个人级 `~/.claude/skills`
- 但如果用户自己加了项目级 `.claude/settings.json`，Claude 仍会和 mock home settings 一起加载，容易出现双触发
- 排查重复 hook 时，不要只看 `<project-home>/.mock/.claude/settings.json`，也要检查项目级 `.claude/settings.json`
...
### 1. `npx ow ... --print` 没有输出、输出格式异常、resume/create 行为不对
...
- 主会话日志文件：`<project-home>/logs/<ctxId>/<sessionId>.log.md`
...
- transformer 日志文件：`<project-home>/logs/<ctxId>/<sessionId>/adapter-claude-code/*.log.md`
- 这是 CCR 内部诊断日志，和主会话 `.log.md` 的 debug 开关是两套概念
- 排查“为什么某个 transformer 没生效”时，优先看 `config.ts` 里是否被注入

### 4. `npx ow init`、hook、plugin logger 的日志级别不对
...
- Claude Code native hooks 的托管配置写在 `<project-home>/.mock/.claude/settings.json`
- 所以这类问题不要只盯着 adapter
...
- `__ONEWORKS_PROJECT_SERVER_DEBUG__=true npx ow 你好 --print`

### 场景 B: 只看本次运行生成的日志

不要直接全量 grep `<project-home>/logs`。

推荐做法：

- `touch /tmp/<marker>`
- 执行复现命令
- `find <project-home>/logs -type f -newer /tmp/<marker> | sort`
