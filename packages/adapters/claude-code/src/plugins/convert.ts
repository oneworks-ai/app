...
export const convertClaudePluginToOneWorks = async (params: {
  ...
  for (const [name, sourceDir] of await collectSkillSourceDirs(params.nativePluginRoot, manifest)) {
    ...
    await rewriteMarkdownTree(targetDir, context, `skill "${name}"`, { rewritePluginPaths: false })
  }
  for (
    const [name, sourcePath] of await collectMarkdownFiles(params.nativePluginRoot, manifest?.commands, 'commands')
  ) {
    const targetPath = path.join(params.oneworksRoot, 'skills', name, 'SKILL.md')
    reserveTargetFile(seenTargets, targetPath, `command "${name}"`, params.oneworksRoot)
  ...
  for (const [name, sourcePath] of await collectMarkdownFiles(params.nativePluginRoot, manifest?.agents, 'agents')) {
    const targetPath = path.join(params.oneworksRoot, 'entities', name, 'README.md')
    reserveTargetFile(seenTargets, targetPath, `agent "${name}"`, params.oneworksRoot)
    await fs.mkdir(path.dirname(targetPath), { recursive: true })
    await fs.writeFile(
      targetPath,
      transformClaudeTemplateString(
  ...
  for (
    ...
    const transformed = transformClaudeTemplateValue(
      value,
      context,
      'Claude plugin MCP config',
      { rewritePluginPaths: false }
    )
    mergeMcpConfig(mergedMcp, transformed)
  }
  for (const [name, config] of Object.entries(mergedMcp)) {
    const targetPath = path.join(params.oneworksRoot, 'mcp', `${name}.json`)
    await fs.mkdir(path.dirname(targetPath), { recursive: true })
    await fs.writeFile(
      targetPath,
      `${JSON.stringify(injectClaudePluginRuntimeEnv(config, context, { rewritePluginPaths: false }), null, 2)}\n`,
      'utf8'
    )
  }
  ...
  if (Object.keys(hooks).length > 0) {
    const hooksConfigPath = path.join(params.oneworksRoot, 'hooks', 'claude-hooks.json')
    await fs.mkdir(path.dirname(hooksConfigPath), { recursive: true })
    await fs.writeFile(hooksConfigPath, `${JSON.stringify({ hooks }, null, 2)}\n`, 'utf8')
    await writeGeneratedHooksModule(params.oneworksRoot, params.pluginName)
  }
}
