...
import { mergeProcessEnvWithProjectEnv, resolveProjectOoPath } from '@oneworks/utils'
...
const buildRouterPaths = (cwd: string, env: Record<string, string | null | undefined>) => {
  const mockHome = resolveProjectOoPath(cwd, env as NodeJS.ProcessEnv, '.mock')
...
export const ensureClaudeCodeRouterReady = async (
  ...
  const { configPath, mockHome, pidPath } = buildRouterPaths(cwd, env)
