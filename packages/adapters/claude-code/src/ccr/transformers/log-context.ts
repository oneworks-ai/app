...
import { formatLoggerEntry, resolveProjectOoPath } from '@oneworks/utils'
...
const resolveStoredRequestLogContext = (sessionId) => {
  const workspace = process.env.__ONEWORKS_PROJECT_WORKSPACE_FOLDER__
  if (
...
export const resolveRequestLogContext = (context) => {
  ...
  if (req?.oneworksLogContext) {
    return req.oneworksLogContext
  }
  ...
  if (resolved != null && req != null) {
    req.oneworksLogContext = resolved
...
const resolveFallbackRequestLogPath = (fileName) => {
  const workspace = process.env.__ONEWORKS_PROJECT_WORKSPACE_FOLDER__ || process.cwd()
  return path.join(
    resolveProjectOoPath(workspace, process.env, 'logs', 'adapter-claude-code'),
    fileName || 'temp.log.md'
  )
}
...
const resolveRequestLogPath = (fileName, context, request) => {
  ...
  const workspace = process.env.__ONEWORKS_PROJECT_WORKSPACE_FOLDER__
  const ctxId = logContext?.ctxId ?? process.env.__ONEWORKS_PROJECT_CTX_ID__
  const sessionId = logContext?.sessionId ?? process.env.__ONEWORKS_PROJECT_SESSION_ID__
  ...
  if (
    ...
    return resolveFallbackRequestLogPath(fileName)
  }

  return path.join(
    resolveProjectOoPath(workspace, process.env, 'logs', ctxId, sessionId, 'adapter-claude-code'),
    fileName
  )
}
...
export const writeRequestDebugLog = (fileName, message, data = null, context, request) => {
  try {
    ...
    fs.appendFileSync(
      resolveFallbackRequestLogPath('temp.log.md'),
