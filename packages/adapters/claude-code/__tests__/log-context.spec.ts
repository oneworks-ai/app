...
vi.mock('@oneworks/utils', async () => await import('../../../utils/src/index'))

const { resolveProjectOoPath } = await import('@oneworks/utils')
...
const createWorkspace = () => {
  const cwd = mkdtempSync(join(tmpdir(), 'ow-claude-log-context-'))
  createdDirs.add(cwd)
  return cwd
}

const writeRequestLogContext = (workspace: string, payload: { ctxId: string; sessionId: string }) => {
  const filePath = resolveProjectOoPath(
    workspace,
    process.env,
    '.mock',
    '.claude-code-router',
