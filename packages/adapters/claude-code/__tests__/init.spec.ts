...
describe('initClaudeCodeAdapter', () => {
  ...
  it('removes stale keychains symlinks when the real home is unavailable', async () => {
    ...
    await rm(join(realHome, 'Library', 'Keychains'), { recursive: true, force: true })
  ...
  it('syncs plugin-provided skills from resolved workspace assets into the isolated Claude home', async () => {
    ...
    const realHome = await createWorkspace()
    const mockHome = resolveTestMockHome(workspace, realHome)
    const pluginSkillDir = join(workspace, 'vendor', 'oneworks-cli-skills', 'skills', 'oneworks-cli-quickstart')

    await mkdir(pluginSkillDir, { recursive: true })
    await writeFile(join(pluginSkillDir, 'SKILL.md'), '# oneworks-cli-quickstart\n')
    ...
    await initClaudeCodeAdapter({
      ...
      assets: {
        ...
        skills: [
          {
            id: 'skill:plugin:0:oneworks-cli-quickstart',
            kind: 'skill',
            name: 'oneworks-cli-quickstart',
            displayName: 'oneworks-cli-quickstart',
            origin: 'plugin',
            sourcePath: join(pluginSkillDir, 'SKILL.md'),
            payload: {
              definition: {
                path: join(pluginSkillDir, 'SKILL.md'),
                body: '# oneworks-cli-quickstart\n',
                attributes: {}
              }
            }
          }
    ...
    const targetPath = join(mockHome, '.claude', 'skills', 'oneworks-cli-quickstart')
    expect((await lstat(targetPath)).isSymbolicLink()).toBe(true)
