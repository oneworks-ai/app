...
export const ensureGeminiPromptFiles = async (
  ...
  return {
    ...
    generatedContextFileName: generatedContextFilePath
  }
}
...
export const buildGeminiSpawnEnv = (params: {
  ...
  const mockHome = resolveMockHome(ctx.cwd, ctx.env)
  const nativeHooksActive = ctx.env.__ONEWORKS_PROJECT_GEMINI_NATIVE_HOOKS_AVAILABLE__ === '1'
  ...
  return {
    ...toProcessEnv({
      ...
      __ONEWORKS_GEMINI_HOOK_MODEL__: nativeHooksActive ? model : undefined,
      __ONEWORKS_GEMINI_HOOK_RUNTIME__: nativeHooksActive ? runtime : undefined,
      __ONEWORKS_GEMINI_TASK_SESSION_ID__: nativeHooksActive ? sessionId : undefined,
      __ONEWORKS_GEMINI_HOOKS_ACTIVE__: nativeHooksActive ? '1' : undefined,
