...
export const buildGeminiNativeHooksSettings = (
  ...
  const command = env.__ONEWORKS_PROJECT_GEMINI_HOOK_COMMAND__?.trim()
  if (env.__ONEWORKS_PROJECT_GEMINI_NATIVE_HOOKS_AVAILABLE__ !== '1' || command == null || command === '') {
    return {}
  }
...
export const prepareGeminiNativeHooks = (
  ...
  env.__ONEWORKS_PROJECT_GEMINI_NATIVE_HOOKS_AVAILABLE__ = '0'
  ...
  try {
    ...
    env.__ONEWORKS_PROJECT_GEMINI_NATIVE_HOOKS_AVAILABLE__ = enabled ? '1' : '0'
    env.__ONEWORKS_PROJECT_GEMINI_HOOK_COMMAND__ = buildNodeScriptCommand({
      nodePath,
    ...
    env.__ONEWORKS_PROJECT_GEMINI_NATIVE_HOOKS_AVAILABLE__ = '0'
    delete env.__ONEWORKS_PROJECT_GEMINI_HOOK_COMMAND__
    return false
  }
}
