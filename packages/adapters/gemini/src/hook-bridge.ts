...
const runtime = process.env.__ONEWORKS_GEMINI_HOOK_RUNTIME__ as AdapterQueryOptions['runtime'] | undefined
const taskSessionId = process.env.__ONEWORKS_GEMINI_TASK_SESSION_ID__?.trim()
const taskModel = process.env.__ONEWORKS_GEMINI_HOOK_MODEL__?.trim()
...
export const isGeminiNativeHookEnv = () => (
  process.env.__ONEWORKS_GEMINI_HOOKS_ACTIVE__ === '1'
)

export const supportsHookEvent = (_eventName: keyof HookInputs) => false

export const mapGeminiHookInputToOneWorks = (input: NativeGeminiHookInput): HookInput => {
  const base = {
    cwd: input.cwd,
...
export const mapOneWorksHookOutputToGemini = (
  ...
  switch (eventName) {
    case 'BeforeTool': {
      ...
      if (hookSpecificOutput?.hookEventName === 'PreToolUse') {
        ...
        result.reason = blockReason(output.stopReason, 'blocked by One Works BeforeTool hook')
      }
      return result
    }
    case 'AfterTool': {
    ...
    case 'BeforeAgent': {
      ...
      if (output.continue === false) {
        result.decision = 'deny'
        result.reason = blockReason(output.stopReason, 'blocked by One Works BeforeAgent hook')
      }
      return result
    }
    case 'AfterAgent': {
      ...
      if (output.continue === false && result.systemMessage == null) {
        result.systemMessage = blockReason(output.stopReason, 'One Works AfterAgent hook requested a stop')
      }
      return result
    }
    case 'SessionStart': {
...
export const runGeminiHookBridge = async () => {
  try {
    ...
    const hookInput = mapGeminiHookInputToOneWorks(input)
    const result = await executeHookInput(hookInput)
    process.stdout.write(`${JSON.stringify(mapOneWorksHookOutputToGemini(input.hookEventName, result))}\n`)
  } catch (error) {
    process.stdout.write(`${
      JSON.stringify({
        continue: true,
        systemMessage: `oneworks gemini hook bridge error: ${String(error)}`
      })
    }\n`)
  }
}
