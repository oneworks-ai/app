  ...
  return {
    ...
    cache: {
      ...
      set: async <K extends keyof Cache>(key: K, value: Cache[K]) => {
        ...
        return { cachePath: join(cwd, '.oo', 'caches', `${key}.json`) }
      }
    },
    logger: {
...
describe('createGeminiSession', () => {
  ...
  it('emits stop and exit events in stream mode', async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'ow-gemini-session-'))
    ...
    const session = await createGeminiSession(
      createCtx(tmpDir, {
        __ONEWORKS_PROJECT_ADAPTER_GEMINI_CLI_PATH__: fakeGeminiPath
      }),
      {
        type: 'create',
        runtime: 'cli',
  ...
  it('starts the native CLI in direct mode and caches the native Gemini session id', async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'ow-gemini-direct-'))
