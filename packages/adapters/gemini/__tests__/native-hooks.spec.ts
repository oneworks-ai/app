...
const createCtx = (env: Record<string, string | undefined> = {}): AdapterCtx =>
  ({
    ...
    cache: {
      get: async () => undefined,
      set: async () => ({ cachePath: '/tmp/project/.oo/cache/base.json' })
    },
    logger: {
...
describe('gemini native hook bridge', () => {
  it('prepares the managed hook runtime and marks native hooks available', () => {
    ...
    expect(ctx.env.__ONEWORKS_PROJECT_GEMINI_NATIVE_HOOKS_AVAILABLE__).toBe('1')
    expect(ctx.env.__ONEWORKS_PROJECT_GEMINI_HOOK_COMMAND__).toContain('call-hook.js')
