...
const createCtx = (config?: Config, userConfig?: Config, env: Record<string, string | undefined> = {}) =>
  ({
    ...
    cache: {
      get: async () => undefined,
      set: async () => ({ cachePath: '/tmp/project/.oo/cache/test.json' })
    },
    logger: {
      stream: undefined,
      info: () => undefined,
...
describe('resolveGeminiBinaryPath', () => {
  it('returns the env-specified path when set', () => {
    expect(resolveGeminiBinaryPath({
      __ONEWORKS_PROJECT_ADAPTER_GEMINI_CLI_PATH__: '/usr/local/bin/gemini'
    })).toBe('/usr/local/bin/gemini')
  })

  it('uses a managed binary from the global bootstrap cache', async () => {
    const home = await mkdtemp(join(tmpdir(), 'ow-gemini-home-'))
    const worktree = await mkdtemp(join(tmpdir(), 'ow-gemini-worktree-'))
    try {
      const env = {
...
describe('gemini prompt and settings helpers', () => {
  ...
  it('builds isolated Gemini settings with native context files and MCP config', () => {
    expect(buildGeminiSettings({
      adapterConfig: {},
      approvalMode: 'default',
      generatedContextFileName: '/tmp/project-home/.mock/.gemini-adapter/session-1/ONEWORKS.md',
      mcpServers: {
        docs: {
          command: 'node',
      ...
      context: {
        fileName: ['GEMINI.md', '/tmp/project-home/.mock/.gemini-adapter/session-1/ONEWORKS.md']
      },
      mcpServers: {
        docs: {
          command: 'node',
  ...
  it('writes native hooks into managed Gemini settings when enabled', () => {
    const nativeHooks = buildGeminiNativeHooksSettings({
      __ONEWORKS_PROJECT_GEMINI_NATIVE_HOOKS_AVAILABLE__: '1',
      __ONEWORKS_PROJECT_GEMINI_HOOK_COMMAND__: `"${process.execPath}" "/tmp/call-hook.js"`
    })
  ...
  it('passes native hook bridge markers to Gemini child processes when enabled', () => {
    ...
    expect(env.__ONEWORKS_GEMINI_HOOKS_ACTIVE__).toBe('1')
    expect(env.__ONEWORKS_GEMINI_HOOK_MODEL__).toBe('kimi-k2.5')
    expect(env.__ONEWORKS_GEMINI_HOOK_RUNTIME__).toBe('cli')
    expect(env.__ONEWORKS_GEMINI_TASK_SESSION_ID__).toBe('session-gemini-native')
...
describe('validateGeminiSelection', () => {
  it('accepts chat-completions model services and resolves them for Gemini CLI', () => {
    ...
    expect(resolveGeminiModel({
      ...
      routedService: {
        serviceKey: 'kimi',
        endpoint: 'https://api.moonshot.oo/v1/chat/completions',
        model: 'kimi-k2.5'
      }
    })
  })
