...
describe('handleIncomingNotification', () => {
  describe('thread/name/updated', () => {
    it('emits a standardized session update for Codex thread titles', () => {
      const { events } = dispatch('thread/name/updated', {
        threadId: 'thr_1',
        threadName: '  Implement title sync  '
      })

      expect(events).toEqual([{
        type: 'session_update',
        data: { title: 'Implement title sync' }
      }])
    })

    it('ignores empty thread titles', () => {
      const { events } = dispatch('thread/name/updated', {
        threadId: 'thr_1',
        threadName: '   '
      })
  ...
  describe('item/started – commandExecution', () => {
    ...
    it('emits a context compaction event for compact items', () => {
      const { events } = dispatch('item/started', {
        item: { type: 'contextCompaction', id: 'cc1', trigger: 'auto', tokenCount: 1234 }
      })
  ...
  describe('turn/completed', () => {
    ...
    it('emits fatal error and stop when status is interrupted', () => {
      const { events } = dispatch('turn/completed', {
      ...
      if (ev.type === 'stop') {
        expect(ev.data?.content).toBe('Turn interrupted')
      }
    })
    ...
    it('formats structured codex turn failure details', () => {
      const { events } = dispatch('turn/completed', {
        turn: {
          id: 'turn_5',
          status: 'failed',
      ...
      expect(ev.type).toBe('stop')
      if (ev.type === 'stop') {
        expect(ev.data?.content).toContain('exceeded retry limit')
        expect(ev.data?.content).toContain('"httpStatusCode":429')
      }
    })
