...
import { mapCodexHookInputToOneWorks, mapOneWorksHookOutputToCodex } from '../src/hook-bridge'
...
describe('codex native hook bridge helpers', () => {
  ...
  it('keeps Stop continue semantics instead of converting them into continuation prompts', () => {
    const result = mapOneWorksHookOutputToCodex('Stop', {
      continue: false,
  ...
  it('passes UserPromptSubmit additional context through the documented hook-specific output', () => {
    const result = mapOneWorksHookOutputToCodex('UserPromptSubmit', {
      hookSpecificOutput: {
