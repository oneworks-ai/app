...
describe('resolveCodexBinaryPath', () => {
  ...
  it('returns the env-specified path when set', () => {
    expect(resolveCodexBinaryPath({
      __ONEWORKS_PROJECT_ADAPTER_CODEX_CLI_PATH__: '/usr/local/bin/codex'
    })).toBe('/usr/local/bin/codex')
  })

  it('falls back to the default binary when env value is empty string', () => {
    const result = resolveCodexBinaryPath({
      __ONEWORKS_PROJECT_ADAPTER_CODEX_CLI_PATH__: ''
    })
    expectDefaultCodexBinary(result)
  })

  it('uses a managed binary from the global bootstrap cache', async () => {
    const home = await mkdtemp(join(tmpdir(), 'ow-codex-home-'))
    const worktree = await mkdtemp(join(tmpdir(), 'ow-codex-worktree-'))
    try {
      const env = {
