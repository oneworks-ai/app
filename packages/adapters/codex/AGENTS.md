...
- `src/runtime/accounts.ts`
  ...
  - tracks each managed inline credential lineage in durable private adapter state, materializes one device-global `0600` cache owner, and symlinks session homes to it; every live probe instead receives an isolated per-probe `0600` copy and may merge it into the owner only after both official reads succeed, while session/app-server teardown flushes complete owner rotations through the same generation / credential-revision CAS
  ...
  - queries Codex account info and rate-limit/quota snapshots through `codex app-server`; chat account selectors consume cached snapshots and never start this independent live probe beside conversation startup
...
如果 global config 里已经有同一 stable identity 的 inline account，本机运行时会保留它的 key、标题和默认账号语义，但优先软链当前 `~/.codex/auth.json`；stable identity 必须由两侧相同且非空的 `accountId` 建立，允许一侧缺少 organization 元数据，但两侧显式 organization 冲突时仍拒绝合并。email / organization title 只能用于显示，不能单独触发 real-home 采用、descriptor 去重、preferred key 或登录 key 复用；完全相同的 auth digest 仍是独立的字节等价规则。新增登录若生成的友好 key 已被占用，canonical config 更新锁内必须只读取一次当前可用 credential source，并从同一份字节导出 digest 与 stable identity；incoming identity 同样以最终 auth bytes 为权威，字节已有非空 accountId 时 probe / live metadata 只能补充显示信息，不能覆盖 collision identity。存在可用源时不能用旧 `authDigest` / identity metadata 建立等价，若不匹配则分配 digest-disambiguated key，不能覆盖占用者。这样 Codex 或 ChatGPT 轮换 refresh token 后不会继续重放旧 inline token，同时也不会把真实 home 的新凭据自动写回 global config。

direct mode 的 HOME 按 session 隔离；standalone stream fallback 按 project、账号与启动 / 进程 / 网络 profile 隔离并复用 app-server HOME；manager-owned stream 则在账号、binary / startup 与有效进程 / 网络 profile 相同时跨 workspace 复用机器共享 HOME 和 app-server。model provider、MCP、cwd、权限、One Works workspace / session 运行时元数据与 selected skills 都下沉到 thread，不构成独立 app-server；其他进程级环境仍参与 profile。账号只来自 global config 时，adapter 会为 generation / credential revision / inline source digest 建立设备级私有 auth lineage，让 direct、standalone、manager、model-sharing HOME 软链同一 owner；live probe 使用独立的一次性 `0600` 副本，不能直接改写共享 owner。每个 lifecycle callback 绑定实际 `HOME/.codex/auth.json` 和启动 digest：官方 Codex 原地 truncate / write 会直接更新 owner；若改为完整 JSON 的 atomic rename，则在私有 owner-state lock 内先精确 claim regular-file 候选，通过 no-follow fd 对同一节点完成 fstat / 读取 / 校验，再用 owner 目录内的原子 writer 合入 owner，避免跨卷 rename，并走现有 canonical config lock 与 generation / revision / source CAS。候选必须是完整 JSON、同一 accountId 且符合 optional-org 语义；在 owner-state 与 canonical CAS 可恢复前保持 claimed 文件，重建 symlink 使用不覆盖写入，新候选出现时保留并重试。symlink 快路径只从锁定 owner 读取启动 digest，并在返回前再次确认 auth path 仍解析到同一 owner；若同一 session HOME 仍指向 real-home、显式 authFile 或旧 lineage owner，则先按 dev / ino 原子 claim 并复核旧 symlink，再以不覆盖方式安装当前 owner，遇到并发替换会保留并重试。adapter 以独立于 disposable cache 的 `0600` 状态记录已初始化 lineage，并在 session / app-server 生命周期结束时 flush 当前 lifecycle 相对启动 digest 产生的完整变化；若 stream / shared-model 在拿到 lease 前失败，则 acquire 已完成子进程终结后仍必须只 flush 一次，并保留原始 acquisition error。probe 只有在 `account/read` 与 `account/rateLimits/read` 均成功后才把隔离候选合入 owner 与 portable config；失败 probe 的副本会被清理，不能污染已运行或后来启动的 lifecycle。成功写回的新 revision 留在原 lineage，所以旧内存快照不会重置 owner 或在下一次收集时派生第二个 writer；外部新 generation / revision / source 才切换 owner，且即使旧 owner 正处于不完整写入也能建立新的确定性 owner；同 lineage 的不完整 owner 和整个 owner cache 丢失仍 fail closed。账号来自 `authFile`、real home fallback，或命中同身份本机凭据覆盖时，session 仍直接 symlink 到对应文件且从不参与写回；probe 同样只使用隔离副本。普通 stream 会在单个 One Works runtime 内按 profile 复用 app-server；共享 pool、登录和 probe 子进程只有在 `pid` 缺失的真实 spawn failure 上立即终结，post-spawn error / abort / 成功清理都会先请求 TERM、必要时有界 KILL，并等待实际 exit / close 后才释放 lifecycle 或删除隔离 HOME；pool / broker disposal 也必须 await 这份真实终态，probe source 在 HOME 创建后、cleanup ownership 交给调用方前失败则由 source helper 自行删除 HOME。聊天账号 hook 只读取缓存快照，不在页面挂载 / 会话启动时另起 live quota probe。设置、额度详情与用户显式刷新仍执行 live quota probe，但普通 `account/read` 不显式请求 OAuth rotation；reset-credit 在 auth source 物化前捕获 account key、canonical-backed provenance、canonical 存在 / 删除 / tombstone、source kind、解析后的实际路径及存在性 / 内容 digest / stable identity、inline digest、generation 与 credential revision，并在 consume 前持有 canonical lock 重新解析和复核同一 effective runtime source；物化 helper 还返回实际写入隔离 HOME 的字节 digest / identity，consume 必须同时匹配这份 artifact，关闭 `A → B(copy) → A` 窗口。canonical-backed `authFile` 删除、墓碑或 generation 连续性中断一律拒绝，只有明确的 project-only `authFile` 可在 global account 缺失时继续。登录与 reset-credit 链路保留各自的显式刷新。官方 Codex 的跨进程“同时在任一写回前发起 refresh”窗口仍由上游负责，不能把共享文件或 preparation lock 描述为跨进程 OAuth mutex。
...
- live probe 如果因官方 CLI 的主动 `auth()` 检查而轮换 global inline account 的 `auth.json`，必须在同一验证结果成功后以 canonical config lock 原子写回；普通账号 / quota refresh 不给 `account/read` 发送 `refreshToken: true`，验证失败保留旧凭据
