...
const runtime = process.env.__ONEWORKS_CODEX_HOOK_RUNTIME__ as AdapterQueryOptions['runtime'] | undefined
const taskSessionId = process.env.__ONEWORKS_CODEX_TASK_SESSION_ID__?.trim()
...
export const isCodexNativeHookEnv = () => (
  process.env.__ONEWORKS_CODEX_HOOKS_ACTIVE__ === '1'
)
...
export const mapOneWorksHookOutputToCodex = (
  ...
  switch (eventName) {
    case 'PreToolUse': {
      ...
      if (hookSpecificOutput?.hookEventName === 'PreToolUse') {
        ...
        const reason = blockReason(output.stopReason, 'blocked by One Works PreToolUse hook')
        result.decision = 'block'
    ...
    case 'UserPromptSubmit': {
      ...
      if (output.continue === false) {
        result.decision = 'block'
        result.reason = blockReason(output.stopReason, 'blocked by One Works UserPromptSubmit hook')
      }
      return result
    }
    case 'Stop': {
...
export const runCodexHookBridge = async () => {
  try {
    ...
    process.stdout.write(`${
      JSON.stringify({
        continue: true,
        systemMessage: `oneworks codex hook bridge error: ${String(error)}`
      })
    }\n`)
  }
}
