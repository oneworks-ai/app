...
const MANAGED_PROJECT_BLOCK_END = '# END ONE WORKS MANAGED CODEX PROJECT CONFIG'
const LEGACY_MANAGED_CONFIG_BLOCK_START = '# BEGIN ONE WORKS MANAGED CODEX CONFIG'
const LEGACY_MANAGED_CONFIG_BLOCK_END = '# END ONE WORKS MANAGED CODEX CONFIG'
const MANAGED_HOOK_TRUST_BLOCK_START = '# BEGIN ONE WORKS MANAGED CODEX HOOK TRUST'
const MANAGED_HOOK_TRUST_BLOCK_COMMENT =
  '# This block is managed by One Works to satisfy Codex native hook trust checks.'
const MANAGED_HOOK_TRUST_BLOCK_END = '# END ONE WORKS MANAGED CODEX HOOK TRUST'
...
const MANAGED_PROJECT_BLOCK_COMMENT = '# This project block is managed by One Works.'
...
const isManagedRuntimeMarkerLine = (line: string) => {
  const trimmedLine = line.trim()
  return isManagedProjectMarkerLine(line) ||
    trimmedLine === MANAGED_HOOK_TRUST_BLOCK_START ||
    trimmedLine === MANAGED_HOOK_TRUST_BLOCK_COMMENT ||
    trimmedLine === MANAGED_HOOK_TRUST_BLOCK_END
}
...
const getManagedOnlySectionBodyLines = (section: TomlSection, lines: TomlLine[]) =>
  trimBlankLines(
    getSectionLineEntries(section, lines)
      .slice(1)
      .filter(line => !(line.stringStateBefore === 'none' && isManagedRuntimeMarkerLine(line.text)))
      .map(line => line.text)
  )

const isManagedOnlyProjectTrustSection = (section: TomlSection, lines: TomlLine[]) => {
  const headerKey = getTomlHeaderKey(section.header)
  if (headerKey?.startsWith('projects.') !== true) return false

  const bodyLines = getManagedOnlySectionBodyLines(section, lines)
  return bodyLines.length > 0 &&
    bodyLines.every(line => /^\s*trust_level\s*=\s*"trusted"\s*(?:#.*)?$/.test(line))
}

const isManagedOnlyHookTrustSection = (section: TomlSection, lines: TomlLine[]) => {
  const headerKey = getTomlHeaderKey(section.header)
  if (headerKey?.startsWith('hooks.state.') !== true) return false

  const bodyLines = getManagedOnlySectionBodyLines(section, lines)
  return bodyLines.length > 0 &&
    bodyLines.every(line => /^\s*trusted_hash\s*=\s*"sha256:[^"]+"\s*(?:#.*)?$/.test(line))
}

const stripManagedRuntimeSections = (content: string) => {
  const scan = scanTomlSections(content)
  const skippedLineNumbers = new Set<number>()
  for (const section of scan.sections) {
    if (!isManagedOnlyProjectTrustSection(section, scan.lines) && !isManagedOnlyHookTrustSection(section, scan.lines)) {
      continue
    }
    for (let lineIndex = section.startLine; lineIndex < section.endLine; lineIndex += 1) {
      skippedLineNumbers.add(lineIndex)
  ...
  return scan.lines
    .filter((line, lineIndex) =>
      !skippedLineNumbers.has(lineIndex) &&
      !(line.stringStateBefore === 'none' && isManagedRuntimeMarkerLine(line.text))
    )
    .map(line => line.text)
    .join('\n')
}

export const isManagedOnlyCodexConfigContent = (params: {
  content: string
}) => {
  const strippedManagedContent = normalizeTomlContent(params.content)
    .replace(MANAGED_ROOT_BLOCK_PATTERN, '')
    .replace(LEGACY_MANAGED_CONFIG_BLOCK_PATTERN, '')
  const strippedWorkspaceProjectContent = stripManagedRuntimeSections(strippedManagedContent)

  return strippedWorkspaceProjectContent.trim() === ''
}

export const resolveInitialManagedCodexConfigContent = (params: {
  currentContent: string | undefined
  inheritedContent: string | undefined
}) => {
  if (params.currentContent == null) return params.inheritedContent
  if (params.inheritedContent == null || params.currentContent === params.inheritedContent) {
    return params.currentContent
  }

  const shouldInheritConfig = isManagedOnlyCodexConfigContent({
    content: params.currentContent
  })

  return shouldInheritConfig ? params.inheritedContent : params.currentContent
}
