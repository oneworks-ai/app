...
const NODE_PATH = process.env.__ONEWORKS_PROJECT_NODE_PATH__ || process.execPath
const CALL_HOOK_PATH = process.env.__ONEWORKS_KIMI_CALL_HOOK_PATH__ || require.resolve('@oneworks/hooks/call-hook.js')
const SESSION_ID = process.env.__ONEWORKS_KIMI_TASK_SESSION_ID__
const RUNTIME = process.env.__ONEWORKS_KIMI_HOOK_RUNTIME__
const MODEL = process.env.__ONEWORKS_KIMI_HOOK_MODEL__
...
const normalizePayload = (raw) => {
  ...
  const cwd = firstString(raw.cwd, process.env.__ONEWORKS_PROJECT_WORKSPACE_FOLDER__, process.cwd()) || process.cwd()
  const payload = {
    ...raw,
    cwd,
...
const runVibeHook = (payload) => {
  const result = spawnSync(NODE_PATH, [CALL_HOOK_PATH], {
    ...
    env: {
      ...process.env,
      __ONEWORKS_PROJECT_WORKSPACE_FOLDER__: payload.cwd || process.env.__ONEWORKS_PROJECT_WORKSPACE_FOLDER__ ||
        process.cwd()
    }
  })

  if ((result.status ?? 0) !== 0) {
    console.error('[oneworks kimi hook] call-hook exited with', result.status, result.stderr)
...
main().catch((error) => {
  console.error('[oneworks kimi hook] failed', error)
  process.exit(0)
})
