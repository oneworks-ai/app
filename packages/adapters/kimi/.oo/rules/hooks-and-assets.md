...
`PreToolUse` 和 `PostToolUse` matcher 当前写 `.*`，由 One Works hook runtime 继续做权限、插件和内置 permission 逻辑。
`PreCompact` 当前会继续透传 Kimi 原生 payload 里的 `trigger` / `token_count`，并保持可阻断语义。

托管配置会写入 generated JSON config，或在复制到的 TOML config 中插入：

```text
# One Works managed Kimi hooks start
...
# One Works managed Kimi hooks end
```
...
- stdin 读取 Kimi hook payload，转换成 One Works hook input。
...
- `Stop` 当前标记为不可阻断；需要阻断 Stop 时先确认官方语义和 One Works hook contract。
...
```text
.oo/caches/<ctxId>/<sessionId>/adapter-kimi/skills
```
