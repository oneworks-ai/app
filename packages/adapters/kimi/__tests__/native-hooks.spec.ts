...
const createCtx = async (overrides: Partial<AdapterCtx> = {}): Promise<AdapterCtx> => {
  const cwd = overrides.cwd ?? await createTempDir('ow-kimi-hooks-')
  return {
...
describe('kimi native hook bridge', () => {
  it('prepares the managed hook runtime and marks native hooks available', async () => {
    ...
    expect(ctx.env.__ONEWORKS_PROJECT_KIMI_NATIVE_HOOKS_AVAILABLE__).toBe('1')
    expect(ctx.env.__ONEWORKS_PROJECT_KIMI_HOOK_COMMAND__).toContain('kimi-hook.js')
    expect(ctx.env.__ONEWORKS_PROJECT_NODE_PATH__).toBe(process.execPath)
    expect(ctx.env.__ONEWORKS_PROJECT_WORKSPACE_FOLDER__).toBe(ctx.cwd)
  })
  ...
  it('writes native hooks into generated session config', async () => {
    ...
    expect(base.spawnEnv.__ONEWORKS_KIMI_TASK_SESSION_ID__).toBe('session-kimi-hooks')
    expect(base.spawnEnv.__ONEWORKS_KIMI_HOOK_RUNTIME__).toBe('cli')
    expect(base.spawnEnv.__ONEWORKS_KIMI_HOOK_MODEL__).toBe('gateway,gpt-5')
  })

  it.skipIf(process.platform === 'win32')(
    'adapts Kimi hook stdin and blocks when Vibe hook output blocks',
    async () => {
      const workspace = await createTempDir('ow-kimi-hook-wrapper-')
...
`
      ...
      const result = spawnSync(
        ...
        [resolve(process.cwd(), 'packages/adapters/kimi/kimi-hook.js')],
        {
          ...
          env: {
            ...process.env,
            __ONEWORKS_KIMI_CALL_HOOK_PATH__: fakeCallHookPath,
            __ONEWORKS_KIMI_TASK_SESSION_ID__: 'ow-session',
            __ONEWORKS_KIMI_HOOK_RUNTIME__: 'cli',
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const workspace = await createTempDir('ow-kimi-hook-precompact-')
      const fakeCallHookPath = join(workspace, 'fake-call-hook.js')
      const capturedInputPath = join(workspace, 'captured.json')
      await mkdir(workspace, { recursive: true })
      await writeFile(
