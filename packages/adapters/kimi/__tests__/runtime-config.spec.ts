...
import { resolveKimiBinaryPath, resolveKimiLegacyManagedToolPaths, resolveKimiManagedToolPaths } from '../src/paths'
...
const createCtx = async () => {
  const cwd = await mkdtemp(join(tmpdir(), 'ow-kimi-test-'))
  const ctx: AdapterCtx = {
    ctxId: 'ctx-kimi',
    cwd,
    env: {
      __ONEWORKS_PROJECT_REAL_HOME__: join(cwd, 'home'),
...
describe('kimi runtime helpers', () => {
  ...
  it('resolves a managed Kimi CLI binary before falling back to PATH', async () => {
    ...
    try {
      ...
      expect(resolveKimiBinaryPath({
        __ONEWORKS_PROJECT_ADAPTER_KIMI_CLI_PATH__: '/usr/local/bin/kimi'
  ...
  it('uses the global bootstrap cache for managed Kimi CLI paths', async () => {
    const { ctx, cleanup } = await createCtx()
    try {
      ...
      expect(paths.rootDir).toBe(
        join(ctx.env.__ONEWORKS_PROJECT_REAL_HOME__!, '.oneworks', 'bootstrap', 'uv', 'kimi-cli-1.36.0', 'python-3.13')
      )
  ...
  it('builds global-cache uv tool install options for managed Kimi CLI installs', async () => {
    const { ctx, cleanup } = await createCtx()
    try {
      const options = resolveKimiCliInstallOptions({
        __ONEWORKS_PROJECT_ADAPTER_KIMI_AUTO_INSTALL__: 'false',
        __ONEWORKS_PROJECT_ADAPTER_KIMI_INSTALL_PACKAGE__: 'kimi-cli==1.2.3',
        __ONEWORKS_PROJECT_ADAPTER_KIMI_INSTALL_PYTHON__: '3.14',
        __ONEWORKS_PROJECT_ADAPTER_KIMI_UV_PATH__: '/opt/bin/uv'
      }, {
        autoInstall: true,
      ...
      const installEnv = buildKimiCliInstallEnv(ctx, options)
  ...
  it('renders actionable install instructions when Kimi CLI or uv is missing', async () => {
    ...
    try {
      ...
      expect(instructions).toContain('__ONEWORKS_PROJECT_ADAPTER_KIMI_CLI_PATH=/absolute/path/to/kimi')
      expect(instructions).toContain(resolveKimiManagedToolPaths(ctx.cwd, ctx.env, options).binDir)
    } finally {
      await cleanup()
    }
  })
  ...
  it('throws actionable install instructions when Kimi CLI is missing and auto-install is disabled', async () => {
    ...
    try {
      ...
      ctx.env.__ONEWORKS_PROJECT_ADAPTER_KIMI_AUTO_INSTALL__ = 'false'
  ...
  it.skipIf(process.platform === 'win32')(
    'moves an existing legacy managed Kimi CLI into the global cache',
    async () => {
      const { ctx, cleanup } = await createCtx()
      try {
        const legacyPaths = resolveKimiLegacyManagedToolPaths(ctx.cwd, ctx.env)
        const managedPaths = resolveKimiManagedToolPaths(ctx.cwd, ctx.env)
        const legacyBinaryPath = legacyPaths.binaryCandidates[0]
        const managedBinaryPath = managedPaths.binaryCandidates[0]
        ...
        await mkdir(legacyPaths.binDir, { recursive: true })
        await writeFile(
          legacyBinaryPath,
          `#!/bin/sh
if [ "$1" = "--version" ]; then
...
`
        ...
        await chmod(legacyBinaryPath, 0o755)
        ctx.env.__ONEWORKS_PROJECT_ADAPTER_KIMI_AUTO_INSTALL__ = 'false'

        const binaryPath = await ensureKimiCli(ctx)

        expect(binaryPath).toContain('/.oneworks/bootstrap/uv/kimi-cli-1.36.0/python-3.13/bin/kimi')
        await expect(access(managedBinaryPath)).resolves.toBeUndefined()
        await expect(access(legacyBinaryPath)).rejects.toThrow()
      } finally {
        await cleanup()
      }
    }
  ...
  it.skipIf(process.platform === 'win32')('falls back to PATH when a managed Kimi binary is stale', async () => {
    ...
    try {
      ...
      expect(ctx.env.__ONEWORKS_PROJECT_ADAPTER_KIMI_CLI_PATH__).toBe('kimi')
    } finally {
      await cleanup()
    }
  })
...
`
        ...
        ctx.env.PATH = `${fakeBinDir}${delimiter}${process.env.PATH ?? ''}`
        ctx.env.__ONEWORKS_PROJECT_ADAPTER_KIMI_UV_PATH__ = fakeUvPath

        const binaryPath = await ensureKimiCli(ctx)

        expect(binaryPath).not.toBe('kimi')
        expect(binaryPath).toContain('/.oneworks/bootstrap/uv/kimi-cli-1.36.0/python-3.13/bin/kimi')
        expect(ctx.env.__ONEWORKS_PROJECT_ADAPTER_KIMI_CLI_PATH__).toBe(binaryPath)
      } finally {
        await cleanup()
      }
    }
  ...
  it('builds routed config for Kimi wire sessions', async () => {
    const { ctx, cleanup } = await createCtx()
    try {
  ...
  it('does not backfill legacy workspace Kimi share sessions', async () => {
    ...
    try {
      ...
      await expect(readFile(join(base.shareDir, 'sessions', 'kimi-session.json'), 'utf8')).rejects.toThrow()
      await expect(readFile(join(base.shareDir, 'sessions', 'gemini-session.json'), 'utf8')).rejects.toThrow()
      await expect(readFile(join(legacySessionsDir, 'kimi-session.json'), 'utf8')).resolves.toContain('kimi-native')
    } finally {
      await cleanup()
    }
  })

  it('does not migrate Kimi share sessions when provenance mismatches', async () => {
    const { ctx, cleanup } = await createCtx()
    try {
      const sessionId = 'session-kimi-provenance-mismatch'
      const oldShareDir = join(
        ctx.cwd,
        '.oo',
        'caches',
        'old-ctx',
      ...
      const oldSessionsDir = join(oldShareDir, 'sessions')
      await mkdir(oldSessionsDir, { recursive: true })
      await writeFile(join(oldSessionsDir, 'wrong-session.json'), '{"id":"wrong"}\n', 'utf8')
      await writeFile(
        join(oldShareDir, '.oneworks-session.json'),
        JSON.stringify({
          version: 1,
          fingerprint: 'different-session-shape',
          summary: {
            agent: 'default',
            config: 'none',
            cwd: ctx.cwd,
            realShareDir: '/tmp/other-kimi'
          }
        }),
        'utf8'
      )
      ...
      await expect(readFile(join(base.shareDir, 'sessions', 'wrong-session.json'), 'utf8')).rejects.toThrow()
    } finally {
      await cleanup()
    }
  })

  it('does not guess between multiple provenance-less legacy Kimi sessions', async () => {
    const { ctx, cleanup } = await createCtx()
    try {
      const sessionId = 'session-kimi-ambiguous-legacy'
      for (const ctxId of ['old-ctx-a', 'old-ctx-b']) {
        const oldSessionsDir = join(
          ctx.cwd,
          '.oo',
          'caches',
          ctxId,
        ...
        await mkdir(oldSessionsDir, { recursive: true })
        await writeFile(join(oldSessionsDir, `${ctxId}.json`), `{"id":"${ctxId}"}\n`, 'utf8')
      }
      ...
      await expect(readFile(join(base.shareDir, 'sessions', 'old-ctx-a.json'), 'utf8')).rejects.toThrow()
      await expect(readFile(join(base.shareDir, 'sessions', 'old-ctx-b.json'), 'utf8')).rejects.toThrow()
    } finally {
      await cleanup()
    }
  })
  ...
  it('normalizes Moonshot chat completions URLs for Kimi providers', async () => {
    ...
    try {
      ...
      expect(config.providers?.kimi).toMatchObject({
        type: 'kimi',
        base_url: 'https://api.moonshot.oo/v1'
      })
      expect(config.services?.moonshot_search).toMatchObject({
        base_url: 'https://api.moonshot.oo/v1/search'
      })
