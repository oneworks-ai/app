...
import { buildNodeScriptCommand, hasManagedHookPlugins, prepareManagedHookRuntime } from '@oneworks/hooks'
import type { AdapterCtx } from '@oneworks/types'

const require = createRequire(import.meta.url ?? __filename)
const adapterPackageDir = dirname(require.resolve('@oneworks/adapter-kimi/package.json'))
...
const MANAGED_TOML_START = '# One Works managed Kimi hooks start'
const MANAGED_TOML_END = '# One Works managed Kimi hooks end'
...
export const prepareKimiNativeHooks = (
  ...
  env.__ONEWORKS_PROJECT_KIMI_NATIVE_HOOKS_AVAILABLE__ = '0'
  ...
  try {
    ...
    env.__ONEWORKS_PROJECT_KIMI_NATIVE_HOOKS_AVAILABLE__ = enabled ? '1' : '0'
    env.__ONEWORKS_PROJECT_KIMI_HOOK_COMMAND__ = buildKimiNativeHookCommand(nodePath)
    return enabled
  } catch (error) {
    logger.warn('[kimi hooks] failed to prepare native hook bridge', error)
    env.__ONEWORKS_PROJECT_KIMI_NATIVE_HOOKS_AVAILABLE__ = '0'
    delete env.__ONEWORKS_PROJECT_KIMI_HOOK_COMMAND__
    return false
  }
}
