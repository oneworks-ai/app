...
const SESSION_PROVENANCE_FILE = '.oneworks-session.json'

interface KimiSessionProvenance {
  version: 1
  fingerprint: string
  summary: {
    agent: string
...
const readTextSafe = async (targetPath: string) => {
  try {
...
const resolveKimiNativeHookState = (ctx: AdapterCtx) => {
  const command = ctx.env.__ONEWORKS_PROJECT_KIMI_HOOK_COMMAND__?.trim()
  return {
    enabled: ctx.env.__ONEWORKS_PROJECT_KIMI_NATIVE_HOOKS_AVAILABLE__ === '1' && command != null && command !== '',
    command: command ?? ''
  }
}
...
const readSessionProvenance = async (shareDir: string) => {
  const content = await readTextSafe(resolve(shareDir, SESSION_PROVENANCE_FILE))
  if (content == null) return undefined

  try {
    const parsed = JSON.parse(content) as Partial<KimiSessionProvenance>
    return parsed.version === 1 && typeof parsed.fingerprint === 'string' ? parsed as KimiSessionProvenance : undefined
  } catch {
    return undefined
  }
}

const writeSessionProvenance = async (
  shareDir: string,
  provenance: KimiSessionProvenance
) => {
  await writeFile(resolve(shareDir, SESSION_PROVENANCE_FILE), `${JSON.stringify(provenance, null, 2)}\n`, 'utf8')
}

const buildSessionProvenance = async (params: {
  agent: string
  cliModel?: string
  configPath?: string
  ...
  const configSource = params.configValue != null
    ? { kind: 'generated', digest: sha256(stableJson(params.configValue)) }
    : params.configPath != null
    ? { kind: 'copied', digest: sha256(await readFile(params.configPath, 'utf8')) }
    : { kind: 'none' }
  const fingerprintInput = {
    agent: params.agent,
    cliModel: params.cliModel ?? null,
    configSource,
    cwd: params.ctx.cwd,
    extraOptions: params.options.extraOptions ?? [],
    mcpServerKeys: params.mcpServerKeys,
    realShareDir: params.realShareDir,
    requestedModel: params.options.model ?? null,
    skillsDir: params.skillsDir != null,
    systemPrompt: params.systemPrompt == null ? null : sha256(params.systemPrompt),
  ...
  return {
    ...
    summary: {
      agent: params.agent,
      ...(params.cliModel != null ? { cliModel: params.cliModel } : {}),
      config: configSource.kind,
      cwd: params.ctx.cwd,
      realShareDir: params.realShareDir,
      ...(params.options.model != null ? { requestedModel: params.options.model } : {})
    }
  } satisfies KimiSessionProvenance
}
...
const restoreLegacyKimiSessionState = async (params: {
  ...
  await migrateProjectHomeSegment(params.ctx.cwd, params.ctx.env, 'caches')
  const cacheRoot = resolveProjectOoPath(params.ctx.cwd, params.ctx.env, 'caches')
  const matchingCandidates: Array<{ mtimeMs: number; sourceDir: string }> = []
  const legacyCandidates: Array<{ mtimeMs: number; sourceDir: string }> = []

  const entries = await readDirSafe(cacheRoot)
  for (const entry of entries) {
    if (!entry.isDirectory() || entry.name === params.ctx.ctxId) continue

    const sourceShareDir = resolve(
      cacheRoot,
      entry.name,
      params.sessionId,
      'adapter-kimi',
    ...
    const sourceProvenance = await readSessionProvenance(sourceShareDir)
    if (sourceProvenance == null) {
      legacyCandidates.push({ mtimeMs, sourceDir })
      continue
    }
    if (sourceProvenance.fingerprint === params.provenance.fingerprint) {
      matchingCandidates.push({ mtimeMs, sourceDir })
    }
  }
  ...
  if (candidates.length === 0) {
    if (legacyCandidates.length > 1) {
      params.ctx.logger.warn('[kimi session] skipping ambiguous legacy session restore', {
        sessionId: params.sessionId,
        candidates: legacyCandidates.map(candidate => candidate.sourceDir)
      })
      return
    }
...
export const resolveKimiSessionBase = async (
  ...
  const sessionRoot = resolveProjectOoPath(ctx.cwd, ctx.env, 'caches', ctx.ctxId, options.sessionId, 'adapter-kimi')
  ...
  const cliModel = generatedModelConfig?.cliModel ?? options.model
  const reportedAgent =
    agentFilePath == null && typeof adapterConfig.agent === 'string' && adapterConfig.agent.trim() !== ''
      ? adapterConfig.agent.trim()
      : agentFilePath != null && typeof adapterConfig.agent === 'string' && adapterConfig.agent.trim() !== ''
      ? adapterConfig.agent.trim()
      : 'default'
  const toolNames = resolveReportedToolNames(options.tools, adapterConfig.agent)
  const provenance = await buildSessionProvenance({
    agent: reportedAgent,
    ...(cliModel != null ? { cliModel } : {}),
    configPath: generatedConfigPath ?? copiedConfigPath,
    configValue: generatedConfig,
    ctx,
    mcpServerKeys: Object.keys(selectedMcpServers).sort(),
    options,
    realShareDir,
    ...(skillsDir != null ? { skillsDir } : {}),
    ...(normalizedSystemPrompt != null && normalizedSystemPrompt !== ''
      ? { systemPrompt: normalizedSystemPrompt }
      : {}),
    thinking,
    toolNames
  })
  await writeSessionProvenance(shareDir, provenance)
  await restoreLegacyKimiSessionState({
    ctx,
    provenance,
  ...
  return {
    ...
    spawnEnv: toProcessEnv({
      ...
      __ONEWORKS_KIMI_TASK_SESSION_ID__: options.sessionId,
      __ONEWORKS_KIMI_HOOK_RUNTIME__: options.runtime,
      ...(options.model != null ? { __ONEWORKS_KIMI_HOOK_MODEL__: options.model } : {})
    }),
    thinking,
    toolNames,
    turnArgPrefix
