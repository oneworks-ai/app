...
const createHookEnv = (params: {
  ...
  __ONEWORKS_COPILOT_HOOKS_ACTIVE__: '1',
  [NATIVE_HOOK_BRIDGE_ADAPTER_ENV]: 'copilot',
  __ONEWORKS_HOOK_EVENT_NAME__: params.eventName,
  __ONEWORKS_PROJECT_WORKSPACE_FOLDER__: params.ctx.cwd,
  __ONEWORKS_COPILOT_TASK_SESSION_ID__: params.options.sessionId,
  __ONEWORKS_COPILOT_HOOK_RUNTIME__: params.options.runtime,
  ...(params.options.model != null ? { __ONEWORKS_COPILOT_HOOK_MODEL__: params.options.model } : {})
})
...
export const prepareCopilotNativeHooks = (
  ...
  env.__ONEWORKS_PROJECT_COPILOT_NATIVE_HOOKS_AVAILABLE__ = '0'
  ...
  try {
    ...
    env.__ONEWORKS_PROJECT_COPILOT_HOOK_COMMAND__ = buildNodeScriptCommand({
      nodePath,
      scriptPath: MANAGED_COMMAND_PATH
    })
    env.__ONEWORKS_PROJECT_COPILOT_NATIVE_HOOKS_AVAILABLE__ = enabled ? '1' : '0'
    return enabled
  } catch (error) {
    logger.warn('[copilot hooks] failed to prepare native hook bridge', error)
    env.__ONEWORKS_PROJECT_COPILOT_NATIVE_HOOKS_AVAILABLE__ = '0'
    delete env.__ONEWORKS_PROJECT_COPILOT_HOOK_COMMAND__
    return false
  }
}
...
export const mergeCopilotNativeHooksIntoSettings = (params: {
  ...
  const hooks = isRecord(params.settings.hooks)
    ...
    : {}
  const enabled = params.ctx.env.__ONEWORKS_PROJECT_COPILOT_NATIVE_HOOKS_AVAILABLE__ === '1'
  const command = params.ctx.env.__ONEWORKS_PROJECT_COPILOT_HOOK_COMMAND__?.trim()
