...
export const toProcessEnv = (env: AdapterCtx['env']): NodeJS.ProcessEnv => (
  mergeProcessEnvWithProjectEnv(env, {
    workspaceFolder: typeof env.__ONEWORKS_PROJECT_WORKSPACE_FOLDER__ === 'string'
...
export const buildCopilotChildEnv = async (
  ...
  const nativeHooksActive = ctx.env.__ONEWORKS_PROJECT_COPILOT_NATIVE_HOOKS_AVAILABLE__ === '1'
