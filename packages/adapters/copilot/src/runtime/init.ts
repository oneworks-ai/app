...
import { migrateProjectHomeSegments, resolveProjectMockHome } from '@oneworks/utils'
...
export const initCopilotAdapter = async (ctx: AdapterCtx) => {
  ...
  ctx.env.__ONEWORKS_PROJECT_ADAPTER_COPILOT_CLI_PATH__ = await ensureManagedNpmCli({
    adapterKey: 'copilot',
    binaryName: 'copilot',
