...
const runtime = process.env.__ONEWORKS_COPILOT_HOOK_RUNTIME__ as AdapterQueryOptions['runtime'] | undefined
const taskSessionId = process.env.__ONEWORKS_COPILOT_TASK_SESSION_ID__?.trim()
...
const normalizeEventName = (input: NativeCopilotHookInput): 'PostToolUse' | 'PreToolUse' | 'Stop' => {
  ...
  const envEventName = normalizeRawEventName(process.env.__ONEWORKS_HOOK_EVENT_NAME__)
...
export const isCopilotNativeHookEnv = () => (
  process.env.__ONEWORKS_COPILOT_HOOKS_ACTIVE__ === '1'
)
...
export const mapCopilotHookInputToOneWorks = (input: NativeCopilotHookInput): HookInput => {
...
export const mapOneWorksHookOutputToCopilot = (
  ...
  switch (eventName) {
    case 'PreToolUse': {
      ...
      if (hookSpecificOutput?.hookEventName === 'PreToolUse') {
        ...
        result.permissionDecisionReason = blockReason(output.stopReason, 'blocked by One Works PreToolUse hook')
      }
      return result
    }
    case 'PostToolUse':
    ...
    case 'Stop': {
      if (output.continue === false) {
        return {
          decision: 'block',
          reason: blockReason(output.stopReason, 'blocked by One Works Stop hook')
        }
      }
      return {}
    }
...
export const runCopilotHookBridge = async () => {
  try {
    ...
    const eventName = normalizeEventName(input)
    const hookInput = mapCopilotHookInputToOneWorks(input)
    const result = await executeHookInput(hookInput)
    process.stdout.write(`${JSON.stringify(mapOneWorksHookOutputToCopilot(eventName, result))}\n`)
  } catch (error) {
    process.stdout.write(`${
      JSON.stringify({
        systemMessage: `oneworks copilot hook bridge error: ${String(error)}`
      })
    }\n`)
  }
}
