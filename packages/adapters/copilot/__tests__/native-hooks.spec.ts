...
const createWorkspace = async () => {
  const dir = await mkdtemp(join(tmpdir(), 'ow-copilot-hooks-'))
  tempDirs.push(dir)
  return dir
}
...
describe('copilot native hooks', () => {
  it('prepares the shared hook runtime and marks Copilot native hooks available', () => {
    ...
    expect(ctx.env.__ONEWORKS_PROJECT_COPILOT_NATIVE_HOOKS_AVAILABLE__).toBe('1')
    expect(ctx.env.__ONEWORKS_PROJECT_COPILOT_HOOK_COMMAND__).toContain('call-hook.js')
  ...
  it('writes managed hooks into Copilot settings without duplicating existing hook entries', async () => {
    ...
    const ctx = {
      cwd,
      env: {
        __ONEWORKS_PROJECT_COPILOT_NATIVE_HOOKS_AVAILABLE__: '1',
        __ONEWORKS_PROJECT_COPILOT_HOOK_COMMAND__: `"${process.execPath}" "/tmp/call-hook.js"`
      },
      logger: {
        info: vi.fn(),
        warn: vi.fn(),
    ...
    expect(JSON.stringify(settings)).toContain('__ONEWORKS_COPILOT_HOOKS_ACTIVE')
    expect(JSON.stringify(settings)).toContain('__ONEWORKS_COPILOT_TASK_SESSION_ID')
  ...
  it('does not write empty native hook arrays when the managed bridge is disabled', async () => {
    ...
    const ctx = {
      cwd,
      env: {
        __ONEWORKS_PROJECT_COPILOT_NATIVE_HOOKS_AVAILABLE__: '0'
      },
      logger: {
        info: vi.fn(),
        warn: vi.fn(),
