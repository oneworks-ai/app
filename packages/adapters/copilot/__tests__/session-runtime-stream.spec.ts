...
describe('createCopilotSession stream runtime', () => {
  ...
  it('runs copilot prompt mode with JSON output and emits message/stop', async () => {
    ...
    expect(spawnMock.mock.calls[0]?.[1]).toEqual([
      ...
      '--no-remote',
      '--config-dir',
      resolveProjectHomePath(ctx.cwd, ctx.env, '.mock', 'copilot'),
      '--prompt',
  ...
  it('maps stable Copilot CLI options into args, env, and managed settings', async () => {
    ...
    expect(JSON.parse(
      await readFile(
        resolveProjectHomePath(cwd, ctx.env, '.mock', 'copilot', 'settings.json'),
        'utf8'
      )
    )).toEqual({
      askUser: false,
  ...
  it('stages system prompt and selected skills for Copilot native discovery', async () => {
    ...
    await createCopilotSession(ctx, {
      ...
      description: 'hello',
      systemPrompt: 'Use the One Works workspace context.',
      assetPlan: {
        adapter: 'copilot',
        diagnostics: [],
        mcpServers: {},
    ...
    const copilotHome = resolveProjectHomePath(cwd, ctx.env, '.mock', 'copilot')
    expect(env?.COPILOT_HOME).toBe(copilotHome)
    expect(env?.COPILOT_SKILLS_DIRS).toBe(`${copilotHome}/sessions/session-assets/skills`)
    expect(env?.COPILOT_CUSTOM_INSTRUCTIONS_DIRS).toBe(
      `${copilotHome}/sessions/session-assets/instructions`
    )
    expect(
      await readFile(
        `${copilotHome}/sessions/session-assets/instructions/copilot-instructions.md`,
        'utf8'
      )
    ).toBe('Use the One Works workspace context.\n')
  })
  ...
  it('maps routed model services to Copilot provider env and resolved model flag', async () => {
    ...
    expect(spawnOptions?.env?.COPILOT_PROVIDER_BASE_URL).toMatch(
      /^http:\/\/127\.0\.0\.1:\d+\/__oneworks_provider\/route-\d+$/
    )
  })
