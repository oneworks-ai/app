...
describe('resolveCopilotBinaryPath', () => {
  ...
  it('returns the env-specified path when set', () => {
    expect(resolveCopilotBinaryPath({
      __ONEWORKS_PROJECT_ADAPTER_COPILOT_CLI_PATH__: '/usr/local/bin/copilot'
    })).toBe('/usr/local/bin/copilot')
  })

  it('uses a managed binary from the global bootstrap cache', async () => {
    const home = await mkdtemp(join(tmpdir(), 'ow-copilot-home-'))
    const worktree = await mkdtemp(join(tmpdir(), 'ow-copilot-worktree-'))
    try {
      const env = {
