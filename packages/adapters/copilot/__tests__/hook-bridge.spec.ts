...
import { mapCopilotHookInputToOneWorks, mapOneWorksHookOutputToCopilot, supportsHookEvent } from '../src/hook-bridge'

describe('copilot native hook bridge helpers', () => {
  const withHookEventEnv = <T>(value: string | undefined, fn: () => T) => {
    const previous = process.env.__ONEWORKS_HOOK_EVENT_NAME__
    if (value == null) {
      delete process.env.__ONEWORKS_HOOK_EVENT_NAME__
    } else {
      process.env.__ONEWORKS_HOOK_EVENT_NAME__ = value
    }
    try {
      return fn()
    } finally {
      if (previous == null) {
        ...
        process.env.__ONEWORKS_HOOK_EVENT_NAME__ = previous
      }
    }
  }

  it('maps Copilot VS Code-compatible tool hooks into the unified hook shape', () => {
    const result = mapCopilotHookInputToOneWorks({
  ...
  it('maps blocked One Works output back into Copilot pre-tool decisions', () => {
    expect(mapOneWorksHookOutputToCopilot('PreToolUse', {
      continue: false,
      stopReason: 'blocked'
    })).toEqual({
  ...
  it('maps Stop blocking into Copilot agentStop continuation fields', () => {
    expect(mapOneWorksHookOutputToCopilot('Stop', {
      continue: false,
  ...
  it('uses a blocking fallback reason when Stop blocks without a stopReason', () => {
    expect(mapOneWorksHookOutputToCopilot('Stop', {
      ...
      decision: 'block',
      reason: 'blocked by One Works Stop hook'
    })
  })
