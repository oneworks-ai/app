...
const larkMcpRuntimeEnvSchema = z.object({
  ONEWORKS_LARK_APP_ID: z.string().min(1),
  ONEWORKS_LARK_APP_SECRET: z.string().min(1),
  ONEWORKS_LARK_DOMAIN: larkDomainSchema.optional(),
  ONEWORKS_CHANNEL_SESSION_ID: z.string().optional(),
  ONEWORKS_CHANNEL_KEY: z.string().optional(),
  ONEWORKS_CHANNEL_TYPE: z.string().optional(),
  ONEWORKS_CHANNEL_ID: z.string().optional(),
  ONEWORKS_CHANNEL_SESSION_TYPE: z.string().optional(),
  ONEWORKS_CHANNEL_REPLY_RECEIVE_ID: z.string().optional(),
  ONEWORKS_CHANNEL_REPLY_RECEIVE_ID_TYPE: larkMessageReceiveIdTypeSchema.optional(),
  ONEWORKS_LARK_DEFAULT_RECEIVE_ID: z.string().optional(),
  ONEWORKS_LARK_DEFAULT_RECEIVE_ID_TYPE: larkMessageReceiveIdTypeSchema.optional()
})
...
export const loadLarkMcpRuntimeEnv = (env: NodeJS.ProcessEnv = process.env): LarkMcpRuntimeEnv => {
  ...
  return {
    appId: parsed.ONEWORKS_LARK_APP_ID,
    appSecret: parsed.ONEWORKS_LARK_APP_SECRET,
    domain: parsed.ONEWORKS_LARK_DOMAIN ?? 'Feishu',
    sessionId: parsed.ONEWORKS_CHANNEL_SESSION_ID,
    channelKey: parsed.ONEWORKS_CHANNEL_KEY,
    channelType: parsed.ONEWORKS_CHANNEL_TYPE,
    channelId: parsed.ONEWORKS_CHANNEL_ID,
    sessionType: parsed.ONEWORKS_CHANNEL_SESSION_TYPE,
    replyReceiveId: parsed.ONEWORKS_CHANNEL_REPLY_RECEIVE_ID,
    replyReceiveIdType: parsed.ONEWORKS_CHANNEL_REPLY_RECEIVE_ID_TYPE,
    defaultReceiveId: parsed.ONEWORKS_LARK_DEFAULT_RECEIVE_ID ?? parsed.ONEWORKS_CHANNEL_REPLY_RECEIVE_ID,
    defaultReceiveIdType: parsed.ONEWORKS_LARK_DEFAULT_RECEIVE_ID_TYPE ?? parsed.ONEWORKS_CHANNEL_REPLY_RECEIVE_ID_TYPE
  }
}
