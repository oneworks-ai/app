...
describe('lark channel companion MCP', () => {
    ...
    expect(server).toMatchObject({
      ...
      config: {
        ...
        env: expect.objectContaining({
          ONEWORKS_LARK_APP_ID: 'app_id',
          ONEWORKS_LARK_APP_SECRET: 'app_secret',
          ONEWORKS_CHANNEL_ID: 'chat_1',
          ONEWORKS_LARK_DEFAULT_RECEIVE_ID: 'chat_1',
          ONEWORKS_LARK_DEFAULT_RECEIVE_ID_TYPE: 'chat_id'
        })
      }
    })
  ...
  it('uploads a local file and sends it to the bound chat', async () => {
    ...
    const tempDir = await mkdtemp(join(tmpdir(), 'ow-lark-mcp-'))
