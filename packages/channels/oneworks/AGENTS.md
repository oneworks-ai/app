...
- debug outbound snapshot 是本地 / simulation 观察面，只记录当前 connection 内存中的出站消息，方便 CLI 和后续 UI 验证 native channel delivery；不要把它当成持久化 room transcript、runtime trace 或审计表。
...
- simulation webhook payload 只表达真实频道事件；复杂 scenario runner 后续应基于 webhook / channel manager 注入事件，而不是直接调用内部 service 得到结果。
