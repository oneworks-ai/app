...
- `modal-focus.ts` owns dependency-free modal focus primitives shared by compact side sheets and app-owned portal dialogs. Keep one connected/visible/enabled/non-hidden/non-inert eligibility predicate, focusable discovery, prevent-scroll focus, and focus-first behavior here; app-specific close state, background ownership, restoration order, and post-focus verification stay in the consuming adapter.
...
- `HostAppShell` responsive mode is owned by its explicit `isCompactLayout` / `host-app-shell--compact` state. Viewport media rules for direct `RouteWorkbenchShell` consumers must exclude `.host-app-shell`; otherwise an intermediate viewport can stack the desktop sidebar while the host still considers itself non-compact and collapse the content region to zero width.
