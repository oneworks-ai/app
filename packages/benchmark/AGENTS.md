...
`@oneworks/benchmark` 承载 benchmark 的发现、运行、workspace 准备与结果读写。
...
- `src/runner.ts`
  - 运行单 case / category
  - 通过 `@oneworks/task` 调用任务执行能力
- `src/result-store.ts`
  - 读写 `.oo/results/.../result.json`
- `src/schema.ts`
...
- benchmark 领域逻辑集中在本包
- CLI 和 server 通过 `@oneworks/app-runtime` 消费本包 runtime
- client 只消费 `@oneworks/types` 里的 benchmark contract
- 真正的任务执行仍由 `@oneworks/task` 负责
