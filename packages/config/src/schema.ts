...
type AdapterConfigContribution = import('@oneworks/core/config-schema').AdapterConfigContribution
...
const BASE_SCHEMA_ID = 'https://oneworks-ai.github.io/schema/config.base.json'
const WORKSPACE_SCHEMA_RELATIVE_PATH = '.oo/schema/config.schema.json'
const nodeRequire = createRequire(__filename)
const repoRoot = resolve(dirname(__filename), '../../..')
const createWorkspaceRequire = (cwd: string) => createRequire(resolve(cwd, '__oneworks_config_schema_loader__.cjs'))
...
const isAdapterPackageName = (name: string) => (
  name.startsWith('@oneworks/adapter-') ||
  /^@[^/]+\/adapter-/.test(name)
)
...
const adapterPackageNameToKey = (packageName: string) => (
  packageName.startsWith('@oneworks/adapter-')
    ? packageName.replace('@oneworks/adapter-', '')
    : packageName
)

const channelPackageNameToType = (packageName: string) => (
  packageName.startsWith('@oneworks/channel-')
    ? packageName.replace('@oneworks/channel-', '')
    : packageName
)
...
const resolveLocalWorkspaceSourcePath = (packageName: string, subpath: string, cwd: string) => {
  ...
  for (const workspaceRoot of workspaceRoots) {
    if (packageName.startsWith('@oneworks/adapter-')) {
      const adapterName = packageName.replace('@oneworks/adapter-', '')
    ...
    if (packageName.startsWith('@oneworks/channel-')) {
      const channelName = packageName.replace('@oneworks/channel-', '')
...
const resolvePackageJsonFromAdapterCache = (packageName: string) => {
  const cacheDir = resolveExistingAdapterPackageCacheDir(packageName)
  if (cacheDir == null) return undefined

  const packageJsonPath = resolve(cacheDir, 'node_modules', ...packageName.split('/'), 'package.json')
  return existsSync(packageJsonPath) ? packageJsonPath : undefined
}

const resolvePackageJsonPath = (packageName: string, cwd: string) => {
  const adapterCachePath = resolvePackageJsonFromAdapterCache(packageName)
  if (adapterCachePath != null) {
    return adapterCachePath
  }
...
const loadPackageExportFromPackageJson = async (
  packageJsonPath: string,
  exportKey: string,
  preferredConditions: readonly string[]
) => {
...
const loadPackageSubpathWithSourceFallback = async (packageName: string, subpath: string, cwd: string) => {
  ...
  const loadFromPackageExport = async (
    ...
    return await loadPackageExportFromPackageJson(packageJsonPath, exportKey, preferredConditions)
  }
  ...
  const adapterCachePackageJsonPath = resolvePackageJsonFromAdapterCache(packageName)
  if (adapterCachePackageJsonPath != null) {
    const adapterCacheModule = await loadPackageExportFromPackageJson(
      adapterCachePackageJsonPath,
      exportKey,
      ['import', 'default', 'require', 'node']
    )
    if (adapterCacheModule != null) return adapterCacheModule
  }
  ...
  try {
    ...
    const workspaceSourceExportModule = await loadFromPackageExport(['__oneworks__', 'default'])
...
const loadAdapterContribution = async (
  ...
  return packageName.startsWith('@oneworks/adapter-')
    ? contribution
    : {
      ...contribution,
...
const loadChannelDefinition = async (
  ...
  const packageName = specifierOrType.startsWith('@')
    ? specifierOrType
    : `@oneworks/channel-${specifierOrType}`
