...
export default defineRegister(({ registerTool }) => {
  registerTool(
    ...
    async (args) => {
      ...
      if (!sessionId) {
        throw new Error(
          'Session ID not found in environment variables. This tool can only be used within a One Works session.'
        )
      }

      const host = process.env.__ONEWORKS_PROJECT_SERVER_HOST__ ?? 'localhost'
      const port = process.env.__ONEWORKS_PROJECT_SERVER_PORT__ ?? '8787'
