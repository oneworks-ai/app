...
- `client/` 通过 `view.route.setSidebar(...)` 把 Room 接入宿主通用二级列表，并通过宿主 `view.ui.AgentRoom` 在 route body 直接呈现真实会话；不要在详情区重复资源列表或复制聊天逻辑。route 已拥有页面级内容间距时给 `AgentRoom` 传 `inset={false}`，避免宿主与嵌入组件重复制造 padding。分享、调试、场景与链路入口通过 `view.route.setActions(...)` 放在通用 Header。
