...
- 敏感值默认脱敏；
- 原始 JavaScript/CDP、完整 Cookie 值和敏感字段默认关闭；OneWorks 可在未连接时保存用户的显式布尔配置，并在兼容浏览器连接后同步执行策略；每次使用高级访问仍需针对准确操作逐次确认；配对后可读取代理状态，每次设置或清除代理都需要准确的高风险确认；
- 普通网站操作可遵循用户显式设置的“始终询问”或“始终允许”URL 规则；高级访问与非网站范围的高风险操作仍需绑定准确操作、目标和参数的确认；
- 审计仅保留必要摘要，不持久化完整秘密值。
