...
`.github/workflows/chrome-extension-ci.yml` builds the developer and minimal ZIPs plus `SHA256SUMS` for relevant pull requests and `main` changes. When `main` creates a `pkg/oneworks-plugin-external-browser-driver/v*` tag, Release Tags explicitly dispatches `.github/workflows/chrome-extension-release.yml` only to create a GitHub Release with provenance attestations. Chrome Web Store submission is separate: one explicit action for the same tag may upload the full developer package only after `fetchStatus` and the Dashboard confirm that no active or accepted submission exists. Any pending, unknown, or action-required state stops at the external-review boundary; never retry the upload or publish.
...
- Results are redacted at both the extension and bridge by default. URLs strip userinfo and sensitive query keys, secret-like form values and console text are masked, inline PAC source is removed, and cookie metadata discovery never returns values. Website permissions can persist safe URL wildcard rules with “Always ask” or “Always allow”; the first match wins and the bridge rechecks the live tab URL immediately before dispatch. “Always allow” can skip the OneWorks confirmation for ordinary site-scoped operations, but never bypasses advanced-access, extension-capability, or Chrome permission gates.
- “Advanced access” provides switches for raw CDP/JavaScript, complete cookie values, and sensitive page fields. OneWorks persists only the user's boolean preferences, independently of connection state, and synchronizes them into the compatible extension's `chrome.storage.session` enforcement policy after connection. Settings is the sole preference writer; the extension popup only reports the effective policy. Raw is a superset of cookie and sensitive-field access and is browser-session-wide; the tab/origin preflight catches accidental navigation but is not a security boundary. Advanced-access operations require an exact per-use confirmation even when a matching website rule says “Always allow”; raw operations also execute globally exclusively. Pending confirmations show a redacted preview, while persistent audit records contain only classification and hashes. Host file paths and file-system escape methods remain blocked.
