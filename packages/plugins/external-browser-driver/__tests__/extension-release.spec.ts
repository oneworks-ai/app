...
describe('external browser extension release packaging', () => {
  ...
  it('maps package prerelease versions to monotonic Chrome versions', () => {
    ...
    expect(release.chromeVersionFor('0.1.0-rc.7')).toBe('0.1.0.30007')
  ...
  it('creates deterministic base and privileged archives with root manifests and icons', () => {
    ...
    expect(base).toMatchObject({ flavor: 'base', manifest_version: '1.0.0.30000' })
    expect(release.stableExtensionId).toBe(CHROME_EXTENSION_ID)
    expect(base).toMatchObject({ extension_id: CHROME_EXTENSION_ID })
    expect(privileged).toMatchObject({ flavor: 'privileged', manifest_version: '1.0.0.30000' })
    ...
    const sharedRuntime = Buffer.from(baseEntries['operations/shared.js']).toString('utf8')
    const contentScript = Buffer.from(baseEntries['content-script.js']).toString('utf8')
    ...
    expect(sharedRuntime).not.toContain("EXTENSION_VERSION = '0.1.0'")
