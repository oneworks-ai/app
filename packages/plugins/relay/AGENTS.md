...
- `src/client/react-view.ts`：账号、服务器、登录、消息中心、账号详情、团队详情和令牌页面的 React 落地页。账号 popover / profile tabs / 独立消息页体验镜像 Relay Admin 的 `AdminNavRail.tsx`、`features/profile/ProfilePage.tsx` 和 `features/messages/MessageCenterPage.tsx`，但插件客户端不直接持有 session token；账号 / 团队详情 tabs 必须消费 client host 暴露的 `NativeTabs`，账号列表必须消费 host `InteractionList`，不要在 relay 插件里复制通用 tabs / list 风格。token / list surface 必须消费 `@oneworks/components/admin-list-surface` 的 CSS source、class map 和 native markup helper，不要在插件里复制一套 AdminListTable 样式。不要恢复 legacy string / `innerHTML` 渲染。
...
- 本地验证 Relay 多客户端链路时，优先用一个本机 Relay Server 加两套独立 client / project-home / device token 登录同一账号：确认两个设备都心跳在线且携带不同 `workspaceFolder`，再通过真实协议验证远端 session 创建、绑定和 forwarding。调试数据保留在本地 auth / relay store，除非用户明确要求，不要为了切换场景删除账号或设备数据。
- 多客户端调试不要通过手工编辑 `auth.json`、relay store 或设备列表硬塞数据；使用真实登录 / `login-callback` / `connect` / heartbeat API。`~/.oneworks/auth.json` 只代表登录 session，设备身份必须来自每个 workspace/project-home 的 `.local/plugins/relay/device.json`；如果不同客户端出现同一个 device id，launcher 会把多个 workspace 合并成一个设备，这是要先修的真实链路问题。
...
- 账号 / 团队目录文档同步复用 `/api/relay/config/global` 的个人快照和 `/api/relay/teams/:teamId/documents` 的团队快照，但服务端只保存密文 payload 和 `documentCount / totalSizeBytes / countsByKind` 元数据，不能读取 `AGENTS.md` 正文。账号同步只读写 `~/.oo/accounts/<account-id>/AGENTS.md`，团队同步只读写 `~/.oo/teams/<team-id>/AGENTS.md`；`~/AGENTS.md` 只作为“导入到当前账号”的一次性来源，不作为持续同步路径。同步实现落在 `src/server/personal-document-sync.ts`，写入远端版本到本机前如遇已有不同内容，要先生成同目录 `*.relay-conflict-*.md` 备份，不要静默覆盖。
