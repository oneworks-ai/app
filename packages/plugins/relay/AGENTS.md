...
- 当前仓库开发配置对 Relay 启用 `watch: true`：Client 使用 Vite `/@fs/` dev entry，插件 watch WebSocket 触发带 `pluginVersion` cachebuster 的 scope re-activation；修改 `src/client/*` 后必须先在同一页面验证 HMR，不得用 `reload()` 代替。watch 模式下插件 server entry 也优先解析源码并由 plugin runtime reload；只有未启用 watch 的生产 / 打包运行时才加载 `dist/`。独立的 `apps/relay-server` 本地进程如果从 `dist/cli.js` 启动，修改该应用后仍需单独 build 并重启，不能和插件 HMR 混为一谈。
