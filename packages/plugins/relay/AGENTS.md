...
- 通用 `connect` / 自动恢复在目标 server 已有登录账号时，必须优先用该账号的有效 session 重新注册设备，再让服务端换发 device token；不能因为本地还留有过期 device token 就绕过有效账号并报 pairing token 失败。显式 `accountKey` 仍用于多账号精确选择。
