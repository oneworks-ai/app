...
2. 再用 `oneworks --resume <sessionId>` 或 `oneworks --resume <sessionId> --print --input-format stream-json` 继续。
3. 如果只是想停掉卡住的会话，用 `oneworks stop <sessionId>` 或 `oneworks kill <sessionId>`。
