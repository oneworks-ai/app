...
- `deepseek`: DeepSeek, `https://api.deepseek.com`; default models `deepseek-v4-flash`, `deepseek-v4-pro`, with legacy aliases `deepseek-chat`, `deepseek-reasoner` retained only for existing configurations after their documented 2026-07-24 deprecation; portal `https://platform.deepseek.com`; API keys `https://platform.deepseek.com/api_keys`; billing `https://platform.deepseek.com/usage`; docs `https://api-docs.deepseek.com`; status `https://status.deepseek.com`; model and balance APIs are supported.
  - Codex uses DeepSeek's native Responses API. As of 2026-07-31 only `deepseek-v4-flash` is compatible; DeepSeek documents `deepseek-v4-pro` support as planned for early August 2026.
