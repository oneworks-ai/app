...
describe('definition core helpers', () => {
  ...
  it('resolves spec, entity, and skill identifiers from conventional files', () => {
    ...
    expect(resolveEntityIdentifier('/tmp/project/.oo/entities/reviewer/README.md')).toBe('reviewer')
    expect(resolveSkillIdentifier('/tmp/project/.oo/skills/research/SKILL.md')).toBe('research')
  })

  it('resolves definition names from explicit names, resolved names, and file paths', () => {
    expect(resolveDefinitionName({
      path: '/tmp/project/.oo/rules/base.md',
      body: '',
      attributes: {}
    })).toBe('base')

    expect(resolveDefinitionName({
      path: '/tmp/project/.oo/specs/release/index.md',
      body: '',
      attributes: {},
