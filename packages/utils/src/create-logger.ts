...
import type { LogLevel, Logger } from '@oneworks/types'

import { resolveProjectOoPath } from './ai-path'
import { migrateProjectHomeSegmentSync } from './project-home-migration'

export type { Logger } from '@oneworks/types'
...
export const createLogger = (
  ...
  try {
    migrateProjectHomeSegmentSync(cwd, env, 'logs')
  } catch {
    // Logging must stay best-effort; startup paths run the full migration and surface failures.
  }

  const logsRoot = resolveProjectOoPath(cwd, env, 'logs')
  const cachesRoot = resolveProjectOoPath(cwd, env, 'caches')
  const taskDir = resolve(logsRoot, ...normalizedLogPrefix, taskId)
