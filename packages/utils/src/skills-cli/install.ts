/* eslint-disable max-lines -- skills CLI install helpers share temp repo/cache handling and output parsing. */
...
import { listInstalledSkillDirs, selectInstalledSkillDir } from './installed-skill'
...
export const installSkillsCliSkillToTemp = async (params: {
  ...
  const tempDir = await mkdtemp(join(tmpdir(), 'ow-skills-cli-install-'))
  try {
    await runSkillsCli({
...
export const installSkillsCliSourceToTemp = async (params: {
  cacheCwd?: string
  config?: SkillsCliConfig
  registry?: string
  source: string
  ...
  try {
    ...
    if (installedSkills.length === 0) {
      throw new Error(`skills CLI did not install any skills for "${source}".`)
    }
