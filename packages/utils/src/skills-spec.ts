...
export const parseSkillsSpec = (value: string): ParsedSkillsSpec => {
  ...
  if (segments.length >= 3 && looksLikeRegistry(segments[0])) {
    ...
    const sourceStart = segments[1] === '' ? 2 : 1
    const sourcePrefix = segments[1] === '' ? '@' : ''
    const nameIndex = segments.length === sourceStart + 2 ? segments.length - 1 : segments.length - 2
    const name = normalizeNonEmptyString(segments[nameIndex])
    const rawSource = normalizeNonEmptyString(segments.slice(sourceStart, nameIndex).join('@'))
    const source = rawSource == null ? undefined : `${sourcePrefix}${rawSource}`
    const version = normalizeNonEmptyString(
      nameIndex === segments.length - 1 ? undefined : segments[segments.length - 1]
    )
  ...
  if (segments[0] === '' && segments.length >= 3) {
    const nameIndex = segments.length === 3 ? 2 : segments.length - 2
    const rawSource = normalizeNonEmptyString(segments.slice(1, nameIndex).join('@'))
    const name = normalizeNonEmptyString(segments[nameIndex])
    const version = normalizeNonEmptyString(
      nameIndex === segments.length - 1 ? undefined : segments[segments.length - 1]
    )

    if (rawSource != null && name != null) {
      return {
        ref,
        source: `@${rawSource}`,
        name,
