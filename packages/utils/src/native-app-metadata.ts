...
export const isFilesystemShapedNativeAppValue = (value: string) => {
  if (/^https?:\/\//iu.test(value)) {
    try {
      ...
      if (
        ...
        ].some(containsDecodedFilesystemSelector)
      }
    } catch {
      return true
    }
  }
  return containsDecodedFilesystemSelector(value)
}
...
export const isCredentialLikeNativeAppValue = (value: string) => (
  CREDENTIAL_VALUE_PATTERN.test(value) || hasSegmentedOpaqueValue(value) || hasHttpUserInfo(value)
)
