...
const adapterResumeFixtures = [
  ...
  },
  {
    ...
    value: { opencodeSessionId: 'opencode-native', title: 'OneWorks:session-1' },
    otherKey: 'adapter.gemini.session',
...
describe('cache utils', () => {
  ...
  it('returns undefined when the cache file does not exist', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'ow-cache-'))
    tempDirs.push(cwd)
    useProjectHome(cwd)
  ...
  it('honors an explicit env instead of falling back to process.env', async () => {
    ...
    const expectedValue = { sessionA: 'thr_123' }
    const writeResult = await setCache(cwd, 'task-1', 'session-1', 'adapter.codex.threads', expectedValue, env)
    const expectedPath = resolveProjectHomePath(
      cwd,
      env,
      'caches',
      'task-1',
      'session-1',
      'adapter.codex.threads.json'
    )

    expect(writeResult.cachePath).toBe(expectedPath)
    await expect(getCache(cwd, 'task-1', 'session-1', 'adapter.codex.threads', env)).resolves.toEqual(expectedValue)
  })

  it('does not backfill legacy cache contents before writing new project-home cache files', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'ow-cache-'))
    tempDirs.push(cwd)
    useProjectHome(cwd)
    ...
    await mkdir(dirname(legacyCachePath), { recursive: true })
    await writeFile(legacyCachePath, '{"legacy":"kept"}\n', 'utf8')

    await setCache(cwd, 'task-1', 'session-1', 'adapter.codex.threads', { current: 'written' })

    const migratedCachePath = resolveProjectHomePath(
      cwd,
      process.env,
      'caches',
      'legacy-ctx',
    ...
    await expect(readFile(migratedCachePath, 'utf8')).rejects.toMatchObject({ code: 'ENOENT' })
  })

  it('skips legacy cache migration before writing non-resume cache files', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'ow-cache-'))
    tempDirs.push(cwd)
    useProjectHome(cwd)
    ...
    const legacyCachePath = join(
      ...
      'legacy-ctx',
      'session-1',
      'adapter.claude-code.settings.json'
    )
    await mkdir(dirname(legacyCachePath), { recursive: true })
    await writeFile(legacyCachePath, '{"legacy":"settings"}\n', 'utf8')

    await setCache(cwd, 'task-1', 'session-1', 'adapter.claude-code.settings', { current: true })
