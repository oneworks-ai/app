...
const useProjectHomeForTest = (cwd: string) => {
  process.env.__ONEWORKS_PROJECT_HOME_PROJECTS_DIR__ = path.join(cwd, '.oneworks-projects')
}

describe('project skill installs', () => {
  let computeSkillDirectoryHash: typeof import('#~/project-skills.js').computeSkillDirectoryHash
  ...
  it('replaces existing installed files via a temp directory swap and rewrites the local name', async () => {
    ...
    const cwd = await realpath(await mkdtemp(path.join(os.tmpdir(), 'ow-project-skills-install-')))
    tempDirs.push(cwd)
    useProjectHomeForTest(cwd)

    const existingDir = path.join(cwd, '.oo', 'skills', 'internal-review')
    await mkdir(existingDir, { recursive: true })
    await writeFile(path.join(existingDir, 'SKILL.md'), '---\nname: stale-review\n---\nOld skill\n')
    await writeFile(path.join(existingDir, 'stale.txt'), 'stale file\n')

    const tempDir = await realpath(await mkdtemp(path.join(os.tmpdir(), 'ow-project-skills-install-src-')))
    ...
    expect(mocks.installSkillsCliSkillToTemp).toHaveBeenCalledWith(expect.objectContaining({
      cacheCwd: cwd
    }))
  ...
  it('checks the lock hash before reusing an already installed target', async () => {
    ;({ computeSkillDirectoryHash, installProjectSkill } = await import('#~/project-skills.js'))
    const cwd = await realpath(await mkdtemp(path.join(os.tmpdir(), 'ow-project-skills-install-')))
    tempDirs.push(cwd)
    useProjectHomeForTest(cwd)
    ...
    const expectedHash = await computeSkillDirectoryHash(existingDir)
    await writeFile(path.join(existingDir, 'LOCAL.md'), 'local edits\n')
  ...
  it('does not clear stale legacy install locks before re-installing the configured skill', async () => {
    ;({ installProjectSkill } = await import('#~/project-skills.js'))
    const cwd = await realpath(await mkdtemp(path.join(os.tmpdir(), 'ow-project-skills-install-')))
    tempDirs.push(cwd)
    useProjectHomeForTest(cwd)
    ...
    const lockDir = path.join(cwd, '.oo', 'caches', 'project-skill-installs', 'locks', 'internal-review')
    ...
    await expect(pathExists(lockDir)).resolves.toBe(true)
    await expect(readFile(path.join(cwd, '.oo', 'skills', 'internal-review', 'SKILL.md'), 'utf8')).resolves
