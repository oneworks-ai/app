...
`@oneworks/hooks` 承载通用 hook runtime、native helper、bridge loader 与 `oneworks-call-hook` 二进制。
hook runtime 直接依赖 `@oneworks/config`、`@oneworks/utils` 与 `@oneworks/types`。

## 先看哪里

- `call-hook.js`
  - hook 子进程入口
  - 负责切到 home project 下的 `.mock` HOME，并加载真实 entry
- `src/entry.ts`
...
- `packages/hooks`:
  ...
  - `oneworks-call-hook` 与 managed script path
...
- 运行时基础设施优先放进 `@oneworks/config` 或 `@oneworks/utils`，不要在本包里重复实现一份。
