...
import type { ChatMessage } from '@oneworks/types'
...
describe('adapter hook bridge', () => {
  ...
  it('keeps nonzero exits marked as failed when the session is killed during cleanup', async () => {
    const logger = createLogger()
    callHookMock.mockClear()
    callHookMock.mockResolvedValue({ continue: true })
    ...
    bridge.wrapSession(session).kill()
    bridge.handleOutput({
      type: 'exit',
      data: {
        exitCode: 1,
