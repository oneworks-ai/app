...
describe('createClaudeJsonHooksPlugin', () => {
  it('executes declarative Claude command hooks and maps the JSON result back to a hook output', async () => {
    const pluginDir = await mkdtemp(join(tmpdir(), 'ow-claude-json-plugin-'))
    ...
    const plugin = createClaudeJsonHooksPlugin({
      ...
      claudePluginRoot: cwd => join(cwd, 'native-plugin'),
      pluginDataDir: cwd => join(cwd, 'plugin-data')
    })
    ...
    expect(result).toEqual(expect.objectContaining({
      continue: true,
      systemMessage: `${join(pluginDir, 'native-plugin')}|${join(pluginDir, 'plugin-data')}`
    }))
    await expect(stat(join(pluginDir, 'plugin-data'))).resolves.toEqual(
