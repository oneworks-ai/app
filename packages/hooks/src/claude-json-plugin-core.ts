...
type RuntimePath = string | ((cwd: string) => string)

const resolveRuntimePath = (value: RuntimePath, cwd: string) => (
  typeof value === 'function' ? value(cwd) : value
)
...
const createEventHandler = <K extends keyof HookInputs>(
  ...
  paths: {
    claudePluginRoot: RuntimePath
    pluginDataDir: RuntimePath
  }
) =>
async (
  ...
  for (const group of groups) {
    for (const hook of group.hooks ?? []) {
      ...
      const result = await runClaudeCommandHook({
        ...
        input,
        cwd: input.cwd,
        claudePluginRoot: resolveRuntimePath(paths.claudePluginRoot, input.cwd),
        pluginDataDir: resolveRuntimePath(paths.pluginDataDir, input.cwd)
      })
