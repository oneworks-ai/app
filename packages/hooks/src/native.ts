...
export const NATIVE_HOOK_BRIDGE_ADAPTER_ENV = '__ONEWORKS_HOOK_BRIDGE_ADAPTER__'
...
export const resolveManagedHookPackageDir = () => {
  try {
    const pkgJsonPath = require.resolve('@oneworks/hooks/package.json')
    return dirname(pkgJsonPath)
  } catch (error) {
    throw new Error('Failed to resolve @oneworks/hooks managed hook entry', { cause: error })
  }
}
