...
import { findWorkspaceAsset, resolveConfiguredWorkspaceAssets } from '@oneworks/workspace-assets'
...
export const resolveWorkspaceTaskTarget = async (params: {
  ...
  if (target == null) {
    ...
    throw new Error(
      available === ''
        ? `Workspace "${name}" not found. Configure workspaces in .oo.config.json.`
