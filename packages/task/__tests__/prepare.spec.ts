...
const mocks = vi.hoisted(() => ({
  ...
  mergeConfigs: vi.fn(),
  resolveUseDefaultOneworksMcpServer: vi.fn(),
...
vi.mock('@oneworks/config', () => ({
  ...
  resolveUseDefaultOneworksMcpServer: mocks.resolveUseDefaultOneworksMcpServer
}))
...
vi.mock('@oneworks/utils/create-logger', () => ({
  createLogger: mocks.createLogger
}))

vi.mock('@oneworks/utils/log-level', () => ({
...
describe('prepare', () => {
  beforeEach(() => {
    ...
    mocks.resolveUseDefaultOneworksMcpServer.mockReturnValue(true)
  ...
  it('stores the resolved permission mode in env for nested runtimes', async () => {
    ...
    expect(ctx.env.__ONEWORKS_PROJECT_PERMISSION_MODE__).toBe('dontAsk')
  ...
  it('preserves an inherited permission mode when the current run does not override it', async () => {
    ...
    const [ctx] = await prepare({
      cwd: '/tmp/project',
      env: {
        __ONEWORKS_PROJECT_PERMISSION_MODE__: 'plan'
      }
    }, {
      type: 'create',
      runtime: 'mcp',
  ...
  it('enables builtin permission hooks for mcp runtimes', async () => {
    ...
    expect(ctx.env.__ONEWORKS_PROJECT_ENABLE_BUILTIN_PERMISSION_HOOKS__).toBe('1')
  })
  ...
  it('syncs declared marketplace plugins before resolving workspace assets on create', async () => {
    ...
    expect(mocks.syncConfiguredMarketplacePlugins).toHaveBeenCalledWith(expect.objectContaining({
      cwd: '/tmp/project',
      marketplaces
    }))
