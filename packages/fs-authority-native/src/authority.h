...
struct TreeRequest { std::string action; std::string authority_id; std::vector<std::string> parent_segments; std::string entry_name; std::string quarantine_name; std::string expected_parent_identity; std::string expected_identity; std::string fault; };
struct PublishResult { std::string state; std::string code; std::string parent_identity; std::string identity; std::vector<std::string> warnings; bool committed = false; bool indeterminate = false; };
class Authority { public: virtual ~Authority() = default; virtual const std::string& id() const = 0; virtual const std::string& capability() const = 0; virtual PublishResult Publish(const PublishRequest& request) = 0; virtual PublishResult Tree(const TreeRequest& request) = 0; };
