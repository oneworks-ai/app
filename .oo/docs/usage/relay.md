...
每条项目规则 assignment 可以拥有独立的渐进式文档库，界面入口统一称为“文档”。Relay 插件把服务端文档同步到 `~/.oo/teams/<team-id>/project-rules/<assignment-id>/`，入口为 `AGENTS.md`，扩展内容放在同目录的 `rules/**/*.md`。只有当前 Git 仓库命中这条项目规则时，插件才会在会话提示词中引导 agent 阅读对应目录。

当前用户通过 `GET` / `PUT` / `POST /api/relay/config-assignments/<assignmentId>/documents` 读取或更新文档快照；平台管理员使用同构的 `/api/admin/config-assignments/<assignmentId>/documents`。Relay Server 的业务路由保存并透传客户端生成的密文格式 payload，以及同步、范围和审计元数据，不解析或索引文档正文。当前命名空间派生密钥用于范围隔离、传输格式完整性和避免正文意外暴露；Relay 部署运营方能够复现该密钥并解密内容，因此这不是面向运营方的零知识加密。账号文档、团队文档和项目规则文档使用三个独立命名空间，不会相互合并；本地同步同时拒绝非规范相对路径和命名空间中的符号链接，避免读写逃逸到其他目录。

## 登录方式

One Works Web 与 Electron 使用纯色、无独立卡片的 Client 原生登录页：它通过 scoped plugin API 读取所选 Relay Server 的 `/api/auth/login-options`，只展示当前启用的登录方式和 SSO provider，不在 Client 硬编码 provider 清单。密码、邀请和验证码请求也由插件 server 的固定 action 代理到已配置 Relay Server，因此 Electron renderer 不依赖跨域 CORS，也不存在任意远端代理入口。最近账号、记住账号、当前方式表单和切换其他方式的交互与 Relay Admin 对齐。密码直接在 Client 表单完成；验证码只有在服务端不要求 Turnstile 时才会作为 Client 原生方式返回。Passkey 需要 Relay 的 WebAuthn origin，SSO 需要身份提供方授权；Web 在当前顶层标签继续，Electron 则打开系统浏览器，并通过 `oneworks://relay/auth` 回到发起登录的 Launcher 或 workspace。旧版 Relay Server 仍可使用兼容 `/login` 页面。

`/login` 也是 Admin、CLI / Electron 和 Web 回跳共用的兼容登录页。部署方可以通过 `ONEWORKS_RELAY_DEFAULT_LOGIN_METHOD` 设置默认方式，浏览器也会记住用户上次选择的方式。
...
One Works 把官方 Relay 登录能力暴露为顶层 CLI 命令，用户不需要感知插件命名空间：
...
`-s, --server` 默认是 `cf`，也支持 `cloudflare`、`vercel` / `vc`、服务 id，或完整 URL。登录认证保存在本机 `~/.oneworks/auth.json`，同一台机器可以同时保存多个 server、多个账号。CLI 不提供“切换当前用户”；`users enable` / `users disable` 表示是否让当前 daemon 使用某个账号。

当一个 server 下只有一个匹配账号时，启用、停用和退出登录可以不传用户；当存在多个匹配账号时，交互终端会展示候选账号让用户选择。脚本和 AI 操作建议使用 `--json`，并用 `--account serverId:userId` 精确指定账号；同一个 `loginId` 或邮箱出现在不同 server 时，也应优先使用 `--account` 避免误操作。`--input <json>` 和 `--stdin` 支持把结构化 JSON 合并进命令 payload。
