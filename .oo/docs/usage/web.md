...
- “主题”页直接使用列表选择由插件提供的主题包；列表下方只展示当前主题插件注册的配置 tabs。默认主题只能查看规则；安装并启用“中国方案主题”插件后，可以分别开启或关闭基础背景、金色边框、按钮、输入框、菜单、浮层和顶部整活横幅，普通组件 padding / 图标尺寸会同时显示启用开关和只读 px 预设值。不同主题的专属选项会保存在各自配置下；插件缺失或停用时界面安全回退默认主题，重新启用后继续使用原设置。
- “外观”页的主题色、主题模式和历史时间线展示模式属于 app 级个人偏好，和主题包一起写入 global `appearance` 配置并跨项目生效。主题包可以声明自己的内置主题色；此时外观页主题色控件只读，运行时使用主题内置值，但保留原 `primaryColor`，切回默认主题后继续使用。历史时间线可选择紧凑的 `Event lines` 或原有的 `Nodes`；未配置时默认使用 `Event lines`。sender 输入区顶栏默认展开/折叠、会话列表搜索栏展示阈值，以及新建会话时是否显示公告、推荐操作列表等 Web UI 偏好继续使用各自现有的持久化方式。
