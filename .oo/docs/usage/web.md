...
- “外观”页的主题色、主题模式和历史时间线展示模式属于 app 级个人偏好，统一写入 global `appearance` 配置并跨项目生效。历史时间线可选择紧凑的 `Event lines` 或原有的 `Nodes`；未配置时默认使用 `Event lines`。sender 输入区顶栏默认展开/折叠、会话列表搜索栏展示阈值，以及新建会话时是否显示公告、推荐操作列表等 Web UI 偏好继续使用各自现有的持久化方式。
