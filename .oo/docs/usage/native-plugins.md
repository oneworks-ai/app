...
这页讲的是 adapter 自己的原生插件格式，不是顶层 `plugins` 配置里的统一 One Works 插件。

## 适用场景

当前完整 installer 支持的只有 Claude：

- 安装 Claude Code 原生插件到当前项目的 project home 私有目录
- 从配置里的 marketplace 解析 `plugin@marketplace`
- 把 Claude 插件里可复用的能力接入到当前项目

Copilot 不提供 One Works installer；但可以通过 `adapters.copilot.pluginDirs` 把本地 Copilot CLI plugin 目录传给官方 `--plugin-dir`。

如果你要配置统一 One Works 插件，继续看 [插件与数据资产](./plugins.md)。
...
如果显式设置了 `__ONEWORKS_PROJECT_CONFIG_DIR__`，则会从该目录读取 marketplace 配置。
...
- 第一次 `oneworks` 创建新会话时，One Works 会自动把它安装到 project home 的 `.local/plugins/<adapter>/<slug>/install`
- `syncOnRun: true` 时，每次创建新会话前都会按 marketplace 重新同步一次
- `syncOnRun: false` 或不写时，只会在缺失时自动补装，不会每次强制更新
- 插件自己的运行时数据会使用 project home 下的 `.local/plugins/<adapter>/<slug>/data`
...
如果你已经在 `plugins` 里声明了它们，也可以不手动执行这些安装命令，直接运行 `oneworks` 让项目在启动时自动同步。

如果你不想依赖整个外部 marketplace，也可以用 `options.source.source: settings` 在项目配置里内联一个最小 catalog，只声明你真正需要的几个插件。

更完整的 plugin marketplace 配置示例见 [Marketplace 详细示例](./native-plugins/marketplaces.md)。

## 用户视角下的行为

执行 `oneworks plugin --adapter claude add ...` 之后，你可以把它理解成两件事：

1. 这个 Claude 插件被安装进当前项目对应的 project home
2. One Works 会自动抹平 Claude 原生插件格式和 adapter 运行时之间的差异

效果上：

- Claude adapter 运行时会自动 stage 并启用对应原生插件
- 转换后的 `oneworks/` 目录不会自动进入 One Works runtime；需要 runtime plugin 能力时，在 `plugins` 里显式声明该目录
- `${CLAUDE_PLUGIN_DATA}` 会指向 project home 下的私有数据目录，不会把插件运行数据写回仓库
- `.local/plugins/<adapter>/<slug>/install/oneworks` 里的转换资产保留 `${CLAUDE_PLUGIN_*}` 占位符，运行时再按当前 workspace 解析，避免把用户 home 绝对路径写进项目快照
- `.oo/plugins` 本身只适合作为用户显式声明的普通本地插件目录，不承担 adapter-native managed 安装语义
- 如果插件是在 `marketplaces.<name>.plugins` 里声明的，`oneworks` 也会在启动时自动补装或同步它
...
- Claude 插件如果声明 `userConfig`，安装会被拒绝；One Works 还没有把 marketplace 选项映射到这类插件配置
