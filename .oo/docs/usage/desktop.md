...
桌面端会记录启动开始、主要阶段、首屏可用、稳定运行、超时、降级与失败等隐私安全诊断事实。需要支持时，从 `Help -> Export Diagnostic Support Bundle...` 导出支持包；它不会包含提示词、凭据、项目路径、原始日志或错误栈。标准 OTel 接入与 Admin 分析见[诊断、遥测与支持包](./diagnostics.md)。
...
桌面安装包默认携带并启用 OneWorks 的 Relay、内置浏览器控制（IAB）、浏览器控制和电脑控制 - CUA；因此打开空目录时也能使用这些插件，不依赖源码仓库的 `node_modules` 或项目插件配置。项目配置仍可显式停用或覆盖对应插件。设置页的“外部控制”分组统一提供外部浏览器配对与 CUA 行为设置。IAB 只控制 OneWorks 自己的内置浏览器页签；浏览器控制当前仅支持 Chrome，使用扩展能力前必须显式完成浏览器配对。CUA 真实电脑操作仍要求 macOS 辅助功能和屏幕录制授权。
