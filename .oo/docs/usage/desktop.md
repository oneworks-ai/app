...
桌面安装包默认携带并启用 OneWorks 的 Relay、内置浏览器驱动（IAB）、外部 Chrome 驱动和 Cua 驱动；因此打开空目录时也能使用这些插件，不依赖源码仓库的 `node_modules` 或项目插件配置。项目配置仍可显式停用或覆盖对应插件。设置页的“外部控制”分组统一提供外部 Chrome 配对与 CUA 行为设置。IAB 只控制 OneWorks 自己的内置浏览器页签；Chrome 扩展能力与 Cua 真实电脑操作仍分别要求用户显式完成浏览器配对以及 macOS 辅助功能和屏幕录制授权。
