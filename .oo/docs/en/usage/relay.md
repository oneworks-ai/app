...
Relay can also ingest standard OTLP/HTTP JSON diagnostics from One Works and Codex, then expose global issue analysis and per-user timelines in Admin. See [Diagnostics, Telemetry, and Support Bundles](./diagnostics.md) for setup, privacy, retention, and support bundles. Diagnostic queries are part of the platform-admin OpenAPI document; diagnostic export is part of the current-user document.
