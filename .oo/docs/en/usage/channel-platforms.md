...
- If every role bot must reply independently, give each bot app its own channel key and `appId` / `appSecret`. Reuse the same entity when a role works in multiple chats, and create one ChannelLink for each `bot app × chat` binding; never reuse one channel key across different entities.
...
- Do not connect the same bot app from multiple stale worktrees or leftover servers. Feishu can deliver an event to any active persistent connection, which makes one bot reply with old configuration intermittently. Use `pnpm --silent tools dev-service status <target> --json` to identify the owner, then stop the stale target only after obtaining its stop authorization.
