...
Watched local directory plugins use `exports["./client"].source`. Development hosts load it through the host Vite server for HMR, TypeScript / TSX transforms, source maps, and React Fast Refresh. Packaged Electron and static hosts compile it into in-memory ESM with a controlled Vite build and load it through a same-origin workspace runtime proxy that preserves source-relative paths. String-based runtime dynamic imports are compiled on demand when they request another source module. Each plugin reload switches the complete source module namespace so browsers cannot reuse stale dynamic child modules. Plugin-local relative, absolute, and `file:` modules, runtime dynamic modules, and assets must stay inside the source entry directory and its descendants; requests into server or sibling local directories are rejected. Normal bare npm dependencies can still be resolved and bundled by Vite. The packaged compiler does not load plugin Vite or PostCSS configs, execute plugin build scripts, or write `dist`. It currently supports plain CSS through `?inline` imports and validates its `url()` / `@import` references. CSS Modules, Sass / Less / Stylus, and `image-set()` are not yet supported; compile them to plain CSS inside the source root. Use Vite `?inline` imports for other assets that would otherwise be emitted separately. Package/download sources, watch-disabled plugins, and published plugins continue to use `exports["./client"].default`, so published packages must still ship `dist`. Do not configure `plugin.client.devServer` for new plugins.
