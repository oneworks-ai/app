...
- `oneworks accounts remove <adapter> <accountName>`: remove One Works' adapter account record; official logout runs only when the CLI can target that account precisely, while machine-wide native logins such as a default home remain on the device
