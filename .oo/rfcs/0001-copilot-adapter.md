...
Add a first-class `copilot` adapter to One Works.
...
- The adapter parses Copilot JSONL events and emits normalized One Works `AdapterOutputEvent` values.
- One Works CLI output formatting remains outside the adapter. The adapter does not implement One Works `--output-format`; it only provides structured events for the existing CLI print layer.

The adapter uses workspace-owned config, instruction, and skill staging paths under `.oo/.mock/copilot` instead of writing managed files into the user's real Copilot home.

## Motivation

One Works adapters are runtime translators, not binary aliases. Copilot CLI has its own model flags, prompt mode, interactive TUI, MCP injection, skill discovery, permission flags, BYOK provider environment variables, and JSONL event stream.

Adding a dedicated adapter lets One Works use those native Copilot surfaces while preserving the shared task runtime:

- `ow run --adapter copilot --print ...` stays headless and script-friendly.
- `ow run --adapter copilot` can delegate the terminal to Copilot's native TUI.
- workspace assets still flow through One Works's shared asset planner.
...
- Add `@oneworks/adapter-copilot`.
- Resolve the Copilot CLI from adapter config, One Works env override, adapter-local package binary, or `PATH`.
...
- Keep One Works `--print` non-interactive. It must not open Copilot TUI, pickers, browser prompts, or terminal UI.
- Stage One Works system prompt content as Copilot custom instructions.
- Stage selected One Works skills for Copilot native skill discovery.
...
- Translate selected One Works MCP servers into `--additional-mcp-config`.
...
- Keep One Works task hook bridge active for Copilot in V1.
...
```text
copilot --resume <ow-session-id> --interactive <prompt>
```
...
```text
copilot --resume <ow-session-id> \
  --prompt <prompt> \
  --output-format json \
  --stream on
```

The adapter reads stdout as JSONL. Copilot's `--output-format json` is only the child-process protocol used between the adapter and Copilot CLI. One Works's public `--output-format` stays owned by the existing CLI print layer, which consumes adapter events.

### Session Identity

One Works `sessionId` is passed as Copilot `--resume <id>`.
...
.oo/.mock/copilot/sessions/<session-id>/instructions/
  copilot-instructions.md

.oo/.mock/copilot/sessions/<session-id>/skills/
  <selected-skill> -> <workspace-or-plugin-skill-dir>
```

Child env:

- `COPILOT_HOME=<workspace>/.oo/.mock/copilot`
- `COPILOT_AUTO_UPDATE=false`
- `COPILOT_CUSTOM_INSTRUCTIONS_DIRS=<managed instructions dir>` when One Works has a system prompt
- `COPILOT_SKILLS_DIRS=<managed skills dir>` when selected skills are staged

The adapter also passes `--config-dir <workspace>/.oo/.mock/copilot`.

By default, the adapter merges the current workspace path into `.oo/.mock/copilot/config.json` under `trusted_folders`. This suppresses Copilot's folder trust prompt for managed runs without touching the user's real `~/.copilot`. Set `adapters.copilot.disableWorkspaceTrust=true` to skip this bootstrap.

### Asset Mapping

| One Works asset          | Copilot V1 mapping                                                          |
| ------------------------ | --------------------------------------------------------------------------- |
...
| hook plugins             | handled by One Works task hook bridge, diagnostic status `translated`       |
...
Routed One Works model services use the existing `<service>,<model>` shape:
...
For GitHub-hosted Copilot models, One Works can observe the process lifecycle and Copilot JSONL session events, but the upstream API calls are owned by Copilot CLI.

For routed `modelServices`, the intended parity target is the Codex local provider proxy:

- route Copilot provider base URL through a local One Works proxy;
...
```text
.oo/logs/<ctx-id>/<session-id>/adapter-copilot/provider-proxy.log.md
```
...
| One Works mode      | Copilot mapping     |
| ------------------- | ------------------- |
...
V1 does not install a Copilot-native hook bridge. One Works hook plugins keep running through the existing task hook bridge.

Reasoning:

- Copilot CLI hooks are real, but their exact file/config behavior needs authenticated smoke coverage.
- The CLI-only stream path is process-per-turn prompt mode, so One Works should avoid promising long-lived SDK hook callback semantics.
...
| Copilot JSONL event       | One Works event         |
| ------------------------- | ----------------------- |
...
```bash
npx oneworks --adapter copilot --print "hello"
npx oneworks --adapter copilot --print --include-skill research "use the research skill"
npx oneworks --adapter copilot "open interactive mode"
```
...
- Which Copilot permission patterns are stable enough to compile One Works allow/deny rules directly?
