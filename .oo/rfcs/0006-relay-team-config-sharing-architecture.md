...
3. 汇总这些团队里可消费且未被成员或设备禁用的 profile assignment。
4. 按设备项目上下文过滤。
...
用户分组下要展示用户加入了哪些团队：用户列表支持按团队过滤，用户详情有“所属团队”区块，展示团队名、团队角色、加入时间、默认发布团队、继承的 config profiles 及启用状态；管理员可为成员配置团队配置默认启用或禁用。

团队详情页展示成员表、角色过滤、配置启用状态、归档操作和审计摘要。拥有 `team.members.write` 的 owner/admin 可以直接在团队界面邀请用户、调整角色、配置成员启停或移除成员；邀请入口不应该藏到全局用户管理页。配置 profile 页展示版本历史、assignment 表、发布/禁用和 secret rotation。

Admin 列表页必须遵循现有表格约定：搜索、过滤、列配置、批量操作入口、固定表头、底部分页、图标操作按钮，以及主识别字段上的详情链接。

## Relay 插件界面

当前远端配置来源 team/profile/version、分享草稿构建器、团队选择、安全字段预览、API key 警告、密钥模式选择、按团队/profile 启停当前用户或设备配置，以及带过期和撤销警告的 refresh/apply 状态。
...
- profile assignment 落地后，Admin 应停止暴露 raw assignment JSON 编辑。
- Snapshot contract 需要对现有 Relay 插件版本保持向后兼容，继续保留 `assignments`、`hash`、`version` 和安全配置字段。
