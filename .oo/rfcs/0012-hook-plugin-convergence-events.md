...
派发语义、约束表与 API 形状见[通用事件 API 设计](0012-hook-plugin-convergence-events-api.md)。共六个 mode：Cordis 的 `emit` / `parallel` / `serial` / `bail` / `waterfall` 全部保留（命名不改，与 DSH 对齐），另加我们特有的 `decide`。

约束不落在词汇表上，而落在事件定义上——`emit` 不可跨进程；`security: true` 的事件只能用 `decide`。`define` 时机械校验。

Mode 是事件定义的一等字段，插件作者不需要从名字推断，宿主据此决定如何派发与合并。

**与 DSH 唯一的 mode 分歧**：DSH 把 `tools/pre-execute` 标为 `waterfall`，我们标为 `decide`（该事件 `security: true`）。DSH 的插件等同 shell 权限所以 waterfall 无妨；我们是 marketplace 分发，必须保证插件只能收紧。
...
| 现有名                 | 新名                     | mode       | 可用性 | 备注                                                                                                  |
| ---------------------- | ------------------------ | ---------- | ------ | ----------------------------------------------------------------------------------------------------- |
| `PreToolUse`           | `tools/pre-execute`      | **decide** | both   | 名称对齐；mode 刻意分歧（DSH 为 waterfall），因该事件 `security: true`                                |
| `PostToolUse`          | `tools/post-execute`     | waterfall  | both   | 与 DSH 完全对齐                                                                                       |
| `GenerateSystemPrompt` | `system-prompt/assemble` | waterfall  | both   | 与 DSH 完全对齐                                                                                       |
| `Stop`                 | `agent/turn-stopping`    | serial     | both   | 与 DSH 完全对齐                                                                                       |
| `StopFailure`          | `agent/error`            | emit       | both   | 对齐                                                                                                  |
| `SubagentStop`         | `subagent/end`           | emit       | both   | 对齐                                                                                                  |
| `SessionStart`         | `agent/session-start`    | emit       | both   | 对齐                                                                                                  |
| `SessionEnd`           | `session/disposed`       | emit       | both   | 对齐                                                                                                  |
| `UserPromptSubmit`     | `agent/prompt-submit`    | waterfall  | both   | **无 1:1 对应**。DSH 最近的 `agent/pre-step` 语义更宽（每步触发）。用自有名字但守同一风格，不假装对齐 |
| `PreCompact`           | `compaction/pre`         | waterfall  | both   | **DSH 无此事件**（它走 `ctx.compaction` 服务）。我们粒度更细，保留                                    |
| `Notification`         | `agent/notification`     | emit       | both   | 我们自有                                                                                              |
| `TaskStart`            | `task/started`           | emit       | both   | 我们自有（适配器概念）                                                                                |
| `TaskStop`             | `task/stopped`           | emit       | both   | 我们自有                                                                                              |
| `StartTasks`           | `task/batch-start`       | emit       | both   | 我们自有                                                                                              |

## 新增：建议补的四个点位

这四个是与 DSH 对照后确认的高价值缺口。共同特征是它们都在**模型请求那一层**或**工具执行的环绕层**，我们当前完全没有对应物。

### `agent/request` — waterfall — 可用性 `bridge`
...
### `agent/request-error` — waterfall — 可用性 `bridge`
...
| `tools/change` / `skills/change` / `commands/change` / `system-prompt/change` | emit   | 能力面变更通知，插件可感知工具集变化                                                                    |
...
- `mode` —— `emit | parallel | serial | bail | waterfall | decide`
- `availability` —— 传输可达性（`in-process | cross-process`）与 source 分级（`both | bridge | native:<adapter>`）
- `security` —— 该事件结果是否影响权限或安全边界
- `payload` —— 结构化 schema
- `result` —— `waterfall` / `bail` 的返回契约、`decide` 的判定格；`emit` / `parallel` / `serial` 此字段为空
- `summary` —— 一句话语义
