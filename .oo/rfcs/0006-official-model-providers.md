...
One Works 继续保留 `modelServices` 作为用户配置模型路由的入口，但在它上面增加一层“官方服务商”能力。一个服务仍然可以是任意自定义 OpenAI 兼容端点，也可以声明为 OpenAI、Claude、Kimi、DeepSeek、MiniMax、阿里千问或智谱 GLM 这类已知官方平台。
