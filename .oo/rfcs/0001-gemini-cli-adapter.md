...
`@oneworks/adapter-gemini` package. The adapter should run Gemini CLI in
...
- Parse Gemini JSONL events into One Works messages, tool calls, errors, init
  events, and exit events.
- Preserve Gemini session IDs in One Works cache so follow-up turns can resume
...
- Install a Gemini native hook bridge for the shared One Works hook runtime.
...
- Do not enable or manage Gemini extensions / extension marketplace support by
  default.
- Do not make Gemini checkpoint restore part of One Works session branching in
  the first pass.
...
- Do not map Gemini `BeforeModel`, `AfterModel`, or `BeforeToolSelection` into
  One Works hooks until the core adapter is stable.
...
- `@oneworks/types` owns adapter contracts and adapter package resolution.
- `@oneworks/task` owns task lifecycle, effective model/effort resolution,
  adapter asset plan attachment, and native/managed hook bridge de-duplication.
- `@oneworks/workspace-assets` owns workspace asset bundle discovery and the
  adapter asset plan.
...
not support One Works effort in V1.
...
- `./hook-bridge` for `@oneworks/hooks`;
...
- `settings`: merged into generated Gemini settings after One Works managed
  fields, with One Works managed fields winning where needed for hooks/MCP.
...
This keeps the user-facing configuration aligned with the rest of One Works:
...
history, policy, and related runtime files. One Works also uses a mock home for
...
`GEMINI_CLI_HOME=.oo/.mock` over changing `HOME`, because changing `HOME` affects
tools and child processes beyond Gemini's own config discovery.

V1 should use a hybrid mock-home strategy:

1. Spawn Gemini with `GEMINI_CLI_HOME=.oo/.mock` so One Works can manage
   `.oo/.mock/.gemini/settings.json` deterministically.
...
5. Set `__ONEWORKS_PROJECT_REAL_HOME__` so hooks and diagnostics can explain whether
...
- Generate the managed mock-home `.oo/.mock/.gemini/settings.json`.
...
| `GEMINI_MODEL`                                  | Gemini default model env                        | Preserve, but always pass adapter-resolved `--model` so ambient env cannot override One Works model selection.                         |
...
| `GEMINI_CLI_SURFACE`                            | User-Agent/reporting label                      | Consider setting to a One Works-specific value or allow override.                                                                      |
...
| `experimental.gemmaModelRouter.classifier.host` | Local Gemma router endpoint in `settings.json`  | Allow via raw `adapters.gemini.settings`, but do not expose as One Works external model support.                                       |

The adapter should merge environment in this order:

1. base process environment from `ctx.env`;
2. One Works managed environment such as `GEMINI_CLI_HOME`, hook bridge vars,
...
- When One Works resolves a routed `service,model`, generate
...
One Works already starts a fresh process per turn, the adapter should keep the
...
mode is already triggered by One Works's piped stdio, but the adapter may also
...
content to appear in argv. One Works can still enforce its own sandbox outside
...
use native command syntax for One Works assets. A future
...
- Only one Gemini process may run per One Works session at a time.
...
The key is One Works `sessionId`; `geminiSessionId` comes from Gemini's
`session_id` in the `init` event.

Gemini also persists full native chat history outside the One Works cache. With
`GEMINI_CLI_HOME=.oo/.mock`, the pinned CLI stores project-scoped sessions under
`.oo/.mock/.gemini/tmp/<project_id>/chats/`, plus related project temp data such
...
Gemini-specific retention config until One Works has a cross-adapter retention
...
sessions while One Works still has a cache mapping.
...
| Gemini exit code | One Works handling                                       |
| ---------------- | -------------------------------------------------------- |
| `0`              | normal `exit`                                            |
...
- One Works normalized tool name;
- call id;
...
Map One Works permission modes to Gemini approval modes:

| One Works mode      | Gemini mode | Notes                                                                                      |
| ------------------- | ----------- | ------------------------------------------------------------------------------------------ |
...
in headless mode, and One Works's runtime uses non-TTY stdio. Interactive
...
V1 should not use Gemini's policy engine as the primary One Works permission
...
auto-approve tools outside the selected One Works permission mode. Restrictive
...
diagnostics rather than silently weakening One Works's own permission display.
...
- Reuse the existing `.oo/.mock/permission-state/<adapter>/<session>.json`
...
- Keep `trust: true` out of generated runtime MCP config unless explicitly
  configured, so remembered One Works permissions do not get bypassed by
  adapter defaults.
...
not an append mechanism. Replacing it with One Works generated context would
...
1. Convert `AdapterQueryOptions.systemPrompt` into a clearly delimited
   "One Works runtime instructions" prelude.
...
- Diagnose `context.includeDirectories` and
  ...
  settings. A future feature can map One Works workspace roots into these
  fields deliberately.
...
- Convert selected One Works MCP servers into Gemini `mcpServers`.
...
- If activation is blocked in safe modes, V1 should surface a clear diagnostic
  and continue relying on One Works prompt assets for selected skills.
- Extension-provided skills are outside V1 because extensions are disabled by
  default.

Hooks:

- Generate `.oo/.mock/.gemini/settings.json` with managed hook entries.
- Set `__ONEWORKS_PROJECT_GEMINI_NATIVE_HOOKS_AVAILABLE__=1` when hooks are
...
- Do not enable Gemini browser/devtools agent features by default. If user
  settings enable browser agents, One Works should preserve them but log that
...
Expose a `builtinModels` list in `@oneworks/adapter-gemini/models`:
...
the One Works-resolved model through `--model`, including the default `auto`,
so ambient `GEMINI_MODEL` or workspace `settings.model.name` cannot silently
change a One Works session. If users want ambient Gemini model selection, they
can set `adapters.gemini.defaultModel` or choose a Gemini model in the normal
One Works model selector.
...
stable public effort/thinking flag that maps to One Works `EffortLevel`.

### External Model Services

One Works supports external model services through `modelServices` and
...
chat model or One Works `modelServices`.
...
- Add `@oneworks/adapter-gemini` to `apps/client/package.json`.
...
- Add Gemini to asset-plan-supported adapters, but not effort-supported
  ...
  `supportedEffortAdapters` in `@oneworks/task`.
...
- Add Gemini native hook bridge de-duplication for
  ...
  of One Works's de-dup contract.
...
- `hook-bridge.ts`: Gemini native payload to One Works hook input and output
  conversion.
...
- `@oneworks/task` attaches Gemini asset plans.
...
- Ambient `GEMINI_MODEL` and workspace `settings.model.name` do not override
  the model selected through One Works.
...
Always passing the resolved One Works model prevents an inherited shell
variable or workspace setting from silently changing the selected model.

### External model mismatch

One Works model services are usually OpenAI-compatible provider definitions,
...
old sessions independently of One Works's cache mapping. The adapter should
handle stale native IDs explicitly so a resumed One Works session never attaches
...
writing, and surface a One Works input-size error before Gemini truncates.
...
before the model sees it. This can bypass One Works's prompt assembly, run custom
...
should diagnose or reject those expansion surfaces until One Works deliberately
maps them.
...
policies can hide or deny tools that One Works expects to display. V1 should not
...
credential storage that One Works V1 does not expose. V1 should disable
...
Gemini can emit native lifecycle hooks that overlap with One Works's framework
...
as a workspace dependency. This keeps One Works lighter and avoids optional
...
- Should a future interactive Gemini adapter mode expose folder trust controls,
  or should One Works keep Gemini strictly headless?
...
- Should Gemini native prompt command syntax stay rejected by default, or should
  One Works expose an explicit mode for slash commands and `@` file injection?
...
- Should `GEMINI_CLI_NO_RELAUNCH=true` remain a managed invariant, or should a
  ...
  cleanup?
- Should One Works own Gemini native session retention, or leave
...
- Should One Works ever add a generic adapter `apiHost` field, or should each
  adapter keep provider-specific endpoint knobs isolated?
- Should `GEMINI_CLI_HOME` fully replace `HOME=.oo/.mock` for Gemini, or should
...
- Should a future version use Gemini policy files for durable One Works
...
- Should Gemini MCP server names with underscores be hard errors, warnings, or
  adapter diagnostics?
- Should One Works expose Gemini's `--allowed-mcp-server-names` and policy
...
- Should Gemini `BeforeToolSelection` become a new shared One Works hook event?
- How should Gemini plan approval artifacts map to One Works plans, if at all?
...
- Should Gemini subagents remain disabled permanently in V1, or should One Works
  expose a native subagent mode after remote/local tool scopes are mapped?
- Should committed workspace `GEMINI.md` always be preserved, or should One Works
...
- Should Gemini native `SessionEnd` stay framework-owned permanently, or should
  One Works add native `SessionEnd` de-duplication across all adapters?
