...
- `pnpm start` / `pnpm tools dev-start web` 默认把 server DB、server data、runtime store、logs、caches 和 mock home 放到 home project 目录：`~/.oneworks/projects/<project-key>/`。如果确实要共享或指定外部位置，可以在启动前显式传入 `DB_PATH`、`__ONEWORKS_PROJECT_SERVER_DATA_DIR__`、`__ONEWORKS_PROJECT_SERVER_LOG_DIR__` 或对应 project path 环境变量。
