...
- Claude Code 会叠加项目级和 mock home 两套 settings；排查重复 hook 时不能只看 `<project-home>/.mock/.claude/settings.json`。
...
- Codex transcript 注入型 e2e 要先清空 `<project-home>/.mock/.codex/sessions/`，再去找当前 run 生成的 transcript；否则 suite 内多 case 连跑时很容易命中旧文件。
