---
alwaysApply: false
description: 当任务涉及 benchmark 目录结构、执行模型、judge 规则和结果格式时加载的 benchmark 方案说明。
---
...
- [目录与输入](./layout.md)：case 布局、输入文件和目录约定。
- [执行模型](./execution.md)：category、workspace、执行流程和并发。
- [判定与结果](./evaluation.md)：judge、评分、状态和 `result.json`。

## 目标

- 评测重点不是模型输出文本，而是模型是否能在真实代码仓库中完成可验证的工程改动。
- 固定输入为 `rfc.md`、`patch.diff`、`patch.test.diff`。
- 固定输出为 `.oo/results/<category>/<title>/result.json`。
