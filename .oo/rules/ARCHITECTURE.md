...
- [启动链路](./architecture/runtime-flows.md)：`ow`、`oneworks-mcp`、`oneworks-call-hook` 与默认 MCP。
- [Server 约定](./architecture/server.md)：传输层、services、db 的职责边界。
- [配置分层](./CONFIG.md)：global / project / user 的读取、合并、关闭全局开关与写回边界。

## 核心边界

- `apps/*` 是入口层，不重复实现共享 runtime。
- `apps/client` 只依赖共享契约和展示元数据，不直接触达文件系统和任务运行时。
- `apps/cli` 与 `apps/server` 复用 `@oneworks/app-runtime` 暴露的 task / benchmark 入口。
