---
alwaysApply: false
description: 当任务需要评估当前架构治理事项、未完成重构边界或技术债时加载的待办清单。
---
...
- `schema` 继续下沉到底层共享包
  - 当前很多 schema 仍然留在运行时包里，例如 benchmark 的 [schema.ts](../../packages/benchmark/src/schema.ts)
  - 目标是让 `@oneworks/types` 承载共享 contract 与 schema，运行时包只保留实现和 orchestration
- `workspace-assets` 继续收敛成纯 asset 实现层
  - 当前 contract 已大幅下沉到 `@oneworks/types`
  - definition loader 已独立到 `@oneworks/definition-loader`
  - 后续优先检查 prompt asset 选择与 adapter asset plan 是否还值得继续拆层
- cache 的文件存储原语继续收敛
  - 当前 cache 已下沉到 `@oneworks/utils`
  - 后续可以继续检查是否值得把 JSON 文件存储原语和 task cache 路径约定拆开
- `benchmark` 继续拆分 schema 与 runtime
  - 当前 benchmark 共享类型已下沉到 `@oneworks/types`
  - 但 schema 仍在 `@oneworks/benchmark`
...
- 排查 Claude mock 会话卡在 `UserPromptSubmit` 后不再继续返回
  - 当前用 `hook-smoke-mock-ccr` 跑 `claude-code` 冒烟时，session log 只到 `UserPromptSubmit`，没有继续进入 `Stop` / `SessionEnd`
  - 需要区分问题位于 mock model service、Claude Code Router、Claude CLI stream 链路，还是 hook bridge / 日志观测本身
