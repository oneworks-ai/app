...
本文件只描述如何参与本仓库开发；如何把 One Works 接入你的项目，请见 [USAGE.md](../USAGE.md)。
...
`pnpm start` 等价于 `pnpm tools dev-start web`，会分别启动：

- `oneworks-server`（后端）
- Vite client（前端）

日志默认输出到当前 workspace 的 project home：`~/.oneworks/projects/<project-key>/logs/server/`。

如果需求是“拉取最新代码，并启动一个 web 服务”或其他开发服务，优先直接运行：

```bash
pnpm tools dev-start <target>
```

target 映射：`web`、`electron`、`electron-workspace`、`pwa`、`homepage`、`docs`。该快路径会自动完成 fetch / 安全拉取、缺依赖时安装、后台启动、端口避让和探活，并输出可访问 URL 或 PID。

### Worktree 启动补充

- 当前 worktree 如果没有本地 `.env` / `.env.dev`，runtime 会回退读取主 worktree 的对应文件。
- 通过 One Works CLI / Server / Web 入口启动时，config loader 会先读取真实用户 Home 默认目录下的全局 `~/.oneworks/.oo.config.json`，作为跨项目个人默认配置。
- 当前 worktree 如果没有本地 `.oo.dev.config.json` / `.oo.dev.config.yaml`，config loader 会回退读取主 worktree 的 dev config。
- 配置合并顺序是全局 `~/.oneworks/.oo.config.json` < workspace `.oo.config.*` < workspace / 主 worktree `.oo.dev.config.*`；越靠后的层级优先级越高。
- 配置里的 `disableGlobalConfig: true` 会让当前解析结果跳过全局配置层；脚本级隔离可以设置 `__ONEWORKS_PROJECT_DISABLE_GLOBAL_CONFIG__=1` 完全不读取全局配置文件。
- 回退依据是 Git common dir，也就是和当前 worktree 绑定的主仓库；这样可以避免每次新建 worktree 都重复复制本地私有配置。
- 如果当前 worktree 有自己的 `.env` 或 `.oo.dev.config.*`，优先使用当前 worktree 的文件，不会被主 worktree 覆盖。
- 如果主 worktree 也没有这些本地文件，仍然需要你自己补齐，或者显式导出所需环境变量。
- `pnpm start` / `pnpm tools dev-start web` 默认把 server DB、server data、runtime store、logs、caches 和 mock home 放到当前 dev-start 实例隔离目录：`~/.oneworks/dev-instances/<worktree-key>/projects/<project-key>/`。同一个源码的多个 worktree 会使用不同 `<worktree-key>`，避免本地并行开发时共享 DB、workspace instance lock、runtime sessions 和 mock home。
- 如果确实要跨 worktree 共享或指定外部位置，可以在启动前显式传入 `__ONEWORKS_PROJECT_HOME_PROJECTS_DIR__`、`DB_PATH`、`__ONEWORKS_PROJECT_SERVER_DATA_DIR__`、`__ONEWORKS_PROJECT_SERVER_LOG_DIR__` 或对应 project path 环境变量。
- 新建或切换到 worktree 后，先在当前 worktree 根目录执行一次 `pnpm install`，不要复用别的 worktree 的依赖状态。
- `pnpm tools` 通过 `scripts/run-tools.mjs` 注册 TS；如果新 worktree 缺少 register 依赖，会先执行 `pnpm install`。`dev-start` 启动前会先做只读的 workspace 安装校验；如果当前 worktree 缺少 `node_modules` 状态文件或 workspace 链接，会自动执行 `pnpm install`。
- 在 Codex worktree 中开发时，优先先切到一个和当前主线任务一致的分支名；不要长期停留在 detached HEAD，也不要为顺手修的小点单独起支线分支名。
- 开发态前端标题会显示当前 git ref，用来区分多个 worktree；如果刚切了分支，需要重启 `pnpm tools dev-start web` 后再刷新页面，标题才会更新。

### Worktree 启动前检查

- 确认主 worktree 的 `.oo.dev.config.json` 里引用的 model service / adapter 配置仍然有效。
- 确认主 worktree 的 `.env` 里存在对应密钥，例如 `BYTE_DANCE_GPT_API_KEY`、`BYTE_DANCE_ARK_API_KEY`。
- 如果刚修改了 `.env` 或 `.oo.dev.config.*`，要重启后端进程；只刷新前端页面不会让子进程重新加载配置。
- `pnpm tools dev-start` 会在启动前检查 server / client 端口；如果默认端口已被其他 worktree 占用，会自动使用下一个可用端口。显式传入 `__ONEWORKS_PROJECT_SERVER_PORT__` 或 `__ONEWORKS_PROJECT_CLIENT_PORT__` 时，如果端口被占用则直接报错，避免脚本调用方误以为绑定到了指定端口。
...
- 再核对 `/api/sessions` 是否只包含当前 worktree 创建的会话；如果出现其他 worktree 的会话，优先检查当前进程是否没有通过 `pnpm tools dev-start` 启动，或是否显式传入了共享的 `DB_PATH` / `__ONEWORKS_PROJECT_HOME_PROJECT_DIR__` 等 project path 覆盖。
...
- 如果改动涉及 adapter 配置页或账号管理 UI，除了真实 Chrome 回归，还要同时检查 CLI `oneworks accounts` 行为和 Web 配置页是否一致；注意当前 Web 账号详情可能使用 adapter 自己的 quota 缓存，而 CLI `oneworks accounts show` 可以是强制刷新。
...
npx oneworks accounts show codex <account>
npx oneworks-mcp --help
...
- Homepage / Docs 工作区维护说明：[`maintenance/homepage-docs.md`](../maintenance/homepage-docs.md)
