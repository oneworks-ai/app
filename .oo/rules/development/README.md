...
- `pnpm --silent tools dev-service ensure web --json` 默认把 server DB、server data、runtime store、logs、caches 和 mock home 放到当前开发实例隔离目录：`~/.oneworks/dev-instances/<worktree-key>/projects/<project-key>/`。同一个源码的多个 worktree 会使用不同 `<worktree-key>`，避免本地并行开发时共享 DB、workspace instance lock、runtime sessions 和 mock home。
...
- `pnpm tools` 通过 `scripts/run-tools.mjs` 注册 TS；如果新 worktree 缺少 register 依赖，会先执行 `pnpm install`。`dev-service ensure` 启动前会先做 workspace 安装校验；如果当前 worktree 缺少 `node_modules` 状态文件或 workspace 链接，会自动执行 `pnpm install`。
- 在 Codex worktree 中开发时，优先先切到一个和当前主线任务一致的分支名；不要长期停留在 detached HEAD，也不要为顺手修的小点单独起支线分支名。
- 开发态前端标题会显示当前 git ref，用来区分多个 worktree；如果刚切了分支，需要用户显式授权后运行 `pnpm --silent tools dev-service restart web --json`，再刷新页面，标题才会更新。
...
- `pnpm --silent tools dev-service ensure web --json` 会在启动前检查 server / client 端口；如果默认端口已被其他 worktree 占用，会自动使用下一个可用端口。显式传入 `__ONEWORKS_PROJECT_SERVER_PORT__` 或 `__ONEWORKS_PROJECT_CLIENT_PORT__` 时，如果端口被占用则直接报错，避免脚本调用方误以为绑定到了指定端口。
...
- 再核对 `/api/sessions` 是否只包含当前 worktree 创建的会话；如果出现其他 worktree 的会话，优先检查当前进程是否没有通过 `pnpm --silent tools dev-service ensure web --json` 启动，或是否显式传入了共享的 `DB_PATH` / `__ONEWORKS_PROJECT_HOME_PROJECT_DIR__` 等 project path 覆盖。
