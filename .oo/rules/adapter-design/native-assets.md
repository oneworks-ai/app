...
| `cursor`      | session 级 `adapter-cursor/home/.cursor/hooks.json`                                  | session 级 `adapter-cursor/home/.cursor/skills/<name>`                                                                               | session 级 `adapter-cursor/home/.cursor/mcp.json`                 | 隔离 `HOME` / `CURSOR_CONFIG_DIR` / `CURSOR_DATA_DIR` / `CURSOR_AGENT_STORE_DIR`；system prompt 写成 always-on Cursor rule；原生 chat id 缓存后用 `--resume` 续聊                                                                                                                                                                                                                   |
