...
adapter 暴露 reasoning effort 时，必须显式定义原生支持集和降级映射，并保持配置 schema、`AdapterMap` 类型、模型能力元数据与最终 CLI / RPC 参数一致。共享层存在但原生 runtime 不接受的档位必须在 adapter 边界规范化，不能原样透传；至少覆盖原生最高档和共享层更高档的映射测试。
...
| `codex`       | `<project-home>/.mock/.codex/hooks.json`                                                                                | `<project-home>/.mock/.agents/skills -> .oo/skills`，并把每个 workspace skill 目录软链到 `<project-home>/.mock/.codex/skills/<name>` | 翻译成 `-c mcp_servers.<name>.*`                                  | managed `config.toml` 写 workspace trust 与 update-check 默认值；账号 auth 快照落到 `<project-home>/.local/adapters/codex/accounts/<key>/auth.json`，账号元数据与 quota 快照落到 `meta.json`；session HOME 隔离到 `<project-home>/caches/<ctx>/<session>/adapter-codex-home`，继承通用 mock home bridge 但不继承整棵 `.codex`，只显式链接 auth / config / hooks / skills / sessions |
...
| `cursor`      | session 级 `adapter-cursor/config/hooks.json`                                                                           | session 级 `adapter-cursor/config/skills/<name>`                                                                                     | session 级 `adapter-cursor/config/mcp.json`                       | 隔离 `HOME` / `CURSOR_CONFIG_DIR` / `CURSOR_DATA_DIR` / `CURSOR_AGENT_STORE_DIR`；system prompt 写成 always-on Cursor rule；原生 chat id 缓存后用 `--resume` 续聊                                                                                                                                                                                                                   |
...
| `grok`        | session `$GROK_HOME/hooks/oneworks.json` 里的 `PreToolUse` / `PostToolUse` / `Stop`                                     | session `$GROK_HOME/skills/<name>`                                                                                                   | 写进 session `config.toml` 的 `mcp_servers`                       | session `$GROK_HOME` 隔离 auth/config/sessions；managed npm launcher 的原生 binary 只在共享 runtime home 展开一次；`modelServices` 写成 native custom model                                                                                                                                                                                                                         |
| `junie`       | session `adapter-junie/config/oneworks.json` 里的 `SessionStart` / `PreToolUse` / `Stop` / `StopFailure` / `SessionEnd` | session `adapter-junie/assets/skills/<name>`；agents 同级投影到 `assets/agents`                                                      | session `adapter-junie/assets/mcp/mcp.json`                       | 隔离 `HOME` / XDG / `JUNIE_DATA`；system prompt、rules 与 plan fallback 写入 session guidelines；`json-stream` 成功后缓存原生 session id，再用 `--session-id=<id> --resume` 续聊                                                                                                                                                                                                    |
...
| `pi`          | normalized RPC hook / tool events 与 managed permission extension                                                       | 选中的 skills 通过显式 `--skill` 注入                                                                                                | skipped（尚无稳定原生 seam）                                      | managed `@earendil-works/pi-coding-agent` RPC；`PI_CODING_AGENT_DIR` 使用 durable private/native 或 session generated 配置；native extensions opt-in                                                                                                                                                                                                                                |
| `qwen-code`   | session `QWEN_HOME/settings.json` 的 native hooks / `hooksConfig`                                                       | session `QWEN_HOME/skills/<name>`                                                                                                    | `settings.json.mcpServers`                                        | `HOME` / `QWEN_HOME` / `QWEN_RUNTIME_DIR` 按 One Works session 隔离；system prompt 写 `ONEWORKS.md`；不链接真实 QWEN_HOME 或凭据文件；仅验证 OpenAI Chat Completions routed model                                                                                                                                                                                                   |
...
| `cursor`      | official Cursor Agent CLI archive                  | 依赖 Cursor 原生登录态 / keychain bridge                              | 使用 Cursor 原生 model / endpoint / headers 配置                               | Cursor 原生 session / prompt / tool / compact / stop hooks                                                           | session `mcp.json`           | session skills；local plugins 通过 `--plugin-dir`                               |
...
| `grok`        | managed npm `@xai-official/grok`                   | 依赖 Grok 原生 `auth.json`                                            | `service,model` 写成 native custom model，支持 chat/responses/messages backend | Grok native `PreToolUse` / `PostToolUse` / `Stop`；只去重这三类 bridge 事件                                          | `config.toml.mcp_servers`    | session `$GROK_HOME/skills`；OpenCode overlay 跳过                              |
| `junie`       | managed npm `@jetbrains/junie`；也支持 system/path | 官方系统凭据存储或显式 allowlisted Junie/provider env；不复制用户目录 | 不接 One Works `modelServices`；只使用 Junie 原生 model/provider/BYOK          | 原生 `SessionStart` / `PreToolUse` / `Stop` / `StopFailure` / `SessionEnd`；native active 时只去重这五类 bridge 事件 | session `mcp.json`           | session skills / agents；rules 与 system prompt 走 guidelines                   |
...
| `pi`          | managed npm `@earendil-works/pi-coding-agent`      | durable private/native auth 或 session generated agent dir            | native/default model reuse；generated services 写 session agent dir            | normalized RPC hook / tool events；managed permission extension                                                      | skipped（尚无稳定原生 seam） | selected skills 显式 `--skill`；native extensions opt-in                        |
| `qwen-code`   | managed npm `@qwen-code/qwen-code`                 | 仅环境变量 / model service bridge；不复制或软链真实凭据               | 只支持已验证的 OpenAI Chat Completions `service,model`                         | Qwen native session/prompt/tool/stop/subagent/compact hooks；去重 framework 事件                                     | `settings.json.mcpServers`   | session `QWEN_HOME/skills`；OpenCode overlay 跳过                               |
...
- 账号快照和额度探测在 [`packages/adapters/codex/src/runtime/accounts.ts`](../../../packages/adapters/codex/src/runtime/accounts.ts) 收口：`~/.codex/auth.json` 会导入到 `<project-home>/.local/adapters/codex/accounts/`，运行时按会话切换到对应 auth 快照；session HOME 不能桥接整棵共享 `.codex`，避免把 Codex 全局 plugin/cache/worktree/app-state 带进一次 `app-server` 启动
...
- init 阶段在 [`packages/adapters/cursor/src/runtime/init.ts`](../../../packages/adapters/cursor/src/runtime/init.ts) 定位系统 CLI 或从 Cursor 官方归档包安装托管 CLI，并预备 native hook bridge
- query 阶段在 [`packages/adapters/cursor/src/runtime/shared.ts`](../../../packages/adapters/cursor/src/runtime/shared.ts) 生成隔离 HOME、skills、MCP、always-on rule、CLI config 和 hooks
- stream runtime 在 [`packages/adapters/cursor/src/runtime/session.ts`](../../../packages/adapters/cursor/src/runtime/session.ts) 消费 `stream-json`，缓存 native chat id，并在后续 turn 用 `--resume` 续聊

设计考量：

- Cursor 的 skills、MCP、rules 与 hooks 都有用户级 `.cursor` 原生落点；为避免覆盖真实配置，One Works 把它们全部投影到 session 隔离的 `CURSOR_CONFIG_DIR` / `CURSOR_DATA_DIR`
- 登录信息从真实 `cli-config.json` 合并进 session 副本；macOS keychain 只通过 session HOME 链接，Cursor 原生登录流程仍是凭据来源
- Cursor project history 的 workspace 信息编码在 `~/.cursor/projects/<encoded-workspace>` 目录名中；历史导入只接受能映射到当前项目、显式选择项目或 Cursor workspace metadata 的 transcript，并保留 `subagents/` 来源
...
- [`runtime/init.ts`](../../../packages/adapters/junie/src/runtime/init.ts) 以 minimal child-env policy 定位 managed/system/path CLI 并验证 26.8 / 2651.4；[`runtime/shared.ts`](../../../packages/adapters/junie/src/runtime/shared.ts) 把 HOME/XDG/data、config、MCP、skills、agents 与 guidelines 隔离到 session，统一拒绝 advanced args 覆盖 adapter-owned 参数，对 Linux native credential store 仅保留 shape-validated session-bus/runtime locator，并在落盘前清洗 `configContent`，不复制或回写真实 Junie home；[`runtime/session/stream.ts`](../../../packages/adapters/junie/src/runtime/session/stream.ts) 仅在验证 pinned `result` / serialized `errorCode` 终态、无后续 terminal failure 且进程正常结束后缓存原生 session id；[`runtime/native-hooks.ts`](../../../packages/adapters/junie/src/runtime/native-hooks.ts) 的 `SessionStart` / `PreToolUse` / `Stop` / `StopFailure` / `SessionEnd` 必须与 bridge suppression 完全一致，其中 `StopFailure` 是不可阻断观测。`modelServices` 与 native Plan Mode 均无已验证映射，不得伪造。
...
- init 阶段在 [`packages/adapters/grok/src/runtime/init.ts`](../../../packages/adapters/grok/src/runtime/init.ts) 托管 `@xai-official/grok`，并在 project-shared runtime home 预热 npm launcher 下载的原生 binary
- query 阶段在 [`packages/adapters/grok/src/runtime/config.ts`](../../../packages/adapters/grok/src/runtime/config.ts) 生成 session `$GROK_HOME`、`config.toml`、MCP、skills、凭据引用和 hook 文件
- stream runtime 在 [`packages/adapters/grok/src/runtime/session/stream.ts`](../../../packages/adapters/grok/src/runtime/session/stream.ts) 每轮调用 headless CLI，并用 `streaming-messages-json` 投影统一消息；direct runtime 保留原生 TUI
- native hooks 由 [`packages/adapters/grok/src/runtime/native-hooks.ts`](../../../packages/adapters/grok/src/runtime/native-hooks.ts) 写入 `PreToolUse` / `PostToolUse` / `Stop`，通过 [`packages/adapters/grok/src/hook-bridge.ts`](../../../packages/adapters/grok/src/hook-bridge.ts) 转成统一 hook 输入输出

设计考量：

- Grok 的 npm 包是原生 binary launcher；共享 runtime home 只展开一次 binary，session home 不重复保存大型二进制
- Grok 原生 session ID 就是 UUID，直接复用 One Works `sessionId`，首轮用 `--session-id`，后续轮次用 `--resume`
- session `$GROK_HOME` 使用 project-shared 稳定路径；resume 时会从旧 context-scoped home 或真实 `$GROK_HOME` 迁移同 UUID 的原生会话目录
- runtime-store 的 external history importer 直接读取 `sessions/*/*/summary.json` 与 `chat_history.jsonl`，支持当前项目过滤、预览与只读导入
- routed `modelServices` 写成 session native custom model，并用 session 专用环境变量注入 key，不修改真实 `~/.grok/config.toml`
- 只有 Grok 原生已经接管的 `PreToolUse` / `PostToolUse` / `Stop` 从通用 hook bridge 去重；`PreToolUse` 的 deny 结果返回原生阻断协议
