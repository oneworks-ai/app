...
1. 计算 `cwd`
2. 生成 `jsonVariables`
3. 调 [`packages/config/src/load.ts`](../../../packages/config/src/load.ts) 读取 global / project / dev config，并产出 effective project config 与 user config
4. 解析 workspace assets

## Worktree dev config fallback

当当前 worktree 没有 `.oo.dev.config.json/.yaml/.yml` 时，`loadConfig()` 会尝试回退到主工作树已有的 dev config。
...
原生 CLI 不作为 adapter package 的运行时依赖。npm 分发的 CLI 统一走 [`packages/utils/src/managed-npm-cli.ts`](../../../packages/utils/src/managed-npm-cli.ts)，Kimi 继续走 uv tool 安装；托管产物必须写到全局 bootstrap package cache，而不是项目 `.oo/caches`、真实 home 或 adapter 依赖树。

查找顺序：

1. 显式 binary path：`adapters.<name>.cli.path` 或 `__ONEWORKS_PROJECT_ADAPTER_<NAME>_CLI_PATH__`
2. 全局 bootstrap package cache：npm CLI 在 `~/.oneworks/bootstrap/npm`，Kimi uv CLI 在 `~/.oneworks/bootstrap/uv`
3. 旧项目共享 cache：`<project-home>/caches/adapter-<name>/cli` 或旧工作区 `.oo/caches/adapter-<name>/cli`（全局缺失时先移动到全局；无法移动时才兼容 fallback）
4. `source=system` 时只检查系统 `PATH`
...
CLI / client / server / hook 入口在切换 `HOME` 到 project mock home 后，会通过 [`packages/register/mock-home-bridge.js`](../../../packages/register/mock-home-bridge.js) 桥接真实 home 的 dot entries。project mock home 默认位于 `~/.oneworks/projects/<project-key>/.mock`，不写入用户本地仓库。默认策略是：

- `.cache` 直接软链到真实 home，缓存类文件复用真实用户空间。
- `.oneworks` 默认不桥接，避免 mock home 把 server 数据库或 runtime store 重新指向用户全局 home。
- macOS 下 `Library/Keychains` 和 `Library/Application Support` 直接软链到真实 home，保证使用系统 Keychain 或应用支持目录保存登录态的 CLI 在隔离 HOME 下仍能读取登录态；不要软链整棵 `Library`。
- 直接软链的桥接路径会在启动时自愈：正确软链保持不动，错误软链会被替换，已有普通文件/目录会先挪到同级 `.backup-*` 路径再创建软链，避免静默丢弃旧 mock home 数据。
- 其他 dot 目录只在 mock home 创建顶层目录，并把下一层 entry 软链到真实 home，避免 adapter 写某个子路径时必须整目录复制。
- adapter 自己管理的路径在写入前必须调用 [`unlinkMockHomeBridgePaths`](../../../packages/utils/src/mock-home.ts) 解除对应 symlink；如果父级是 symlink，先把父级物化成 mock home 下的真实目录，禁止通过 symlink 写回或删除真实 home。

新增 adapter 时不要在 CLI 侧增加 adapter 专属 denylist；应该在 adapter init / native hook / session prepare 中 claim 自己要写的路径，例如 `.codex/config.toml`、`.claude/settings.json`、`.gemini/settings.json`、`.config/opencode`。
...
- 自动生效内容：
  ...
  - `<project-home>/.mock/copilot/settings.json` 里的 workspace trust、`configContent` 和 managed native hooks
...
- `cli` 与 `configContent` 在 effective project / user 两层配置之间做深合并；这是 Copilot adapter 内部读取 `ctx.configs[0/1]` 时的显式语义，和 extends 链的 `deepMergeKeys` 互补
- `mode` 直接映射官方 `--mode`，并且官方禁止与 `--autopilot` / `--plan` 同时使用；所以 `mode` 存在时不再附加 `--autopilot` 或 `--plan`
- Copilot auth 仍由官方 CLI 自己维护，One Works 只桥接 mock home/keychain 并清晰暴露 auth 错误，不实现虚假的多账号列表
...
- 自动生效内容：
  - `GEMINI_CLI_HOME=<project-home>/.mock`
  - 托管的 `<project-home>/.mock/.gemini/settings.json`
  - managed native hooks 对应的 `hooks` / `hooksConfig.enabled`
  - selected MCP servers 映射到 Gemini `mcpServers`
  - `<project-home>/.mock/.agents/skills -> .oo/skills`
  - telemetry / auto-update / relaunch 相关受控 env
  - `__ONEWORKS_PROJECT_GEMINI_NATIVE_HOOKS_AVAILABLE__`、`__ONEWORKS_GEMINI_HOOKS_ACTIVE__` 与 Gemini native hook bridge env
