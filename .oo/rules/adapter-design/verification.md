...
```bash
npx oneworks --adapter claude --print 你好
```

### Codex

```bash
npx oneworks --adapter codex --print 你好
```

### OpenCode

```bash
npx oneworks --adapter opencode --print 你好
```

### Gemini

```bash
npx oneworks --adapter gemini --print 你好
```
...
```bash
npx oneworks --adapter claude --print --no-inject-default-system-prompt 'ow claude native skill probe'
```

预期：

- 输出命中 probe skill 的固定文本
- 说明 `.oo/skills -> <project-home>/.mock/.claude/skills` 这条链路生效
...
```bash
npx oneworks --adapter codex --print --no-inject-default-system-prompt 'ow codex native skill probe'
```

预期：

- 输出命中 probe skill 的固定文本
- 说明 `.oo/skills -> <project-home>/.mock/.agents/skills` 这条链路生效
...
```bash
npx oneworks --adapter gemini --print --no-inject-default-system-prompt 'ow gemini native skill probe'
```
...
```bash
npx oneworks --adapter copilot --print --no-inject-default-system-prompt --include-skill <probe> 'ow copilot native skill probe'
```
...
```bash
npx oneworks --adapter gemini --model kimi,kimi-k2.5 --print hi
```
...
```bash
npx oneworks --adapter copilot --model local,gpt-5 --print hi
```
...
1. 临时移走当前 worktree 的 `.oo.dev.config.json`
2. 执行：

```bash
npx oneworks --adapter claude --print 'hello from fallback test'
```
