...
- mock model service 应该临时加到 `.oo.config.json`，跑完恢复
- 这样可以与现有 `.oo.dev.config.json` 共存，不污染默认开发配置
- `ONEWORKS_MOCK_PORT`、`ONEWORKS_MOCK_API_KEY`、`BYTE_DANCE_GPT_API_KEY` 这类变量应显式设置，不要依赖本机环境碰巧可用

经验：临时配置必须可回滚。调试文档里要写“怎么接入”，但更重要的是写“跑完要恢复”。

### Worktree 本地配置回退

- worktree 下如果缺少 `.oo.dev.config.*`，loader 会回退到主 worktree 的 dev config。
...
- 如果改了主 worktree 的 `.env` 或 `.oo.dev.config.*`，需要重启服务端进程，不能只刷新浏览器。
...
- `apiBaseUrl` 指向 `http://127.0.0.1:${ONEWORKS_MOCK_PORT}/responses`
...
```bash
export ONEWORKS_MOCK_PORT=40111
export ONEWORKS_MOCK_API_KEY=mock-key
...
```bash
npx oneworks --print "trigger malformed stream" \
...
```bash
npx oneworks --print "trigger bad request" \
  --output-format stream-json \
  --adapter claude-code \
