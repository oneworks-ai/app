...
One Works 的 adapter 不是简单“换个 binary 名字”，而是负责把统一 workspace 语义翻译成各家 agent 的原生运行时：

- 统一配置：`.oo.config.*` / `.oo.dev.config.*`
...
2. 不污染用户真实 home 或本地仓库；项目运行产物落在 home 下的 project-scoped 目录（默认 `~/.oneworks/projects/<project-key>/{.mock,caches,logs,runtime}`）或 session config dir，跨项目复用的托管 package / adapter CLI 落在全局 bootstrap package cache。
...
## 为什么统一走 project mock home

- 真实 home 和本地仓库保持不变，降低本地环境污染和回归风险
- worktree、E2E、CLI smoke 都能复用同一套 home project 隔离布局
...
- OpenCode Agent Skills: [https://opencode.oo/docs/skills](https://opencode.oo/docs/skills)
