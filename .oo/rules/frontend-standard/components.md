...
- 全局或页面级 `keydown` handler 必须先判断 IME composition。组合输入期间不要响应 Enter、Esc、方向键执行、section 切换或提交操作。
- IME 判断必须复用前端共享 helper；不要在单个组件里各自维护 `event.isComposing` / `keyCode === 229` 的特殊分支。
- 快捷键录入控件属于独立交互区。页面级快捷键、shortcut reveal、tooltip reveal 都不能抢它的事件。
- 快捷键能力要可发现，但不要常驻挤占界面；优先使用延迟显示的 shortcut chip、tooltip 和 footer hint。

## 紧凑工具界面

- Command palette、launcher、sender toolbar 这类高频工具界面优先保证密度和可扫性，不要堆长说明文案。
- 当前对象的操作提示和 app 级入口要分区。不要把全局菜单、section reset、当前 item 操作混在一个视觉组里。
- `kbd + label` 结构要稳定；多个 hint 之间使用短分割线。图标型 keycap 要单独校准 glyph 视觉大小，避免图标字体默认尺寸把按钮撑大。
- tabs 表示当前上下文时，列表滚动期间应 sticky pin 在顶部，并保证背景足够实，不让下面内容影响可读性。
