...
- **卡片 / 侧栏内联 icon action**：配置卡片标题、侧栏 quick link、列表行尾这类非 route chrome 的内联工具按钮，默认对齐 `.sidebar-header__quick-link-action` 视觉语言：20px 点击槽、16px 图标、5px 圆角、透明背景、无边框无阴影，hover / focus 只把图标切到 `var(--primary-color)`。这类按钮必须是 icon-only，使用 tooltip 和 `aria-label` 说明动作；同一对象的必要快捷动作放在 `更多` 左侧，低频动作收进 `更多` 菜单。默认只在 hover / focus 时出现的操作必须覆盖在卡片或行的右侧内容层上：不得在默认态预留空 action 槽，也不得改变正文可用宽度、换行、位置或卡片几何。操作层遮住正文时必须提供与当前 surface 一致的可读背景 / 渐隐；永久可见且承担主信息结构的动作才可以参与正常布局。不要在局部写成带文字、带边框或 28px 方块按钮，除非它是明确的主操作按钮。
- **卡片网格排序动作**：只有集合确实有用户可调整的顺序时才渲染排序动作；图标和 tooltip 必须表达目标在当前可见布局中的方向。多列网格中只对同一行真实左右邻居使用左 / 右图标，不用上 / 下图标冒充横向移动；单列有上下相邻项时才使用上 / 下图标。行尾跨到下一行不是“向右”，需要跨行排序时使用拖拽或明确的目标选择。没有对应移动目标时不渲染“前移 / 后移”之类的不可达动作。响应式断点改变布局方向时，动作的语义、可达性和文案必须一起复核。
