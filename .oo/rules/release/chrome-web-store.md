...
4. 创建 GitHub environment `chrome-web-store`。Release Tags 仅在 main 首次创建 Browser Control tag 时传入 `publish_store=true`；workflow 的商店 job 必须经过该 environment，重跑已有 tag 默认不重复提交商店。
...
main 首次创建 `pkg/oneworks-plugin-external-browser-driver/v<version>` tag 时会自动 dispatch 正式提交。只有商店 job 失败或需要恢复时，才从同一 tag 手动执行：
