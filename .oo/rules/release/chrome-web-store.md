...
4. 创建 GitHub environment `chrome-web-store`。Release Tags 对 Browser Control tag 只自动生成并证明 GitHub Release，固定传入 `publish_store=false`；workflow 的商店 job 必须经过该 environment，并且只能在先查清现有 upload / submission / review 状态后，由独立 owner 对同一 tag 显式提交一次。
...
main 首次创建 `pkg/oneworks-plugin-external-browser-driver/v<version>` tag 时自动产出 GitHub Release、checksums 与 attestation，但不自动提交商店。确认没有前一版本处于 upload、pending review 或 accepted-submission 状态后，才从同一 tag 手动执行一次：

```bash
gh workflow run chrome-extension-release.yml \
  --repo oneworks-ai/app \
  --ref pkg/oneworks-plugin-external-browser-driver/v<version> \
  -f release_tag=pkg/oneworks-plugin-external-browser-driver/v<version> \
  -f publish_store=true
```

workflow 使用 `google-github-actions/auth@v3` 获取 15 分钟 access token，scope 仅为 `https://www.googleapis.com/auth/chromewebstore`。发布脚本会先重新验证 ZIP 确实是完整开发者 flavor，再执行 upload -> 异步状态轮询 -> publish；`blockOnWarnings=true`，不请求跳过 review。若 API / Dashboard 证据不完整、发现未终态前序提交，或该 tag 已有 accepted / pending submission，停止并记录为外部审核，不要重试、上传或重新提交。
