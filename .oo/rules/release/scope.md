# 发布范围与 changelog

返回入口：[RELEASE.md](../RELEASE.md)

## 范围判断

- 以最近一次版本更新或发布提交为基线做 diff。
- 只有运行时代码变更、发布元数据变更，才应计入发版范围。
- 纯测试、snapshot、AGENTS、普通文档改动默认不进入发版范围。
- 如果某个包本身不发布，不要因为测试改动把依赖闭包上的上层包一起带入发布计划。
- 先列出“候选发布包 -> 发版依据”的最小清单，再决定版本和 changelog。

## 发布类型

- 整体发布：一组 public workspace 包一起发布，在 `changelog/<version>/readme.md` 记录。
- 单包发布：只发布明确选中的包，在 `changelog/<version>/<package>.md` 记录。

## changelog 约定

- 每个版本使用一个独立目录 `<version>/`
- 整体发布记录在 `<version>/readme.md`
- 单包发布记录在 `<version>/<package>.md`
- 预发布版本（alpha / beta / rc）和正式版使用各自独立目录；不要假设预发布记录会自动折叠到后续正式版
- 发布正式版时，Release PR 需要人工整理 `changelog/<stable-version>/readme.md`，从相关预发布记录中筛选用户可感知变化并去重；预发布目录继续作为历史记录保留
- 运行时更新 UI 可以按安装版本到目标版本的区间展示多个已有 changelog entry，但这只是读取已有文件，不会生成或合并正式版 changelog
- 只记录已经发布的版本
- 聚焦用户可感知变化、兼容性影响和发布范围
- 单包记录只描述该包本次真正发布的内容，不混写其他包的变化

## PR 变更记录门禁

- 功能新增和 bug 修复类 PR，若改动产品代码，必须同步更新对应的 `changelog/<version>/readme.md` 或 `changelog/<version>/<package>.md`。
- 文档更新、测试补充、工具链升级、CI 调整等非产品行为默认不需要 changelog。
- 如果 PR 涉及新的界面交互或可见 UI 变化，PR 正文必须附带对应截图；AI 创建 PR 后也应主动生成并加入截图。
- 所有 PR 正文必须包含并勾选 `## Experience Review` checklist；需要沉淀经验时，先运行 `$post-task-experience-review` 并等待 Experience Review Result 为 `PASS`，没有可沉淀反馈时允许 `NOT APPLICABLE`。
- CI 使用 `pnpm tools pr-change-check <base> <head> --body-file <path>` 检查上述规则。

## publish-plan

- `pnpm tools publish-plan -- [args]` 用于基于显式包选择和内部依赖生成发布顺序。
- 它不是“哪些包该发布”的唯一依据；范围判断仍以前面的规则为准。
- 只做试算时，优先不要带 `--bump`。
