...
- `edit / fork` 分支会话需要记录消息分支组元数据（`messageBranchGroupId`、源 session、源 message、基准消息索引与 action），前端用这些元数据在用户消息下方渲染版本切换，让用户感知为同一条消息的多个版本；这类内部版本 session 不应出现在侧边栏会话列表里，也不应切到子 session path，当前版本状态应放在 `branchSessionId` query 上。
- 多轮分支时，`branchSessionId` 表示当前 active leaf / active timeline，而不是单条消息的局部选择；版本控件应沿 active leaf 的祖先链解析每个分支点，避免把已经被后续更早编辑截断的祖先分支点继续渲染出来。
