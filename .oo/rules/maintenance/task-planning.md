...
- 每次为 Git / PR 任务排查审批或 sandbox 问题时，先读取该任务实际加载的 `.codex/config.toml`，区分项目声明的 policy 与任务运行时真正获得的 capability；不要先归咎用户配置，也不要把某个 worktree 的配置值概括成全项目恒定事实。曾有可信项目配置明确为 `approval_policy = "never"`、`sandbox_mode = "danger-full-access"`，但新建独立任务仍显示 `waitingOnApproval`，并报告 linked Git metadata 位于可写 sandbox 之外、网络代理受限等症状。此组合应按下述 provisioning/loading mismatch 处理，而不是断言 Full Access 必然未继承或用户配置错误。
- 先查当前任务创建 API 的 schema，再选择能够承载所需上下文的线程入口。若 schema 没有 approval 或 sandbox override 字段，必须如实记录这个能力限制；不能在 prompt、fork 历史或状态标签中声称已经传入不存在的 override。项目配置和规则只会在重新加载后的任务中生效，并不自动证明 sandbox、网络或 Git metadata access 已正确 provisioned。
- 当有效配置要求非交互 Full Access（例如 `approval_policy = "never"` 与 `sandbox_mode = "danger-full-access"`），但一个全新任务仍等待批准或 sandbox 拒绝预期访问时，归类为 task provisioning/loading mismatch。记录精确的 capability gap 与可公开的症状；不要反复向用户索要已经授予的批准。修复或重新加载后最多创建一个干净、受控的验证任务，避免连续 fork 带有长协调历史的主任务。需要共享同一 worktree 时，prompt 必须明确“你就是执行者，不得再委派”。
- Git 写操作仍必须交给通过合适项目线程机制新建、且拥有可追溯授权的独立 Git operator；不要把继承父任务有效权限的 collaboration child 当作交付 operator，也不要在 provisioning/loading mismatch 时回退由主线程完成 Git 写入。只有该独立 operator 在已明确授权的受限远端操作中没有停在人工 `waitingOnApproval`，才能作为该授权链路已加载的证据；旧线程成功不能证明新任务已正确加载。
