...
- 主任务日志默认落在 home project 目录的 `logs/<ctxId>/<sessionId>.log.md`（默认根目录形如 `~/.oneworks/projects/<project-key>`）。
- 适配器协议转换日志默认落在 `logs/<ctxId>/<sessionId>/adapter-claude-code/*`。
- `.mock/.claude-code-router/` 更偏运行时内部状态，不是业务排查的第一入口。

推荐顺序：

1. 先看主日志，确认这次任务到底加载了哪些 rules、skills、entity、spec。
2. 再看 adapter 日志，确认真正发给模型的 request 和收到的 response 是什么。
3. 只有怀疑 CCR 复用、session 继承或 header 传递异常时，再看 `<project-home>/.mock`。
...
1. 找到本次任务目录：`<project-home>/logs/<ctxId>/`
2. 打开主日志：`<project-home>/logs/<ctxId>/<sessionId>.log.md`
...
`npx oneworks clear` 会清理日志和 cache 产物，但日志异常不一定都是 clear 没生效。
...
这类问题优先看 request 上下文和 CCR 运行态，而不是只盯 `<project-home>/logs` 目录本身。
