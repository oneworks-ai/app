...
- 每个 PR 创建、编辑或同步时都会收到一条提醒评论，提示确认是否需要沉淀经验。
...
- 软提醒 workflow：`.github/workflows/pr-experience-review.yml`
  ...
  - 不 checkout PR 代码，只通过 GitHub API upsert 带 marker 的提醒评论。
  - 如需调整提醒文案，保持 marker `<!-- oneworks:experience-review-reminder -->` 不变，避免重复评论。
