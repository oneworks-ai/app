...
更新本规则时优先使用官方当前模型页和费率页；独立评测只用于补充不确定性和外部验证，不用单一榜单覆盖仓库自己的代表性任务评测。模型、费率或速度档位变化时，按 [`model-routing-evaluation.md`](./model-routing-evaluation.md) 重跑目标任务族，再同步更新表格和路由建议。
