...
在 `.oo.config.json`、`.oo.config.yaml` 或 `.oo.config.ts` 里声明 hook 插件即可：
...
上面的 `logger` 会解析为 workspace 包 `@oneworks/plugin-logger/hooks`。
...
native hooks 可用时，One Works 会关闭对应的 bridge 事件，避免同一个事件被打两次：
...
- 优先看 `<project-home>/.mock` 下的托管配置是否落地。
- 再看 `<project-home>/logs/<ctxId>/<sessionId>/` 下的 hook 日志。
